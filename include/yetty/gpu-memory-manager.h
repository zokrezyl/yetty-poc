...
/**
 * GpuMemoryManager - Coordinates CardBufferManager and GpuTextureManager.
 *
 * Owns the metadata GPU buffer (binding 1) and shared bind group.
 * CardBufferManager owns the linear storage buffer (binding 2).
 * GpuTextureManager owns the atlas texture (binding 3,4) and texture buffer (binding 5).
 */
class GpuMemoryManager {
public:
    using Ptr = std::shared_ptr<GpuMemoryManager>;
    ...
    virtual GpuTextureManager::Ptr textureManager() const = 0;
