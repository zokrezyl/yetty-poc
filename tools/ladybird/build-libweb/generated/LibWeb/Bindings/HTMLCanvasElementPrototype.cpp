...
GC_DEFINE_ALLOCATOR(HTMLCanvasElementPrototype);

HTMLCanvasElementPrototype::HTMLCanvasElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void HTMLCanvasElementPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "toDataURL"_utf16_fly_string, to_data_url, 0, default_attributes);

    define_native_function(realm, "toBlob"_utf16_fly_string, to_blob, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "HTMLCanvasElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void HTMLCanvasElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::HTMLCanvasElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::HTMLCanvasElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLCanvasElement");
    return static_cast<HTML::HTMLCanvasElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(HTMLCanvasElementPrototype::width_getter)
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::width_getter");
...
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::width_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::height_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::height_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::get_context");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (!false || !arg0.is_null()) {
        context_id = TRY(WebIDL::to_string(vm, arg0));
    }
    ...
    return retval.visit(
        ...
        [&vm, &realm]([[maybe_unused]] GC::Root<CanvasRenderingContext2D> const& visited_union_value0) -> JS::Value {
            // These may be unused.
            (void)vm;
            (void)realm;

    return &const_cast<CanvasRenderingContext2D&>(*visited_union_value0);
...
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::to_data_url");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (!arg0.is_undefined()) {
        ...
        type = "image/png"_string;
    }

    auto arg1 = vm.argument(1);

    JS::Value quality = JS::js_undefined();
    if (!arg1.is_undefined())
        quality = arg1;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->to_data_url(type, quality); }));

    return JS::PrimitiveString::create(vm, retval);

}

JS_DEFINE_NATIVE_FUNCTION(HTMLCanvasElementPrototype::to_blob)
{
    WebIDL::log_trace(vm, "HTMLCanvasElementPrototype::to_blob");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "toBlob");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->to_blob(callback, type, quality); }));
