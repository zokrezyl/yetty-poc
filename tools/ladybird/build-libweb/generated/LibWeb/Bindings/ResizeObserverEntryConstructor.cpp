...
GC_DEFINE_ALLOCATOR(ResizeObserverEntryConstructor);

ResizeObserverEntryConstructor::ResizeObserverEntryConstructor(JS::Realm& realm)
    : NativeFunction("ResizeObserverEntry"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ResizeObserverEntryConstructor::~ResizeObserverEntryConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ResizeObserverEntryConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ResizeObserverEntry");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ResizeObserverEntryConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ResizeObserverEntryConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "ResizeObserverEntry");
}


void ResizeObserverEntryConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ResizeObserverEntry"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ResizeObserverEntryPrototype>(realm, "ResizeObserverEntry"_fly_string), 0);
