...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/DOM/AbortController.h>
...
GC_DEFINE_ALLOCATOR(AbortControllerConstructor);

AbortControllerConstructor::AbortControllerConstructor(JS::Realm& realm)
    : NativeFunction("AbortController"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "AbortController");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> AbortControllerConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "AbortControllerConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface AbortController:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<AbortControllerPrototype>(*target_realm, "AbortController"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::AbortController::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface AbortController" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOM::AbortController::construct_impl().
...
void AbortControllerConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "AbortController"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<AbortControllerPrototype>(realm, "AbortController"_fly_string), 0);
