...
#if __has_include(<LibWeb/Bindings/DocumentPrototype.h>)
#    include <LibWeb/Bindings/DocumentPrototype.h>
#endif


#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/HTMLDocument.h>
...
GC_DEFINE_ALLOCATOR(HTMLDocumentConstructor);

HTMLDocumentConstructor::HTMLDocumentConstructor(JS::Realm& realm)
    : NativeFunction("HTMLDocument"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

HTMLDocumentConstructor::~HTMLDocumentConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> HTMLDocumentConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLDocument");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLDocumentConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLDocumentConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "HTMLDocument");
}


void HTMLDocumentConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<DocumentPrototype>(realm, "Document"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLDocument"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLDocumentPrototype>(realm, "HTMLDocument"_fly_string), 0);
