...
GC_DEFINE_ALLOCATOR(KeyboardEventConstructor);

KeyboardEventConstructor::KeyboardEventConstructor(JS::Realm& realm)
    : NativeFunction("KeyboardEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "KeyboardEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> KeyboardEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "KeyboardEventConstructor::construct");
    ...
    auto char_code_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        char_code_property_value_0 = TRY(arg1.as_object().get("charCode"_utf16_fly_string));

    WebIDL::UnsignedLong char_code_value_0;

    if (!char_code_property_value_0.is_undefined())

    char_code_value_0 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, char_code_property_value_0, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        char_code_value_0 = static_cast<WebIDL::UnsignedLong>(0);

    event_init_dict.char_code = char_code_value_0;

    auto code_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        code_property_value_1 = TRY(arg1.as_object().get("code"_utf16_fly_string));

    String code_value_1;

    if (!code_property_value_1.is_undefined()) {
        if (!false || !code_property_value_1.is_null())
            code_value_1 = TRY(WebIDL::to_string(vm, code_property_value_1));
    } else {
        code_value_1 = ""_string;
    }

    event_init_dict.code = code_value_1;

    auto is_composing_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        is_composing_property_value_2 = TRY(arg1.as_object().get("isComposing"_utf16_fly_string));

    bool is_composing_value_2;

    if (!is_composing_property_value_2.is_undefined())

    is_composing_value_2 = is_composing_property_value_2.to_boolean();

    else
        is_composing_value_2 = static_cast<bool>(false);

    event_init_dict.is_composing = is_composing_value_2;

    auto key_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        key_property_value_3 = TRY(arg1.as_object().get("key"_utf16_fly_string));

    String key_value_3;

    if (!key_property_value_3.is_undefined()) {
        if (!false || !key_property_value_3.is_null())
            key_value_3 = TRY(WebIDL::to_string(vm, key_property_value_3));
    } else {
        key_value_3 = ""_string;
    }

    event_init_dict.key = key_value_3;

    auto key_code_property_value_4 = JS::js_undefined();
    if (arg1.is_object())
        key_code_property_value_4 = TRY(arg1.as_object().get("keyCode"_utf16_fly_string));

    WebIDL::UnsignedLong key_code_value_4;

    if (!key_code_property_value_4.is_undefined())

    key_code_value_4 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, key_code_property_value_4, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        key_code_value_4 = static_cast<WebIDL::UnsignedLong>(0);

    event_init_dict.key_code = key_code_value_4;

    auto location_property_value_5 = JS::js_undefined();
    if (arg1.is_object())
        location_property_value_5 = TRY(arg1.as_object().get("location"_utf16_fly_string));

    WebIDL::UnsignedLong location_value_5;

    if (!location_property_value_5.is_undefined())

    location_value_5 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, location_property_value_5, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        location_value_5 = static_cast<WebIDL::UnsignedLong>(0);

    event_init_dict.location = location_value_5;

    auto repeat_property_value_6 = JS::js_undefined();
    if (arg1.is_object())
        repeat_property_value_6 = TRY(arg1.as_object().get("repeat"_utf16_fly_string));

    bool repeat_value_6;

    if (!repeat_property_value_6.is_undefined())

    repeat_value_6 = repeat_property_value_6.to_boolean();

    else
        repeat_value_6 = static_cast<bool>(false);

    event_init_dict.repeat = repeat_value_6;

    auto alt_key_property_value_7 = JS::js_undefined();
    if (arg1.is_object())
        alt_key_property_value_7 = TRY(arg1.as_object().get("altKey"_utf16_fly_string));

    bool alt_key_value_7;

    if (!alt_key_property_value_7.is_undefined())

    alt_key_value_7 = alt_key_property_value_7.to_boolean();

    else
        alt_key_value_7 = static_cast<bool>(false);

    event_init_dict.alt_key = alt_key_value_7;

    auto ctrl_key_property_value_8 = JS::js_undefined();
    if (arg1.is_object())
        ctrl_key_property_value_8 = TRY(arg1.as_object().get("ctrlKey"_utf16_fly_string));

    bool ctrl_key_value_8;

    if (!ctrl_key_property_value_8.is_undefined())

    ctrl_key_value_8 = ctrl_key_property_value_8.to_boolean();

    else
        ctrl_key_value_8 = static_cast<bool>(false);

    event_init_dict.ctrl_key = ctrl_key_value_8;

    auto meta_key_property_value_9 = JS::js_undefined();
    if (arg1.is_object())
        meta_key_property_value_9 = TRY(arg1.as_object().get("metaKey"_utf16_fly_string));

    bool meta_key_value_9;

    if (!meta_key_property_value_9.is_undefined())

    meta_key_value_9 = meta_key_property_value_9.to_boolean();

    else
        meta_key_value_9 = static_cast<bool>(false);

    event_init_dict.meta_key = meta_key_value_9;

    auto modifier_alt_graph_property_value_10 = JS::js_undefined();
    if (arg1.is_object())
        modifier_alt_graph_property_value_10 = TRY(arg1.as_object().get("modifierAltGraph"_utf16_fly_string));

    bool modifier_alt_graph_value_10;

    if (!modifier_alt_graph_property_value_10.is_undefined())

    modifier_alt_graph_value_10 = modifier_alt_graph_property_value_10.to_boolean();

    else
        modifier_alt_graph_value_10 = static_cast<bool>(false);

    event_init_dict.modifier_alt_graph = modifier_alt_graph_value_10;

    auto modifier_caps_lock_property_value_11 = JS::js_undefined();
    if (arg1.is_object())
        modifier_caps_lock_property_value_11 = TRY(arg1.as_object().get("modifierCapsLock"_utf16_fly_string));

    bool modifier_caps_lock_value_11;

    if (!modifier_caps_lock_property_value_11.is_undefined())

    modifier_caps_lock_value_11 = modifier_caps_lock_property_value_11.to_boolean();

    else
        modifier_caps_lock_value_11 = static_cast<bool>(false);

    event_init_dict.modifier_caps_lock = modifier_caps_lock_value_11;

    auto modifier_fn_property_value_12 = JS::js_undefined();
    if (arg1.is_object())
        modifier_fn_property_value_12 = TRY(arg1.as_object().get("modifierFn"_utf16_fly_string));

    bool modifier_fn_value_12;

    if (!modifier_fn_property_value_12.is_undefined())

    modifier_fn_value_12 = modifier_fn_property_value_12.to_boolean();

    else
        modifier_fn_value_12 = static_cast<bool>(false);

    event_init_dict.modifier_fn = modifier_fn_value_12;

    auto modifier_fn_lock_property_value_13 = JS::js_undefined();
    if (arg1.is_object())
        modifier_fn_lock_property_value_13 = TRY(arg1.as_object().get("modifierFnLock"_utf16_fly_string));

    bool modifier_fn_lock_value_13;

    if (!modifier_fn_lock_property_value_13.is_undefined())

    modifier_fn_lock_value_13 = modifier_fn_lock_property_value_13.to_boolean();

    else
        modifier_fn_lock_value_13 = static_cast<bool>(false);

    event_init_dict.modifier_fn_lock = modifier_fn_lock_value_13;

    auto modifier_hyper_property_value_14 = JS::js_undefined();
    if (arg1.is_object())
        modifier_hyper_property_value_14 = TRY(arg1.as_object().get("modifierHyper"_utf16_fly_string));

    bool modifier_hyper_value_14;

    if (!modifier_hyper_property_value_14.is_undefined())

    modifier_hyper_value_14 = modifier_hyper_property_value_14.to_boolean();

    else
        modifier_hyper_value_14 = static_cast<bool>(false);

    event_init_dict.modifier_hyper = modifier_hyper_value_14;

    auto modifier_num_lock_property_value_15 = JS::js_undefined();
    if (arg1.is_object())
        modifier_num_lock_property_value_15 = TRY(arg1.as_object().get("modifierNumLock"_utf16_fly_string));

    bool modifier_num_lock_value_15;

    if (!modifier_num_lock_property_value_15.is_undefined())

    modifier_num_lock_value_15 = modifier_num_lock_property_value_15.to_boolean();

    else
        modifier_num_lock_value_15 = static_cast<bool>(false);

    event_init_dict.modifier_num_lock = modifier_num_lock_value_15;

    auto modifier_scroll_lock_property_value_16 = JS::js_undefined();
    if (arg1.is_object())
        modifier_scroll_lock_property_value_16 = TRY(arg1.as_object().get("modifierScrollLock"_utf16_fly_string));

    bool modifier_scroll_lock_value_16;

    if (!modifier_scroll_lock_property_value_16.is_undefined())

    modifier_scroll_lock_value_16 = modifier_scroll_lock_property_value_16.to_boolean();

    else
        modifier_scroll_lock_value_16 = static_cast<bool>(false);

    event_init_dict.modifier_scroll_lock = modifier_scroll_lock_value_16;

    auto modifier_super_property_value_17 = JS::js_undefined();
    if (arg1.is_object())
        modifier_super_property_value_17 = TRY(arg1.as_object().get("modifierSuper"_utf16_fly_string));

    bool modifier_super_value_17;

    if (!modifier_super_property_value_17.is_undefined())

    modifier_super_value_17 = modifier_super_property_value_17.to_boolean();

    else
        modifier_super_value_17 = static_cast<bool>(false);

    event_init_dict.modifier_super = modifier_super_value_17;

    auto modifier_symbol_property_value_18 = JS::js_undefined();
    if (arg1.is_object())
        modifier_symbol_property_value_18 = TRY(arg1.as_object().get("modifierSymbol"_utf16_fly_string));

    bool modifier_symbol_value_18;

    if (!modifier_symbol_property_value_18.is_undefined())

    modifier_symbol_value_18 = modifier_symbol_property_value_18.to_boolean();

    else
        modifier_symbol_value_18 = static_cast<bool>(false);

    event_init_dict.modifier_symbol = modifier_symbol_value_18;

    auto modifier_symbol_lock_property_value_19 = JS::js_undefined();
    if (arg1.is_object())
        modifier_symbol_lock_property_value_19 = TRY(arg1.as_object().get("modifierSymbolLock"_utf16_fly_string));

    bool modifier_symbol_lock_value_19;

    if (!modifier_symbol_lock_property_value_19.is_undefined())

    modifier_symbol_lock_value_19 = modifier_symbol_lock_property_value_19.to_boolean();

    else
        modifier_symbol_lock_value_19 = static_cast<bool>(false);

    event_init_dict.modifier_symbol_lock = modifier_symbol_lock_value_19;

    auto shift_key_property_value_20 = JS::js_undefined();
    if (arg1.is_object())
        shift_key_property_value_20 = TRY(arg1.as_object().get("shiftKey"_utf16_fly_string));

    bool shift_key_value_20;

    if (!shift_key_property_value_20.is_undefined())

    shift_key_value_20 = shift_key_property_value_20.to_boolean();

    else
        shift_key_value_20 = static_cast<bool>(false);

    event_init_dict.shift_key = shift_key_value_20;

    auto detail_property_value_21 = JS::js_undefined();
    if (arg1.is_object())
        detail_property_value_21 = TRY(arg1.as_object().get("detail"_utf16_fly_string));

    WebIDL::Long detail_value_21;

    if (!detail_property_value_21.is_undefined())

    detail_value_21 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, detail_property_value_21, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        detail_value_21 = static_cast<WebIDL::Long>(0);

    event_init_dict.detail = detail_value_21;

    auto view_property_value_22 = JS::js_undefined();
    if (arg1.is_object())
        view_property_value_22 = TRY(arg1.as_object().get("view"_utf16_fly_string));

    GC::Ptr<WindowProxy> view_value_22;

    if (!view_property_value_22.is_nullish()) {
        if (!view_property_value_22.is_object() || !is<WindowProxy>(view_property_value_22.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WindowProxy");

        view_value_22 = &static_cast<WindowProxy&>(view_property_value_22.as_object());
    }

    event_init_dict.view = view_value_22;

    auto bubbles_property_value_23 = JS::js_undefined();
    if (arg1.is_object())
        bubbles_property_value_23 = TRY(arg1.as_object().get("bubbles"_utf16_fly_string));

    bool bubbles_value_23;

    if (!bubbles_property_value_23.is_undefined())

    bubbles_value_23 = bubbles_property_value_23.to_boolean();

    else
        bubbles_value_23 = static_cast<bool>(false);

    event_init_dict.bubbles = bubbles_value_23;

    auto cancelable_property_value_24 = JS::js_undefined();
    if (arg1.is_object())
        cancelable_property_value_24 = TRY(arg1.as_object().get("cancelable"_utf16_fly_string));

    bool cancelable_value_24;

    if (!cancelable_property_value_24.is_undefined())

    cancelable_value_24 = cancelable_property_value_24.to_boolean();

    else
        cancelable_value_24 = static_cast<bool>(false);

    event_init_dict.cancelable = cancelable_value_24;

    auto composed_property_value_25 = JS::js_undefined();
    if (arg1.is_object())
        composed_property_value_25 = TRY(arg1.as_object().get("composed"_utf16_fly_string));

    bool composed_value_25;

    if (!composed_property_value_25.is_undefined())

    composed_value_25 = composed_property_value_25.to_boolean();

    else
        composed_value_25 = static_cast<bool>(false);

    event_init_dict.composed = composed_value_25;
...
void KeyboardEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "KeyboardEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<KeyboardEventPrototype>(realm, "KeyboardEvent"_fly_string), 0);
