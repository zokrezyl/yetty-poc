...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMathProduct.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMathValue.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSNumericArray.h>
...
GC_DEFINE_ALLOCATOR(CSSMathProductConstructor);

CSSMathProductConstructor::CSSMathProductConstructor(JS::Realm& realm)
    : NativeFunction("CSSMathProduct"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMathProductConstructor::~CSSMathProductConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMathProductConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMathProduct");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMathProductConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMathProductConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSMathProduct:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSMathProductPrototype>(*target_realm, "CSSMathProduct"_fly_string);
    }
    ...
    if (isinf(arg0_number) || isnan(arg0_number)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "args");
    }
    ...
    if (isinf(arg0_number) || isnan(arg0_number)) {
        ...
        Vector<Variant<double, GC::Root<CSSNumericValue>>> args;

        if (vm.argument_count() > 0) {
            args.ensure_capacity(vm.argument_count() - 0);

            for (size_t i = 0; i < vm.argument_count(); ++i) {
                auto result = TRY(arg0_to_variant(vm.argument(i)));
                args.unchecked_append(move(result));
            }
        }
    
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSMathProduct::construct_impl(realm, move(args)); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSMathProduct" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSMathProduct::construct_impl().
...
void CSSMathProductConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMathProduct"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMathProductPrototype>(realm, "CSSMathProduct"_fly_string), 0);
