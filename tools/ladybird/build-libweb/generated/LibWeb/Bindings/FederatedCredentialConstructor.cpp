...
GC_DEFINE_ALLOCATOR(FederatedCredentialConstructor);

FederatedCredentialConstructor::FederatedCredentialConstructor(JS::Realm& realm)
    : NativeFunction("FederatedCredential"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

FederatedCredentialConstructor::~FederatedCredentialConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> FederatedCredentialConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "FederatedCredential");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> FederatedCredentialConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "FederatedCredentialConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface FederatedCredential:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<FederatedCredentialPrototype>(*target_realm, "FederatedCredential"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "FederatedCredential");
    ...
    auto protocol_property_value_2 = JS::js_undefined();
    if (arg0.is_object())
        protocol_property_value_2 = TRY(arg0.as_object().get("protocol"_utf16_fly_string));

    if (!protocol_property_value_2.is_undefined()) {

    Optional<String> protocol_value_2;

    if (!protocol_property_value_2.is_undefined()) {
        if (!false || !protocol_property_value_2.is_null())
            protocol_value_2 = TRY(WebIDL::to_string(vm, protocol_property_value_2));
    }

    data.protocol = protocol_value_2;

    }

    auto provider_property_value_3 = JS::js_undefined();
    if (arg0.is_object())
        provider_property_value_3 = TRY(arg0.as_object().get("provider"_utf16_fly_string));

    if (provider_property_value_3.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "provider");

    String provider_value_3;
    if (!false || !provider_property_value_3.is_null()) {
        provider_value_3 = TRY(WebIDL::to_usv_string(vm, provider_property_value_3));
    }

    data.provider = provider_value_3;

    auto id_property_value_4 = JS::js_undefined();
    if (arg0.is_object())
        id_property_value_4 = TRY(arg0.as_object().get("id"_utf16_fly_string));
    ...
    if (!false || !id_property_value_4.is_null()) {
        id_value_4 = TRY(WebIDL::to_usv_string(vm, id_property_value_4));
    }

    data.id = id_value_4;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return FederatedCredential::construct_impl(realm, data); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface FederatedCredential" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within FederatedCredential::construct_impl().
...
void FederatedCredentialConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "FederatedCredential"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<FederatedCredentialPrototype>(realm, "FederatedCredential"_fly_string), 0);
