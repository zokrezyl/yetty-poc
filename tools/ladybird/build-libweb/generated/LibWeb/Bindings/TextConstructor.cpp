...
GC_DEFINE_ALLOCATOR(TextConstructor);

TextConstructor::TextConstructor(JS::Realm& realm)
    : NativeFunction("Text"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Text");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> TextConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "TextConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface Text:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<TextPrototype>(*target_realm, "Text"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::Text::construct_impl(realm, data); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface Text" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOM::Text::construct_impl().
...
void TextConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Text"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<TextPrototype>(realm, "Text"_fly_string), 0);
