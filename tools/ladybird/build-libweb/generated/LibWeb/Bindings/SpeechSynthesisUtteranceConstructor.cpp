...
GC_DEFINE_ALLOCATOR(SpeechSynthesisUtteranceConstructor);

SpeechSynthesisUtteranceConstructor::SpeechSynthesisUtteranceConstructor(JS::Realm& realm)
    : NativeFunction("SpeechSynthesisUtterance"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SpeechSynthesisUtteranceConstructor::~SpeechSynthesisUtteranceConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SpeechSynthesisUtteranceConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SpeechSynthesisUtterance");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SpeechSynthesisUtteranceConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SpeechSynthesisUtteranceConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface SpeechSynthesisUtterance:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<SpeechSynthesisUtterancePrototype>(*target_realm, "SpeechSynthesisUtterance"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return SpeechSynthesisUtterance::construct_impl(realm, text); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface SpeechSynthesisUtterance" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within SpeechSynthesisUtterance::construct_impl().
...
void SpeechSynthesisUtteranceConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SpeechSynthesisUtterance"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SpeechSynthesisUtterancePrototype>(realm, "SpeechSynthesisUtterance"_fly_string), 0);
