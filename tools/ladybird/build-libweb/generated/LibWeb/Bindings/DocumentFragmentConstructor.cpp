...
GC_DEFINE_ALLOCATOR(DocumentFragmentConstructor);

DocumentFragmentConstructor::DocumentFragmentConstructor(JS::Realm& realm)
    : NativeFunction("DocumentFragment"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DocumentFragment");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DocumentFragmentConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DocumentFragmentConstructor::construct");
...
void DocumentFragmentConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DocumentFragment"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DocumentFragmentPrototype>(realm, "DocumentFragment"_fly_string), 0);
