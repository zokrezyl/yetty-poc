...
GC_DEFINE_ALLOCATOR(SVGDescElementConstructor);

SVGDescElementConstructor::SVGDescElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGDescElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGDescElementConstructor::~SVGDescElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGDescElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGDescElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGDescElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGDescElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGDescElement");
}


void SVGDescElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGDescElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGDescElementPrototype>(realm, "SVGDescElement"_fly_string), 0);
