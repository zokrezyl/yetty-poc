...
GC_DEFINE_ALLOCATOR(ScrollTimelinePrototype);

ScrollTimelinePrototype::ScrollTimelinePrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

ScrollTimelinePrototype::~ScrollTimelinePrototype()
{
}

void ScrollTimelinePrototype::initialize(JS::Realm& realm)
{
    ...
    GC::Ptr<JS::NativeFunction> native_axis_setter;

    define_direct_accessor("axis"_utf16_fly_string, native_axis_getter, native_axis_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "ScrollTimeline"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void ScrollTimelinePrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<ScrollTimeline*> impl_from(JS::VM& vm)
{
    ...
    if (!is<ScrollTimeline>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ScrollTimeline");
    return static_cast<ScrollTimeline*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(ScrollTimelinePrototype::source_getter)
{
    WebIDL::log_trace(vm, "ScrollTimelinePrototype::source_getter");
...
JS_DEFINE_NATIVE_FUNCTION(ScrollTimelinePrototype::axis_getter)
{
    WebIDL::log_trace(vm, "ScrollTimelinePrototype::axis_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->axis(); }));
