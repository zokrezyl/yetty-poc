...
GC_DEFINE_ALLOCATOR(IntersectionObserverPrototype);

IntersectionObserverPrototype::IntersectionObserverPrototype([[maybe_unused]] JS::Realm& realm)
...
void IntersectionObserverPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_root_getter = JS::NativeFunction::create(realm, root_getter, 0, "root"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_root_setter;

    define_direct_accessor("root"_utf16_fly_string, native_root_getter, native_root_setter, default_attributes);

    auto native_root_margin_getter = JS::NativeFunction::create(realm, root_margin_getter, 0, "rootMargin"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_root_margin_setter;

    define_direct_accessor("rootMargin"_utf16_fly_string, native_root_margin_getter, native_root_margin_setter, default_attributes);

    auto native_scroll_margin_getter = JS::NativeFunction::create(realm, scroll_margin_getter, 0, "scrollMargin"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_scroll_margin_setter;

    define_direct_accessor("scrollMargin"_utf16_fly_string, native_scroll_margin_getter, native_scroll_margin_setter, default_attributes);

    auto native_thresholds_getter = JS::NativeFunction::create(realm, thresholds_getter, 0, "thresholds"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_thresholds_setter;

    define_direct_accessor("thresholds"_utf16_fly_string, native_thresholds_getter, native_thresholds_setter, default_attributes);

    auto native_delay_getter = JS::NativeFunction::create(realm, delay_getter, 0, "delay"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_delay_setter;

    define_direct_accessor("delay"_utf16_fly_string, native_delay_getter, native_delay_setter, default_attributes);

    auto native_track_visibility_getter = JS::NativeFunction::create(realm, track_visibility_getter, 0, "trackVisibility"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_track_visibility_setter;

    define_direct_accessor("trackVisibility"_utf16_fly_string, native_track_visibility_getter, native_track_visibility_setter, default_attributes);

    define_native_function(realm, "observe"_utf16_fly_string, observe, 1, default_attributes);

    define_native_function(realm, "unobserve"_utf16_fly_string, unobserve, 1, default_attributes);
    ...
    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "IntersectionObserver"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<IntersectionObserver::IntersectionObserver*> impl_from(JS::VM& vm)
{
    ...
    if (!is<IntersectionObserver::IntersectionObserver>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "IntersectionObserver");
    return static_cast<IntersectionObserver::IntersectionObserver*>(this_object);
}
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::root_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->root(); }));

    return retval.visit(

        [&vm, &realm]([[maybe_unused]] GC::Root<Element> const& visited_union_value0) -> JS::Value {
            // These may be unused.
            (void)vm;
            (void)realm;

    return &const_cast<Element&>(*visited_union_value0);

        },

        [&vm, &realm]([[maybe_unused]] GC::Root<Document> const& visited_union_value0) -> JS::Value {
            // These may be unused.
            (void)vm;
            (void)realm;

    return &const_cast<Document&>(*visited_union_value0);
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::root_margin_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->root_margin(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::scroll_margin_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->scroll_margin(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::thresholds_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->thresholds(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::delay_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->delay(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::track_visibility_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->track_visibility(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::observe)
{
    ...
    auto& target = static_cast<Element&>(arg0.as_object());

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->observe(target); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverPrototype::unobserve)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->unobserve(target); }));
