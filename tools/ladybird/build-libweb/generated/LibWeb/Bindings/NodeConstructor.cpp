...
GC_DEFINE_ALLOCATOR(NodeConstructor);

NodeConstructor::NodeConstructor(JS::Realm& realm)
    : NativeFunction("Node"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Node");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> NodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "NodeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "Node");
}


void NodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Node"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<NodePrototype>(realm, "Node"_fly_string), 0);
