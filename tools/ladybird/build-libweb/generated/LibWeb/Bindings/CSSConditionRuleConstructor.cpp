...
GC_DEFINE_ALLOCATOR(CSSConditionRuleConstructor);

CSSConditionRuleConstructor::CSSConditionRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSConditionRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSConditionRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSConditionRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSConditionRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSConditionRule");
}


void CSSConditionRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSConditionRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSConditionRulePrototype>(realm, "CSSConditionRule"_fly_string), 0);
