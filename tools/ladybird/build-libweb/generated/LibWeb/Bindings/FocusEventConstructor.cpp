...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/UIEvents/FocusEvent.h>
...
GC_DEFINE_ALLOCATOR(FocusEventConstructor);

FocusEventConstructor::FocusEventConstructor(JS::Realm& realm)
    : NativeFunction("FocusEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "FocusEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> FocusEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "FocusEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface FocusEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<FocusEventPrototype>(*target_realm, "FocusEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "FocusEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "FocusEventInit");

    FocusEventInit event_init_dict {};

    auto related_target_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        related_target_property_value_0 = TRY(arg1.as_object().get("relatedTarget"_utf16_fly_string));

    GC::Ptr<EventTarget> related_target_value_0;

    if (!related_target_property_value_0.is_nullish()) {
        if (!related_target_property_value_0.is_object() || !is<EventTarget>(related_target_property_value_0.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "EventTarget");

        related_target_value_0 = &static_cast<EventTarget&>(related_target_property_value_0.as_object());
    }

    event_init_dict.related_target = related_target_value_0;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return UIEvents::FocusEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface FocusEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within UIEvents::FocusEvent::construct_impl().
...
void FocusEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "FocusEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<FocusEventPrototype>(realm, "FocusEvent"_fly_string), 0);
