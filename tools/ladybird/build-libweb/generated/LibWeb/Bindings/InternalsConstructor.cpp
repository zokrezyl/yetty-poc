...
GC_DEFINE_ALLOCATOR(InternalsConstructor);

InternalsConstructor::InternalsConstructor(JS::Realm& realm)
    : NativeFunction("Internals"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

InternalsConstructor::~InternalsConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> InternalsConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Internals");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> InternalsConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "InternalsConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "Internals");
}


void InternalsConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Internals"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<InternalsPrototype>(realm, "Internals"_fly_string), 0);
