...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSPerspective.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSKeywordValue.h>
...
GC_DEFINE_ALLOCATOR(CSSPerspectiveConstructor);

CSSPerspectiveConstructor::CSSPerspectiveConstructor(JS::Realm& realm)
    : NativeFunction("CSSPerspective"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSPerspectiveConstructor::~CSSPerspectiveConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSPerspectiveConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSPerspective");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSPerspectiveConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSPerspectiveConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSPerspective:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSPerspectivePrototype>(*target_realm, "CSSPerspective"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSPerspective");

    auto arg0 = vm.argument(0);

    auto arg0_to_variant = [&vm, &realm](JS::Value arg0) -> JS::ThrowCompletionOr<Variant<GC::Root<CSSNumericValue>, String, GC::Root<CSSKeywordValue>>> {
        ...
        if (arg0.is_object()) {
            ...
            if (is<PlatformObject>(arg0_object)) {
                ...
                if (auto* arg0_result = as_if<CSSKeywordValue>(arg0_object))
                    return GC::make_root(*arg0_result);
    ...
    Variant<GC::Root<CSSNumericValue>, String, GC::Root<CSSKeywordValue>> length = TRY(arg0_to_variant(arg0));

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSPerspective::construct_impl(realm, length); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSPerspective" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSPerspective::construct_impl().
...
void CSSPerspectiveConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSPerspective"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSPerspectivePrototype>(realm, "CSSPerspective"_fly_string), 0);
