...
GC_DEFINE_ALLOCATOR(Path2DConstructor);

Path2DConstructor::Path2DConstructor(JS::Realm& realm)
    : NativeFunction("Path2D"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

Path2DConstructor::~Path2DConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> Path2DConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Path2D");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> Path2DConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "Path2DConstructor::construct");
    ...
    auto arg0_to_variant = [&vm, &realm](JS::Value arg0) -> JS::ThrowCompletionOr<Variant<GC::Root<Path2D>, String>> {
        ...
        if (arg0.is_object()) {
            ...
            if (is<PlatformObject>(arg0_object)) {
                ...
                if (auto* arg0_result = as_if<Path2D>(arg0_object))
    ...
    Optional<Variant<GC::Root<Path2D>, String>> path;
    if (!arg0.is_undefined())
        path = TRY(arg0_to_variant(arg0));
...
void Path2DConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Path2D"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<Path2DPrototype>(realm, "Path2D"_fly_string), 0);
