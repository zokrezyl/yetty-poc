...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/SVG/SVGFEDisplacementMapElement.h>
...
GC_DEFINE_ALLOCATOR(SVGFEDisplacementMapElementConstructor);

SVGFEDisplacementMapElementConstructor::SVGFEDisplacementMapElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGFEDisplacementMapElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGFEDisplacementMapElementConstructor::~SVGFEDisplacementMapElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGFEDisplacementMapElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGFEDisplacementMapElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGFEDisplacementMapElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGFEDisplacementMapElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGFEDisplacementMapElement");
}


void SVGFEDisplacementMapElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGFEDisplacementMapElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGFEDisplacementMapElementPrototype>(realm, "SVGFEDisplacementMapElement"_fly_string), 0);


    auto constant_SVG_CHANNEL_UNKNOWN_value = JS::Value(static_cast<WebIDL::UnsignedShort>(0));

    define_direct_property("SVG_CHANNEL_UNKNOWN"_utf16_fly_string, constant_SVG_CHANNEL_UNKNOWN_value, JS::Attribute::Enumerable);

    auto constant_SVG_CHANNEL_R_value = JS::Value(static_cast<WebIDL::UnsignedShort>(1));

    define_direct_property("SVG_CHANNEL_R"_utf16_fly_string, constant_SVG_CHANNEL_R_value, JS::Attribute::Enumerable);

    auto constant_SVG_CHANNEL_G_value = JS::Value(static_cast<WebIDL::UnsignedShort>(2));

    define_direct_property("SVG_CHANNEL_G"_utf16_fly_string, constant_SVG_CHANNEL_G_value, JS::Attribute::Enumerable);

    auto constant_SVG_CHANNEL_B_value = JS::Value(static_cast<WebIDL::UnsignedShort>(3));

    define_direct_property("SVG_CHANNEL_B"_utf16_fly_string, constant_SVG_CHANNEL_B_value, JS::Attribute::Enumerable);

    auto constant_SVG_CHANNEL_A_value = JS::Value(static_cast<WebIDL::UnsignedShort>(4));

    define_direct_property("SVG_CHANNEL_A"_utf16_fly_string, constant_SVG_CHANNEL_A_value, JS::Attribute::Enumerable);
