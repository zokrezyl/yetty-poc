...
GC_DEFINE_ALLOCATOR(UIEventPrototype);

UIEventPrototype::UIEventPrototype([[maybe_unused]] JS::Realm& realm)
...
void UIEventPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_which_getter = JS::NativeFunction::create(realm, which_getter, 0, "which"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_which_setter;

    define_direct_accessor("which"_utf16_fly_string, native_which_getter, native_which_setter, default_attributes);

    define_native_function(realm, "initUIEvent"_utf16_fly_string, init_ui_event, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "UIEvent"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<UIEvents::UIEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<UIEvents::UIEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "UIEvent");
    return static_cast<UIEvents::UIEvent*>(this_object);
}
...
JS_DEFINE_NATIVE_FUNCTION(UIEventPrototype::which_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->which(); }));
...
JS_DEFINE_NATIVE_FUNCTION(UIEventPrototype::init_ui_event)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->init_ui_event(type_arg, bubbles_arg, cancelable_arg, view_arg, detail_arg); }));
