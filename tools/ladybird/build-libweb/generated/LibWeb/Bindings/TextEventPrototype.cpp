...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/UIEvents/TextEvent.h>
...
GC_DEFINE_ALLOCATOR(TextEventPrototype);

TextEventPrototype::TextEventPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

TextEventPrototype::~TextEventPrototype()
{
}

void TextEventPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "initTextEvent"_utf16_fly_string, init_text_event, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "TextEvent"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void TextEventPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<UIEvents::TextEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<UIEvents::TextEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "TextEvent");
    return static_cast<UIEvents::TextEvent*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(TextEventPrototype::data_getter)
{
    WebIDL::log_trace(vm, "TextEventPrototype::data_getter");
...
JS_DEFINE_NATIVE_FUNCTION(TextEventPrototype::init_text_event)
{
    WebIDL::log_trace(vm, "TextEventPrototype::init_text_event");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "initTextEvent");
    ...
    if (!arg3.is_nullish()) {
        ...
        view = &static_cast<WindowProxy&>(arg3.as_object());
    }
    ...
    if (!arg4.is_undefined()) {
        if (!false || !arg4.is_null())
            data = TRY(WebIDL::to_string(vm, arg4));
    } else {
        data = "undefined"_string;
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->init_text_event(type, bubbles, cancelable, view, data); }));
