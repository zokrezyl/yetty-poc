...
GC_DEFINE_ALLOCATOR(CloseWatcherConstructor);

CloseWatcherConstructor::CloseWatcherConstructor(JS::Realm& realm)
    : NativeFunction("CloseWatcher"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CloseWatcherConstructor::~CloseWatcherConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CloseWatcherConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CloseWatcher");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CloseWatcherConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CloseWatcherConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CloseWatcher:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CloseWatcherPrototype>(*target_realm, "CloseWatcher"_fly_string);
    }
    ...
    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CloseWatcherOptions");

    CloseWatcherOptions options {};

    auto signal_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        signal_property_value_0 = TRY(arg0.as_object().get("signal"_utf16_fly_string));

    if (!signal_property_value_0.is_undefined()) {

    GC::Ptr<AbortSignal> signal_value_0;
    if (!signal_property_value_0.is_undefined()) {
        if (!signal_property_value_0.is_object() || !is<AbortSignal>(signal_property_value_0.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "AbortSignal");

        signal_value_0 = static_cast<AbortSignal&>(signal_property_value_0.as_object());
    }

    options.signal = signal_value_0;

    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::CloseWatcher::construct_impl(realm, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CloseWatcher" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::CloseWatcher::construct_impl().
...
void CloseWatcherConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CloseWatcher"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CloseWatcherPrototype>(realm, "CloseWatcher"_fly_string), 0);
