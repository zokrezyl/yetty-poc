...
GC_DEFINE_ALLOCATOR(HTMLCollectionConstructor);

HTMLCollectionConstructor::HTMLCollectionConstructor(JS::Realm& realm)
    : NativeFunction("HTMLCollection"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLCollection");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLCollectionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLCollectionConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "HTMLCollection");
}


void HTMLCollectionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLCollection"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLCollectionPrototype>(realm, "HTMLCollection"_fly_string), 0);
