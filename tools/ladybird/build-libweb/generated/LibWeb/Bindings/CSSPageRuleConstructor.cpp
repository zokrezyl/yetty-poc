...
GC_DEFINE_ALLOCATOR(CSSPageRuleConstructor);

CSSPageRuleConstructor::CSSPageRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSPageRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSPageRuleConstructor::~CSSPageRuleConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSPageRuleConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSPageRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSPageRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSPageRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSPageRule");
}


void CSSPageRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSPageRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSPageRulePrototype>(realm, "CSSPageRule"_fly_string), 0);
