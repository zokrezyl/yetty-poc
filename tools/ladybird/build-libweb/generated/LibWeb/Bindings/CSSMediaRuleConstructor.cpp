...
GC_DEFINE_ALLOCATOR(CSSMediaRuleConstructor);

CSSMediaRuleConstructor::CSSMediaRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSMediaRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMediaRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMediaRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMediaRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSMediaRule");
}


void CSSMediaRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMediaRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMediaRulePrototype>(realm, "CSSMediaRule"_fly_string), 0);
