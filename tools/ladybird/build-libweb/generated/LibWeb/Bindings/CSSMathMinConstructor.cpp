...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMathMin.h>
...
GC_DEFINE_ALLOCATOR(CSSMathMinConstructor);

CSSMathMinConstructor::CSSMathMinConstructor(JS::Realm& realm)
    : NativeFunction("CSSMathMin"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMathMinConstructor::~CSSMathMinConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMathMinConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMathMin");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMathMinConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMathMinConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSMathMin:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSMathMinPrototype>(*target_realm, "CSSMathMin"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSMathMin::construct_impl(realm, move(args)); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSMathMin" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSMathMin::construct_impl().
...
void CSSMathMinConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMathMin"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMathMinPrototype>(realm, "CSSMathMin"_fly_string), 0);
