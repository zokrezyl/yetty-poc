...
GC_DEFINE_ALLOCATOR(GainNodeConstructor);

GainNodeConstructor::GainNodeConstructor(JS::Realm& realm)
    : NativeFunction("GainNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

GainNodeConstructor::~GainNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> GainNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "GainNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> GainNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "GainNodeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface GainNode:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<GainNodePrototype>(*target_realm, "GainNode"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "GainNode");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "GainOptions");

    GainOptions options {};

    auto gain_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        gain_property_value_0 = TRY(arg1.as_object().get("gain"_utf16_fly_string));

    float gain_value_0;

    if (!gain_property_value_0.is_undefined())
        gain_value_0 = TRY(gain_property_value_0.to_double(vm));

    else
        gain_value_0 = 1.0;

    if (isinf(gain_value_0) || isnan(gain_value_0)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "gain");
    }
    
    options.gain = gain_value_0;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::GainNode::construct_impl(realm, context, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface GainNode" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::GainNode::construct_impl().
...
void GainNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "GainNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<GainNodePrototype>(realm, "GainNode"_fly_string), 0);
