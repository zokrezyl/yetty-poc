...
GC_DEFINE_ALLOCATOR(SVGEllipseElementConstructor);

SVGEllipseElementConstructor::SVGEllipseElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGEllipseElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGEllipseElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGEllipseElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGEllipseElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGEllipseElement");
}


void SVGEllipseElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGEllipseElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGEllipseElementPrototype>(realm, "SVGEllipseElement"_fly_string), 0);
