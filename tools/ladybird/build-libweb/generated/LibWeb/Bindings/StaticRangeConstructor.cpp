...
GC_DEFINE_ALLOCATOR(StaticRangeConstructor);

StaticRangeConstructor::StaticRangeConstructor(JS::Realm& realm)
    : NativeFunction("StaticRange"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "StaticRange");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> StaticRangeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "StaticRangeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface StaticRange:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<StaticRangePrototype>(*target_realm, "StaticRange"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "StaticRange");

    auto arg0 = vm.argument(0);

    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "StaticRangeInit");

    StaticRangeInit init {};

    auto end_container_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        end_container_property_value_0 = TRY(arg0.as_object().get("endContainer"_utf16_fly_string));

    if (end_container_property_value_0.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "endContainer");

    if (!end_container_property_value_0.is_object() || !is<Node>(end_container_property_value_0.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "Node");

    auto& end_container_value_0 = static_cast<Node&>(end_container_property_value_0.as_object());

    init.end_container = end_container_value_0;

    auto end_offset_property_value_1 = JS::js_undefined();
    if (arg0.is_object())
        end_offset_property_value_1 = TRY(arg0.as_object().get("endOffset"_utf16_fly_string));

    if (end_offset_property_value_1.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "endOffset");

    WebIDL::UnsignedLong end_offset_value_1;

    end_offset_value_1 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, end_offset_property_value_1, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    init.end_offset = end_offset_value_1;

    auto start_container_property_value_2 = JS::js_undefined();
    if (arg0.is_object())
        start_container_property_value_2 = TRY(arg0.as_object().get("startContainer"_utf16_fly_string));

    if (start_container_property_value_2.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "startContainer");

    if (!start_container_property_value_2.is_object() || !is<Node>(start_container_property_value_2.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "Node");

    auto& start_container_value_2 = static_cast<Node&>(start_container_property_value_2.as_object());

    init.start_container = start_container_value_2;

    auto start_offset_property_value_3 = JS::js_undefined();
    if (arg0.is_object())
        start_offset_property_value_3 = TRY(arg0.as_object().get("startOffset"_utf16_fly_string));

    if (start_offset_property_value_3.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "startOffset");

    WebIDL::UnsignedLong start_offset_value_3;

    start_offset_value_3 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, start_offset_property_value_3, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    init.start_offset = start_offset_value_3;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::StaticRange::construct_impl(realm, init); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface StaticRange" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOM::StaticRange::construct_impl().
...
void StaticRangeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "StaticRange"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<StaticRangePrototype>(realm, "StaticRange"_fly_string), 0);
