...
GC_DEFINE_ALLOCATOR(FormDataConstructor);

FormDataConstructor::FormDataConstructor(JS::Realm& realm)
    : NativeFunction("FormData"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<JS::Value> FormDataConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "FormData");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> FormDataConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "FormDataConstructor::construct");
    ...
    if (!arg0.is_undefined()) {
        ...
        form = static_cast<HTMLFormElement&>(arg0.as_object());
    }
    ...
    if (!arg1.is_nullish()) {
        if (!arg1.is_object() || !is<HTMLElement>(arg1.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLElement");

        submitter = &static_cast<HTMLElement&>(arg1.as_object());
    }
...
void FormDataConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "FormData"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<FormDataPrototype>(realm, "FormData"_fly_string), 0);
