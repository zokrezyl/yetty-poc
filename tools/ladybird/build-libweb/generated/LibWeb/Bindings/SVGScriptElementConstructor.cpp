...
GC_DEFINE_ALLOCATOR(SVGScriptElementConstructor);

SVGScriptElementConstructor::SVGScriptElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGScriptElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGScriptElementConstructor::~SVGScriptElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGScriptElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGScriptElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGScriptElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGScriptElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGScriptElement");
}


void SVGScriptElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGScriptElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGScriptElementPrototype>(realm, "SVGScriptElement"_fly_string), 0);
