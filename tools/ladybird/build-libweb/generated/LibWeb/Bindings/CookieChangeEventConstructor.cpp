...
GC_DEFINE_ALLOCATOR(CookieChangeEventConstructor);

CookieChangeEventConstructor::CookieChangeEventConstructor(JS::Realm& realm)
    : NativeFunction("CookieChangeEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CookieChangeEventConstructor::~CookieChangeEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CookieChangeEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CookieChangeEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CookieChangeEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CookieChangeEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CookieChangeEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CookieChangeEventPrototype>(*target_realm, "CookieChangeEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CookieChangeEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CookieChangeEventInit");

    CookieChangeEventInit event_init_dict {};

    auto changed_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        changed_property_value_0 = TRY(arg1.as_object().get("changed"_utf16_fly_string));

    if (!changed_property_value_0.is_undefined()) {

    Optional<Vector<CookieListItem>> changed_value_0;

    if (!changed_property_value_0.is_undefined()) {

    if (!changed_property_value_0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObject, changed_property_value_0);

    auto changed_property_value_0_iterator_method0 = TRY(changed_property_value_0.get_method(vm, vm.well_known_symbol_iterator()));
    if (!changed_property_value_0_iterator_method0)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotIterable, changed_property_value_0);

    auto changed_property_value_0_iterator1 = TRY(JS::get_iterator_from_method(vm, changed_property_value_0, *changed_property_value_0_iterator_method0));

    Vector<CookieListItem> changed_value_0_non_optional;

    for (;;) {
        auto next1 = TRY(JS::iterator_step(vm, changed_property_value_0_iterator1));
    ...
    if (!next_item1.is_nullish() && !next_item1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CookieListItem");

    CookieListItem sequence_item1 {};

    auto name_property_value_0 = JS::js_undefined();
    if (next_item1.is_object())
        name_property_value_0 = TRY(next_item1.as_object().get("name"_utf16_fly_string));
    ...
    sequence_item1.name = name_value_0;

    }

    auto value_property_value_1 = JS::js_undefined();
    if (next_item1.is_object())
        value_property_value_1 = TRY(next_item1.as_object().get("value"_utf16_fly_string));

    if (!value_property_value_1.is_undefined()) {

    Optional<String> value_value_1;

    if (!value_property_value_1.is_undefined()) {
        if (!false || !value_property_value_1.is_null())
            value_value_1 = TRY(WebIDL::to_usv_string(vm, value_property_value_1));
    }

    sequence_item1.value = value_value_1;

    }

    changed_value_0_non_optional.append(sequence_item1);
    }

        changed_value_0 = move(changed_value_0_non_optional);
    }

    event_init_dict.changed = changed_value_0;

    }

    auto deleted_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        deleted_property_value_1 = TRY(arg1.as_object().get("deleted"_utf16_fly_string));

    if (!deleted_property_value_1.is_undefined()) {

    Optional<Vector<CookieListItem>> deleted_value_1;

    if (!deleted_property_value_1.is_undefined()) {

    if (!deleted_property_value_1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObject, deleted_property_value_1);

    auto deleted_property_value_1_iterator_method0 = TRY(deleted_property_value_1.get_method(vm, vm.well_known_symbol_iterator()));
    if (!deleted_property_value_1_iterator_method0)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotIterable, deleted_property_value_1);

    auto deleted_property_value_1_iterator1 = TRY(JS::get_iterator_from_method(vm, deleted_property_value_1, *deleted_property_value_1_iterator_method0));

    Vector<CookieListItem> deleted_value_1_non_optional;

    for (;;) {
        auto next1 = TRY(JS::iterator_step(vm, deleted_property_value_1_iterator1));
        if (!next1.has<JS::IterationResult>())
            break;
    ...
    if (next_item1.is_object())
        name_property_value_2 = TRY(next_item1.as_object().get("name"_utf16_fly_string));
    ...
    sequence_item1.name = name_value_2;

    }

    auto value_property_value_3 = JS::js_undefined();
    if (next_item1.is_object())
        value_property_value_3 = TRY(next_item1.as_object().get("value"_utf16_fly_string));

    if (!value_property_value_3.is_undefined()) {

    Optional<String> value_value_3;

    if (!value_property_value_3.is_undefined()) {
        if (!false || !value_property_value_3.is_null())
            value_value_3 = TRY(WebIDL::to_usv_string(vm, value_property_value_3));
    }

    sequence_item1.value = value_value_3;

    }

    deleted_value_1_non_optional.append(sequence_item1);
    }

        deleted_value_1 = move(deleted_value_1_non_optional);
    }

    event_init_dict.deleted = deleted_value_1;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CookieStore::CookieChangeEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CookieChangeEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CookieStore::CookieChangeEvent::construct_impl().
...
void CookieChangeEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CookieChangeEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CookieChangeEventPrototype>(realm, "CookieChangeEvent"_fly_string), 0);
