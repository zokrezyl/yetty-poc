...
GC_DEFINE_ALLOCATOR(KeyframeEffectPrototype);

KeyframeEffectPrototype::KeyframeEffectPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

KeyframeEffectPrototype::~KeyframeEffectPrototype()
{
}

void KeyframeEffectPrototype::initialize(JS::Realm& realm)
{
    ...
    set_prototype(&ensure_web_prototype<AnimationEffectPrototype>(realm, "AnimationEffect"_fly_string));
    ...
    auto native_pseudo_element_setter = JS::NativeFunction::create(realm, pseudo_element_setter, 1, "pseudoElement"_utf16_fly_string, &realm, "set"sv);
    ...
    auto native_composite_getter = JS::NativeFunction::create(realm, composite_getter, 0, "composite"_utf16_fly_string, &realm, "get"sv);

    auto native_composite_setter = JS::NativeFunction::create(realm, composite_setter, 1, "composite"_utf16_fly_string, &realm, "set"sv);

    define_direct_accessor("composite"_utf16_fly_string, native_composite_getter, native_composite_setter, default_attributes);

    define_native_function(realm, "getKeyframes"_utf16_fly_string, get_keyframes, 0, default_attributes);

    define_native_function(realm, "setKeyframes"_utf16_fly_string, set_keyframes, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "KeyframeEffect"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void KeyframeEffectPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<KeyframeEffect*> impl_from(JS::VM& vm)
{
    ...
    if (!is<KeyframeEffect>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "KeyframeEffect");
    return static_cast<KeyframeEffect*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::target_getter)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::target_getter");
...
JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::target_setter)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::target_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    TRY(throw_dom_exception_if_needed(vm, [&] { return impl->set_target(cpp_value); }));

    return JS::js_undefined();
}

JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::pseudo_element_getter)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::pseudo_element_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::pseudo_element_setter)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::pseudo_element_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    TRY(throw_dom_exception_if_needed(vm, [&] { return impl->set_pseudo_element(cpp_value); }));

    return JS::js_undefined();
}

JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::composite_getter)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::composite_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->composite(); }));

    return JS::PrimitiveString::create(vm, Bindings::idl_enum_to_string(retval));

}

JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::composite_setter)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::composite_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    CompositeOperation cpp_value { CompositeOperation::Replace };

    auto value_string = TRY(value.to_string(vm));

    if (value_string == "replace"sv)
        cpp_value = CompositeOperation::Replace;

    else if (value_string == "add"sv)
        cpp_value = CompositeOperation::Add;

    else if (value_string == "accumulate"sv)
        cpp_value = CompositeOperation::Accumulate;

    else
        return JS::js_undefined();

    TRY(throw_dom_exception_if_needed(vm, [&] { return impl->set_composite(cpp_value); }));

    return JS::js_undefined();
}

JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::get_keyframes)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::get_keyframes");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_keyframes(); }));
...
JS_DEFINE_NATIVE_FUNCTION(KeyframeEffectPrototype::set_keyframes)
{
    WebIDL::log_trace(vm, "KeyframeEffectPrototype::set_keyframes");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "setKeyframes");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->set_keyframes(keyframes); }));
