...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/UIEvents/CompositionEvent.h>
...
GC_DEFINE_ALLOCATOR(CompositionEventConstructor);

CompositionEventConstructor::CompositionEventConstructor(JS::Realm& realm)
    : NativeFunction("CompositionEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CompositionEventConstructor::~CompositionEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CompositionEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CompositionEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CompositionEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CompositionEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CompositionEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CompositionEventPrototype>(*target_realm, "CompositionEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CompositionEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CompositionEventInit");
    ...
    if (!data_property_value_0.is_undefined()) {
        if (!false || !data_property_value_0.is_null())
            data_value_0 = TRY(WebIDL::to_string(vm, data_property_value_0));
    } else {
        data_value_0 = ""_string;
    }

    event_init_dict.data = data_value_0;

    auto detail_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        detail_property_value_1 = TRY(arg1.as_object().get("detail"_utf16_fly_string));

    WebIDL::Long detail_value_1;

    if (!detail_property_value_1.is_undefined())

    detail_value_1 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, detail_property_value_1, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        detail_value_1 = static_cast<WebIDL::Long>(0);

    event_init_dict.detail = detail_value_1;

    auto view_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        view_property_value_2 = TRY(arg1.as_object().get("view"_utf16_fly_string));

    GC::Ptr<WindowProxy> view_value_2;

    if (!view_property_value_2.is_nullish()) {
        if (!view_property_value_2.is_object() || !is<WindowProxy>(view_property_value_2.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WindowProxy");

        view_value_2 = &static_cast<WindowProxy&>(view_property_value_2.as_object());
    }

    event_init_dict.view = view_value_2;
    ...
    event_init_dict.bubbles = bubbles_value_3;
    ...
    event_init_dict.cancelable = cancelable_value_4;
    ...
    event_init_dict.composed = composed_value_5;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return UIEvents::CompositionEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CompositionEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within UIEvents::CompositionEvent::construct_impl().
...
void CompositionEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CompositionEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CompositionEventPrototype>(realm, "CompositionEvent"_fly_string), 0);
