...
GC_DEFINE_ALLOCATOR(AttrConstructor);

AttrConstructor::AttrConstructor(JS::Realm& realm)
    : NativeFunction("Attr"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

AttrConstructor::~AttrConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> AttrConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Attr");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> AttrConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "AttrConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "Attr");
}


void AttrConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Attr"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<AttrPrototype>(realm, "Attr"_fly_string), 0);
