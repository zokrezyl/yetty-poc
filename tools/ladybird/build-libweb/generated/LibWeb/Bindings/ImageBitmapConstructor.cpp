...
GC_DEFINE_ALLOCATOR(ImageBitmapConstructor);

ImageBitmapConstructor::ImageBitmapConstructor(JS::Realm& realm)
    : NativeFunction("ImageBitmap"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ImageBitmapConstructor::~ImageBitmapConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ImageBitmapConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ImageBitmap");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ImageBitmapConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ImageBitmapConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "ImageBitmap");
}


void ImageBitmapConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ImageBitmap"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ImageBitmapPrototype>(realm, "ImageBitmap"_fly_string), 0);
