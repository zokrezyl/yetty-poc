...
GC_DEFINE_ALLOCATOR(SVGElementConstructor);

SVGElementConstructor::SVGElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGElement");
}


void SVGElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGElementPrototype>(realm, "SVGElement"_fly_string), 0);
