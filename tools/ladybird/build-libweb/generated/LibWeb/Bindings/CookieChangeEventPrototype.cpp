...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CookieStore/CookieChangeEvent.h>
...
GC_DEFINE_ALLOCATOR(CookieChangeEventPrototype);

CookieChangeEventPrototype::CookieChangeEventPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

CookieChangeEventPrototype::~CookieChangeEventPrototype()
{
}

void CookieChangeEventPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_changed_getter = JS::NativeFunction::create(realm, changed_getter, 0, "changed"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_changed_setter;

    define_direct_accessor("changed"_utf16_fly_string, native_changed_getter, native_changed_setter, default_attributes);

    auto native_deleted_getter = JS::NativeFunction::create(realm, deleted_getter, 0, "deleted"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_deleted_setter;

    define_direct_accessor("deleted"_utf16_fly_string, native_deleted_getter, native_deleted_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "CookieChangeEvent"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void CookieChangeEventPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<CookieStore::CookieChangeEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<CookieStore::CookieChangeEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CookieChangeEvent");
    return static_cast<CookieStore::CookieChangeEvent*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(CookieChangeEventPrototype::changed_getter)
{
    WebIDL::log_trace(vm, "CookieChangeEventPrototype::changed_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto cached_changed = impl->cached_changed();
    if (cached_changed)
        return cached_changed;

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->changed(); }));
...
JS::Value wrapped_element0;

    {
        ...
        Optional<JS::Value> wrapped_name1_value;

    if (element0.name.has_value()) {

    wrapped_name1_value = JS::PrimitiveString::create(vm, const_cast<decltype(element0.name)&>(element0.name).release_value());

    }

        if (wrapped_name1_value.has_value())
            MUST(dictionary_object1->create_data_property("name"_utf16_fly_string, wrapped_name1_value.release_value()));

        Optional<JS::Value> wrapped_value1_value;

    if (element0.value.has_value()) {

    wrapped_value1_value = JS::PrimitiveString::create(vm, const_cast<decltype(element0.value)&>(element0.value).release_value());

    }

        if (wrapped_value1_value.has_value())
            MUST(dictionary_object1->create_data_property("value"_utf16_fly_string, wrapped_value1_value.release_value()));
    ...
    cached_changed = new_array0_0;

    impl->set_cached_changed(cached_changed);
    return cached_changed;

}

JS_DEFINE_NATIVE_FUNCTION(CookieChangeEventPrototype::deleted_getter)
{
    WebIDL::log_trace(vm, "CookieChangeEventPrototype::deleted_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto cached_deleted = impl->cached_deleted();
    if (cached_deleted)
        return cached_deleted;

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->deleted(); }));
...
JS::Value wrapped_element0;
    ...
    cached_deleted = new_array0_0;

    impl->set_cached_deleted(cached_deleted);
    return cached_deleted;
