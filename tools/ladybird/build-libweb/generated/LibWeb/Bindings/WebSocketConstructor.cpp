...
GC_DEFINE_ALLOCATOR(WebSocketConstructor);

WebSocketConstructor::WebSocketConstructor(JS::Realm& realm)
    : NativeFunction("WebSocket"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "WebSocket");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> WebSocketConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "WebSocketConstructor::construct");
    ...
    protocols.append(sequence_item1);
    }
    ...
    if (!arg1.is_undefined())
        protocols = TRY(arg1_to_variant(arg1));
...
void WebSocketConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "WebSocket"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<WebSocketPrototype>(realm, "WebSocket"_fly_string), 0);
