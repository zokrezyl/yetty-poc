...
GC_DEFINE_ALLOCATOR(NodeListConstructor);

NodeListConstructor::NodeListConstructor(JS::Realm& realm)
    : NativeFunction("NodeList"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "NodeList");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> NodeListConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "NodeListConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "NodeList");
}


void NodeListConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "NodeList"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<NodeListPrototype>(realm, "NodeList"_fly_string), 0);
