...
GC_DEFINE_ALLOCATOR(SelectionConstructor);

SelectionConstructor::SelectionConstructor(JS::Realm& realm)
    : NativeFunction("Selection"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Selection");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SelectionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SelectionConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "Selection");
}


void SelectionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Selection"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SelectionPrototype>(realm, "Selection"_fly_string), 0);
