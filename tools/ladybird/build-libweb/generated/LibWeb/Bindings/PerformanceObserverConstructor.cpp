...
GC_DEFINE_ALLOCATOR(PerformanceObserverConstructor);

PerformanceObserverConstructor::PerformanceObserverConstructor(JS::Realm& realm)
    : NativeFunction("PerformanceObserver"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

PerformanceObserverConstructor::~PerformanceObserverConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> PerformanceObserverConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "PerformanceObserver");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> PerformanceObserverConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "PerformanceObserverConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface PerformanceObserver:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<PerformanceObserverPrototype>(*target_realm, "PerformanceObserver"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "PerformanceObserver");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return PerformanceObserver::construct_impl(realm, callback); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface PerformanceObserver" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within PerformanceObserver::construct_impl().
...
void PerformanceObserverConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "PerformanceObserver"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<PerformanceObserverPrototype>(realm, "PerformanceObserver"_fly_string), 0);


    define_native_accessor(realm, "supportedEntryTypes"_utf16_fly_string, supported_entry_types_getter, nullptr, default_attributes);

}

JS_DEFINE_NATIVE_FUNCTION(PerformanceObserverConstructor::supported_entry_types_getter)
{
    WebIDL::log_trace(vm, "PerformanceObserverConstructor::supported_entry_types_getter");
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return PerformanceObserver::supported_entry_types(vm); }));
