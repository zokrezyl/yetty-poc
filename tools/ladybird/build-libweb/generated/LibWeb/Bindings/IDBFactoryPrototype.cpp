...
GC_DEFINE_ALLOCATOR(IDBFactoryPrototype);

IDBFactoryPrototype::IDBFactoryPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

IDBFactoryPrototype::~IDBFactoryPrototype()
{
}

void IDBFactoryPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "deleteDatabase"_utf16_fly_string, delete_database, 1, default_attributes);

    define_native_function(realm, "databases"_utf16_fly_string, databases, 0, default_attributes);

    define_native_function(realm, "cmp"_utf16_fly_string, cmp, 2, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "IDBFactory"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<IDBFactory*> impl_from(JS::VM& vm)
{
...
JS_DEFINE_NATIVE_FUNCTION(IDBFactoryPrototype::open)
{
    ...
    version = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLongLong>(vm, arg1, WebIDL::EnforceRange::Yes, WebIDL::Clamp::No));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->open(name, version); }));

    return &const_cast<IDBOpenDBRequest&>(*retval);
...
JS_DEFINE_NATIVE_FUNCTION(IDBFactoryPrototype::delete_database)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->delete_database(name); }));
...
JS_DEFINE_NATIVE_FUNCTION(IDBFactoryPrototype::databases)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->databases(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IDBFactoryPrototype::cmp)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->cmp(first, second); }));
