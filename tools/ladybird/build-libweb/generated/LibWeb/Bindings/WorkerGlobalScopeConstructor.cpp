...
GC_DEFINE_ALLOCATOR(WorkerGlobalScopeConstructor);

WorkerGlobalScopeConstructor::WorkerGlobalScopeConstructor(JS::Realm& realm)
    : NativeFunction("WorkerGlobalScope"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "WorkerGlobalScope");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> WorkerGlobalScopeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "WorkerGlobalScopeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "WorkerGlobalScope");
}


void WorkerGlobalScopeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "WorkerGlobalScope"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<WorkerGlobalScopePrototype>(realm, "WorkerGlobalScope"_fly_string), 0);
