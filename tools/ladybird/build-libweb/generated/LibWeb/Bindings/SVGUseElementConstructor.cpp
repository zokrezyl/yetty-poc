...
GC_DEFINE_ALLOCATOR(SVGUseElementConstructor);

SVGUseElementConstructor::SVGUseElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGUseElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGUseElementConstructor::~SVGUseElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGUseElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGUseElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGUseElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGUseElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGUseElement");
}


void SVGUseElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGUseElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGUseElementPrototype>(realm, "SVGUseElement"_fly_string), 0);
