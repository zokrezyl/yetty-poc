...
GC_DEFINE_ALLOCATOR(StyleSheetListConstructor);

StyleSheetListConstructor::StyleSheetListConstructor(JS::Realm& realm)
    : NativeFunction("StyleSheetList"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "StyleSheetList");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> StyleSheetListConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "StyleSheetListConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "StyleSheetList");
}


void StyleSheetListConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "StyleSheetList"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<StyleSheetListPrototype>(realm, "StyleSheetList"_fly_string), 0);
