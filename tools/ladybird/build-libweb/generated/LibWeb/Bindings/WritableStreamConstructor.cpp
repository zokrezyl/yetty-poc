...
GC_DEFINE_ALLOCATOR(WritableStreamConstructor);

WritableStreamConstructor::WritableStreamConstructor(JS::Realm& realm)
    : NativeFunction("WritableStream"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

WritableStreamConstructor::~WritableStreamConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> WritableStreamConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "WritableStream");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> WritableStreamConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "WritableStreamConstructor::construct");
    ...
    Optional<GC::Root<JS::Object>> underlying_sink;
    if (!arg0.is_undefined()) {
        if (!arg0.is_object())
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObject, arg0);
        underlying_sink = GC::make_root(arg0.as_object());
    }
...
void WritableStreamConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "WritableStream"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<WritableStreamPrototype>(realm, "WritableStream"_fly_string), 0);
