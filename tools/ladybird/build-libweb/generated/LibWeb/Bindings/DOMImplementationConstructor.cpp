...
GC_DEFINE_ALLOCATOR(DOMImplementationConstructor);

DOMImplementationConstructor::DOMImplementationConstructor(JS::Realm& realm)
    : NativeFunction("DOMImplementation"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DOMImplementation");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DOMImplementationConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DOMImplementationConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "DOMImplementation");
}


void DOMImplementationConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DOMImplementation"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DOMImplementationPrototype>(realm, "DOMImplementation"_fly_string), 0);
