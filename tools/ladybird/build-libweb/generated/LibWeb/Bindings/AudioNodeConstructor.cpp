...
GC_DEFINE_ALLOCATOR(AudioNodeConstructor);

AudioNodeConstructor::AudioNodeConstructor(JS::Realm& realm)
    : NativeFunction("AudioNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

AudioNodeConstructor::~AudioNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> AudioNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "AudioNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> AudioNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "AudioNodeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "AudioNode");
}


void AudioNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "AudioNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<AudioNodePrototype>(realm, "AudioNode"_fly_string), 0);
