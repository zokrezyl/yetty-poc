...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSPropertyRule.h>
...
GC_DEFINE_ALLOCATOR(CSSPropertyRulePrototype);

CSSPropertyRulePrototype::CSSPropertyRulePrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

CSSPropertyRulePrototype::~CSSPropertyRulePrototype()
{
}

void CSSPropertyRulePrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_syntax_getter = JS::NativeFunction::create(realm, syntax_getter, 0, "syntax"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_syntax_setter;

    define_direct_accessor("syntax"_utf16_fly_string, native_syntax_getter, native_syntax_setter, default_attributes);

    auto native_inherits_getter = JS::NativeFunction::create(realm, inherits_getter, 0, "inherits"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_inherits_setter;

    define_direct_accessor("inherits"_utf16_fly_string, native_inherits_getter, native_inherits_setter, default_attributes);

    auto native_initial_value_getter = JS::NativeFunction::create(realm, initial_value_getter, 0, "initialValue"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_initial_value_setter;

    define_direct_accessor("initialValue"_utf16_fly_string, native_initial_value_getter, native_initial_value_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "CSSPropertyRule"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void CSSPropertyRulePrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<CSS::CSSPropertyRule*> impl_from(JS::VM& vm)
{
    ...
    if (!is<CSS::CSSPropertyRule>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CSSPropertyRule");
    return static_cast<CSS::CSSPropertyRule*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(CSSPropertyRulePrototype::name_getter)
{
    WebIDL::log_trace(vm, "CSSPropertyRulePrototype::name_getter");
...
JS_DEFINE_NATIVE_FUNCTION(CSSPropertyRulePrototype::syntax_getter)
{
    WebIDL::log_trace(vm, "CSSPropertyRulePrototype::syntax_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->syntax(); }));

    return JS::PrimitiveString::create(vm, retval);

}

JS_DEFINE_NATIVE_FUNCTION(CSSPropertyRulePrototype::inherits_getter)
{
    WebIDL::log_trace(vm, "CSSPropertyRulePrototype::inherits_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->inherits(); }));

    return JS::Value(retval);

}

JS_DEFINE_NATIVE_FUNCTION(CSSPropertyRulePrototype::initial_value_getter)
{
    WebIDL::log_trace(vm, "CSSPropertyRulePrototype::initial_value_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->initial_value(); }));
