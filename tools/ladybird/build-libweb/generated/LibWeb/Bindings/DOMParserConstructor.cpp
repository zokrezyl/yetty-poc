...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/DOMParser.h>
...
GC_DEFINE_ALLOCATOR(DOMParserConstructor);

DOMParserConstructor::DOMParserConstructor(JS::Realm& realm)
    : NativeFunction("DOMParser"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DOMParser");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DOMParserConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DOMParserConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DOMParser:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DOMParserPrototype>(*target_realm, "DOMParser"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::DOMParser::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DOMParser" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::DOMParser::construct_impl().
...
void DOMParserConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DOMParser"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DOMParserPrototype>(realm, "DOMParser"_fly_string), 0);
