...
GC_DEFINE_ALLOCATOR(MediaSourceConstructor);

MediaSourceConstructor::MediaSourceConstructor(JS::Realm& realm)
    : NativeFunction("MediaSource"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<JS::Value> MediaSourceConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MediaSource");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MediaSourceConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MediaSourceConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface MediaSource:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<MediaSourcePrototype>(*target_realm, "MediaSource"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return MediaSourceExtensions::MediaSource::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface MediaSource" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within MediaSourceExtensions::MediaSource::construct_impl().
...
void MediaSourceConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MediaSource"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MediaSourcePrototype>(realm, "MediaSource"_fly_string), 0);


    define_native_accessor(realm, "canConstructInDedicatedWorker"_utf16_fly_string, can_construct_in_dedicated_worker_getter, nullptr, default_attributes);

    define_native_function(realm, "isTypeSupported"_utf16_fly_string, is_type_supported, 1, JS::Attribute::Writable | JS::Attribute::Enumerable | JS::Attribute::Configurable);

}

JS_DEFINE_NATIVE_FUNCTION(MediaSourceConstructor::can_construct_in_dedicated_worker_getter)
{
    WebIDL::log_trace(vm, "MediaSourceConstructor::can_construct_in_dedicated_worker_getter");
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return MediaSourceExtensions::MediaSource::can_construct_in_dedicated_worker(vm); }));

    return JS::Value(retval);

}

JS_DEFINE_NATIVE_FUNCTION(MediaSourceConstructor::is_type_supported)
{
    WebIDL::log_trace(vm, "MediaSourceConstructor::is_type_supported");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "isTypeSupported");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return MediaSourceExtensions::MediaSource::is_type_supported(vm, type); }));
