...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/WebAudio/ConstantSourceNode.h>
...
GC_DEFINE_ALLOCATOR(ConstantSourceNodeConstructor);

ConstantSourceNodeConstructor::ConstantSourceNodeConstructor(JS::Realm& realm)
    : NativeFunction("ConstantSourceNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ConstantSourceNodeConstructor::~ConstantSourceNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ConstantSourceNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ConstantSourceNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ConstantSourceNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ConstantSourceNodeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface ConstantSourceNode:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<ConstantSourceNodePrototype>(*target_realm, "ConstantSourceNode"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "ConstantSourceNode");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ConstantSourceOptions");

    ConstantSourceOptions options {};

    auto offset_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        offset_property_value_0 = TRY(arg1.as_object().get("offset"_utf16_fly_string));

    float offset_value_0;

    if (!offset_property_value_0.is_undefined())
        offset_value_0 = TRY(offset_property_value_0.to_double(vm));

    else
        offset_value_0 = 1;

    if (isinf(offset_value_0) || isnan(offset_value_0)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "offset");
    }
    
    options.offset = offset_value_0;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::ConstantSourceNode::construct_impl(realm, context, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface ConstantSourceNode" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::ConstantSourceNode::construct_impl().
...
void ConstantSourceNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ConstantSourceNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ConstantSourceNodePrototype>(realm, "ConstantSourceNode"_fly_string), 0);
