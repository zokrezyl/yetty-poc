...
GC_DEFINE_ALLOCATOR(SVGLineElementConstructor);

SVGLineElementConstructor::SVGLineElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGLineElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGLineElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGLineElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGLineElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGLineElement");
}


void SVGLineElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGLineElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGLineElementPrototype>(realm, "SVGLineElement"_fly_string), 0);
