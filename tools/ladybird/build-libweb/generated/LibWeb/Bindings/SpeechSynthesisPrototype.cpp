...
GC_DEFINE_ALLOCATOR(SpeechSynthesisPrototype);

SpeechSynthesisPrototype::SpeechSynthesisPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

SpeechSynthesisPrototype::~SpeechSynthesisPrototype()
{
}

void SpeechSynthesisPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_speaking_getter = JS::NativeFunction::create(realm, speaking_getter, 0, "speaking"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_speaking_setter;

    define_direct_accessor("speaking"_utf16_fly_string, native_speaking_getter, native_speaking_setter, default_attributes);
    ...
    auto native_onvoiceschanged_getter = JS::NativeFunction::create(realm, onvoiceschanged_getter, 0, "onvoiceschanged"_utf16_fly_string, &realm, "get"sv);

    auto native_onvoiceschanged_setter = JS::NativeFunction::create(realm, onvoiceschanged_setter, 1, "onvoiceschanged"_utf16_fly_string, &realm, "set"sv);

    define_direct_accessor("onvoiceschanged"_utf16_fly_string, native_onvoiceschanged_getter, native_onvoiceschanged_setter, default_attributes);

        define_direct_property("speak"_utf16_fly_string, JS::js_undefined(), default_attributes | JS::Attribute::Unimplemented);
            
        define_direct_property("pause"_utf16_fly_string, JS::js_undefined(), default_attributes | JS::Attribute::Unimplemented);
            
        define_direct_property("resume"_utf16_fly_string, JS::js_undefined(), default_attributes | JS::Attribute::Unimplemented);
            
    define_native_function(realm, "cancel"_utf16_fly_string, cancel, 0, default_attributes);

    define_native_function(realm, "getVoices"_utf16_fly_string, get_voices, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "SpeechSynthesis"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<SpeechSynthesis*> impl_from(JS::VM& vm)
{
...
JS_DEFINE_NATIVE_FUNCTION(SpeechSynthesisPrototype::speaking_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->speaking(); }));
...
JS_DEFINE_NATIVE_FUNCTION(SpeechSynthesisPrototype::onvoiceschanged_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->onvoiceschanged(); }));
...
JS_DEFINE_NATIVE_FUNCTION(SpeechSynthesisPrototype::onvoiceschanged_setter)
{
    ...
    TRY(throw_dom_exception_if_needed(vm, [&] { return impl->set_onvoiceschanged(cpp_value); }));
...
JS_DEFINE_NATIVE_FUNCTION(SpeechSynthesisPrototype::get_voices)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_voices(); }));
