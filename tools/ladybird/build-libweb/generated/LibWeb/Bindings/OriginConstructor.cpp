...
GC_DEFINE_ALLOCATOR(OriginConstructor);

OriginConstructor::OriginConstructor(JS::Realm& realm)
    : NativeFunction("Origin"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

OriginConstructor::~OriginConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> OriginConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Origin");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> OriginConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "OriginConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface Origin:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<OriginPrototype>(*target_realm, "Origin"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOMURL::Origin::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface Origin" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOMURL::Origin::construct_impl().
...
void OriginConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Origin"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<OriginPrototype>(realm, "Origin"_fly_string), 0);


    define_native_function(realm, "from"_utf16_fly_string, from, 1, JS::Attribute::Writable | JS::Attribute::Enumerable | JS::Attribute::Configurable);

}

JS_DEFINE_NATIVE_FUNCTION(OriginConstructor::from)
{
    WebIDL::log_trace(vm, "OriginConstructor::from");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return DOMURL::Origin::from(vm, value); }));

    return &const_cast<Origin&>(*retval);
