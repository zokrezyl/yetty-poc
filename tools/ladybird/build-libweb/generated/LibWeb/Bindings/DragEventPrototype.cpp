...
GC_DEFINE_ALLOCATOR(DragEventPrototype);

DragEventPrototype::DragEventPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

DragEventPrototype::~DragEventPrototype()
{
}

void DragEventPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_data_transfer_getter = JS::NativeFunction::create(realm, data_transfer_getter, 0, "dataTransfer"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_data_transfer_setter;

    define_direct_accessor("dataTransfer"_utf16_fly_string, native_data_transfer_getter, native_data_transfer_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "DragEvent"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void DragEventPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::DragEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::DragEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DragEvent");
    return static_cast<HTML::DragEvent*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(DragEventPrototype::data_transfer_getter)
{
    WebIDL::log_trace(vm, "DragEventPrototype::data_transfer_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->data_transfer(); }));
