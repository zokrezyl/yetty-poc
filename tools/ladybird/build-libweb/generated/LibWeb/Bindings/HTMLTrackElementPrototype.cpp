...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/HTMLTrackElement.h>
...
GC_DEFINE_ALLOCATOR(HTMLTrackElementPrototype);

HTMLTrackElementPrototype::HTMLTrackElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void HTMLTrackElementPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_kind_setter = JS::NativeFunction::create(realm, kind_setter, 1, "kind"_utf16_fly_string, &realm, "set"sv);
    ...
    auto native_srclang_getter = JS::NativeFunction::create(realm, srclang_getter, 0, "srclang"_utf16_fly_string, &realm, "get"sv);

    auto native_srclang_setter = JS::NativeFunction::create(realm, srclang_setter, 1, "srclang"_utf16_fly_string, &realm, "set"sv);

    define_direct_accessor("srclang"_utf16_fly_string, native_srclang_getter, native_srclang_setter, default_attributes);
    ...
    auto native_default_setter = JS::NativeFunction::create(realm, default_setter, 1, "default"_utf16_fly_string, &realm, "set"sv);
    ...
    auto constant_LOADED_value = JS::Value(static_cast<WebIDL::UnsignedShort>(2));

    define_direct_property("LOADED"_utf16_fly_string, constant_LOADED_value, JS::Attribute::Enumerable);

    auto constant_ERROR_value = JS::Value(static_cast<WebIDL::UnsignedShort>(3));

    define_direct_property("ERROR"_utf16_fly_string, constant_ERROR_value, JS::Attribute::Enumerable);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "HTMLTrackElement"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::HTMLTrackElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::HTMLTrackElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLTrackElement");
    return static_cast<HTML::HTMLTrackElement*>(this_object);
}
...
JS_DEFINE_NATIVE_FUNCTION(HTMLTrackElementPrototype::kind_getter)
{
    ...
    auto contentAttributeValue = impl->attribute("kind"_fly_string);

    auto retval = contentAttributeValue.value_or(String {});

    auto did_set_to_missing_value = false;
    if (!contentAttributeValue.has_value()) {
        retval = "subtitles"_string;
        did_set_to_missing_value = true;
    }

    Array valid_values { "subtitles"_string, "captions"_string, "descriptions"_string, "chapters"_string, "metadata"_string };
...
JS_DEFINE_NATIVE_FUNCTION(HTMLTrackElementPrototype::kind_setter)
{
    ...
    impl->set_attribute_value("kind"_fly_string, cpp_value);
...
JS_DEFINE_NATIVE_FUNCTION(HTMLTrackElementPrototype::srclang_getter)
{
    ...
    auto contentAttributeValue = impl->attribute("srclang"_fly_string);
...
JS_DEFINE_NATIVE_FUNCTION(HTMLTrackElementPrototype::srclang_setter)
{
    ...
    impl->set_attribute_value("srclang"_fly_string, cpp_value);
...
JS_DEFINE_NATIVE_FUNCTION(HTMLTrackElementPrototype::label_getter)
{
    ...
    auto contentAttributeValue = impl->attribute("label"_fly_string);
...
JS_DEFINE_NATIVE_FUNCTION(HTMLTrackElementPrototype::label_setter)
{
    ...
    impl->set_attribute_value("label"_fly_string, cpp_value);
...
{
    WebIDL::log_trace(vm, "HTMLTrackElementPrototype::default_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    auto retval = impl->has_attribute("default"_fly_string);
...
{
    WebIDL::log_trace(vm, "HTMLTrackElementPrototype::default_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (!cpp_value)
        impl->remove_attribute("default"_fly_string);
    else
        impl->set_attribute_value("default"_fly_string, String {});
