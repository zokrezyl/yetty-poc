...
GC_DEFINE_ALLOCATOR(CSSRuleListConstructor);

CSSRuleListConstructor::CSSRuleListConstructor(JS::Realm& realm)
    : NativeFunction("CSSRuleList"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSRuleList");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSRuleListConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSRuleListConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSRuleList");
}


void CSSRuleListConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSRuleList"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSRuleListPrototype>(realm, "CSSRuleList"_fly_string), 0);
