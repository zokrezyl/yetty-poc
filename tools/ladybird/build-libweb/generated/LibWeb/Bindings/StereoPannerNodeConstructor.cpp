...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/WebAudio/StereoPannerNode.h>
...
GC_DEFINE_ALLOCATOR(StereoPannerNodeConstructor);

StereoPannerNodeConstructor::StereoPannerNodeConstructor(JS::Realm& realm)
    : NativeFunction("StereoPannerNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

StereoPannerNodeConstructor::~StereoPannerNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> StereoPannerNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "StereoPannerNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> StereoPannerNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "StereoPannerNodeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface StereoPannerNode:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<StereoPannerNodePrototype>(*target_realm, "StereoPannerNode"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "StereoPannerNode");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "StereoPannerOptions");

    StereoPannerOptions options {};

    auto pan_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        pan_property_value_0 = TRY(arg1.as_object().get("pan"_utf16_fly_string));

    float pan_value_0;

    if (!pan_property_value_0.is_undefined())
        pan_value_0 = TRY(pan_property_value_0.to_double(vm));

    else
        pan_value_0 = 0;

    if (isinf(pan_value_0) || isnan(pan_value_0)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "pan");
    }
    
    options.pan = pan_value_0;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::StereoPannerNode::construct_impl(realm, context, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface StereoPannerNode" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::StereoPannerNode::construct_impl().
...
void StereoPannerNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "StereoPannerNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<StereoPannerNodePrototype>(realm, "StereoPannerNode"_fly_string), 0);
