...
GC_DEFINE_ALLOCATOR(XPathResultConstructor);

XPathResultConstructor::XPathResultConstructor(JS::Realm& realm)
    : NativeFunction("XPathResult"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

XPathResultConstructor::~XPathResultConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> XPathResultConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "XPathResult");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> XPathResultConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "XPathResultConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "XPathResult");
}


void XPathResultConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "XPathResult"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<XPathResultPrototype>(realm, "XPathResult"_fly_string), 0);
