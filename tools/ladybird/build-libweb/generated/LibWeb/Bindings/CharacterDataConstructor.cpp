...
GC_DEFINE_ALLOCATOR(CharacterDataConstructor);

CharacterDataConstructor::CharacterDataConstructor(JS::Realm& realm)
    : NativeFunction("CharacterData"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CharacterData");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CharacterDataConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CharacterDataConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CharacterData");
}


void CharacterDataConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<NodePrototype>(realm, "Node"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CharacterData"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CharacterDataPrototype>(realm, "CharacterData"_fly_string), 0);
