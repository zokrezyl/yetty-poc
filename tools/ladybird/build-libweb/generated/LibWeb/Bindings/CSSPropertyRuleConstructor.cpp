...
GC_DEFINE_ALLOCATOR(CSSPropertyRuleConstructor);

CSSPropertyRuleConstructor::CSSPropertyRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSPropertyRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSPropertyRuleConstructor::~CSSPropertyRuleConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSPropertyRuleConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSPropertyRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSPropertyRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSPropertyRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSPropertyRule");
}


void CSSPropertyRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSPropertyRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSPropertyRulePrototype>(realm, "CSSPropertyRule"_fly_string), 0);
