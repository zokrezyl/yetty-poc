...
GC_DEFINE_ALLOCATOR(StyleSheetConstructor);

StyleSheetConstructor::StyleSheetConstructor(JS::Realm& realm)
    : NativeFunction("StyleSheet"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "StyleSheet");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> StyleSheetConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "StyleSheetConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "StyleSheet");
}


void StyleSheetConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "StyleSheet"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<StyleSheetPrototype>(realm, "StyleSheet"_fly_string), 0);
