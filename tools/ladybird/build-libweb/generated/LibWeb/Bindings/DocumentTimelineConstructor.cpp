...
GC_DEFINE_ALLOCATOR(DocumentTimelineConstructor);

DocumentTimelineConstructor::DocumentTimelineConstructor(JS::Realm& realm)
    : NativeFunction("DocumentTimeline"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DocumentTimelineConstructor::~DocumentTimelineConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DocumentTimelineConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DocumentTimeline");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DocumentTimelineConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DocumentTimelineConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DocumentTimeline:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DocumentTimelinePrototype>(*target_realm, "DocumentTimeline"_fly_string);
    }
    ...
    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DocumentTimelineOptions");

    DocumentTimelineOptions options {};

    auto origin_time_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        origin_time_property_value_0 = TRY(arg0.as_object().get("originTime"_utf16_fly_string));

    double origin_time_value_0;

    if (!origin_time_property_value_0.is_undefined())
        origin_time_value_0 = TRY(origin_time_property_value_0.to_double(vm));

    else
        origin_time_value_0 = 0;

    if (isinf(origin_time_value_0) || isnan(origin_time_value_0)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "originTime");
    }
    
    options.origin_time = origin_time_value_0;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DocumentTimeline::construct_impl(realm, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DocumentTimeline" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DocumentTimeline::construct_impl().
...
void DocumentTimelineConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DocumentTimeline"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DocumentTimelinePrototype>(realm, "DocumentTimeline"_fly_string), 0);
