...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/Speech/SpeechRecognitionPhrase.h>
...
GC_DEFINE_ALLOCATOR(SpeechRecognitionPhraseConstructor);

SpeechRecognitionPhraseConstructor::SpeechRecognitionPhraseConstructor(JS::Realm& realm)
    : NativeFunction("SpeechRecognitionPhrase"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SpeechRecognitionPhraseConstructor::~SpeechRecognitionPhraseConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SpeechRecognitionPhraseConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SpeechRecognitionPhrase");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SpeechRecognitionPhraseConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SpeechRecognitionPhraseConstructor::construct");
    ...
    if (!false || !arg0.is_null()) {
        phrase = TRY(WebIDL::to_string(vm, arg0));
    }
    ...
    if (!arg1.is_undefined())
        boost = TRY(arg1.to_double(vm));

    else
        boost = 1.0;

    if (isinf(boost) || isnan(boost)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "boost");
    }
    
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return SpeechRecognitionPhrase::construct_impl(realm, phrase, boost); }));
    ...
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within SpeechRecognitionPhrase::construct_impl().
...
void SpeechRecognitionPhraseConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SpeechRecognitionPhrase"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SpeechRecognitionPhrasePrototype>(realm, "SpeechRecognitionPhrase"_fly_string), 0);
