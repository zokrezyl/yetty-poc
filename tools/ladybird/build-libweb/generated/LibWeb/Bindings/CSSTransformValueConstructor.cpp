...
GC_DEFINE_ALLOCATOR(CSSTransformValueConstructor);

CSSTransformValueConstructor::CSSTransformValueConstructor(JS::Realm& realm)
    : NativeFunction("CSSTransformValue"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSTransformValueConstructor::~CSSTransformValueConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSTransformValueConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSTransformValue");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSTransformValueConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSTransformValueConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSTransformValue:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSTransformValuePrototype>(*target_realm, "CSSTransformValue"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSTransformValue");
    ...
    GC::RootVector<GC::Root<CSSTransformComponent>> transforms { vm.heap() };
    ...
    if (!next_item1.is_object() || !is<CSSTransformComponent>(next_item1.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CSSTransformComponent");

    auto& sequence_item1 = static_cast<CSSTransformComponent&>(next_item1.as_object());

    transforms.append(sequence_item1);
    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSTransformValue::construct_impl(realm, transforms); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSTransformValue" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSTransformValue::construct_impl().
...
void CSSTransformValueConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSTransformValue"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSTransformValuePrototype>(realm, "CSSTransformValue"_fly_string), 0);
