...
GC_DEFINE_ALLOCATOR(ManagedMediaSourceConstructor);

ManagedMediaSourceConstructor::ManagedMediaSourceConstructor(JS::Realm& realm)
    : NativeFunction("ManagedMediaSource"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ManagedMediaSourceConstructor::~ManagedMediaSourceConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ManagedMediaSourceConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ManagedMediaSource");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ManagedMediaSourceConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ManagedMediaSourceConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface ManagedMediaSource:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<ManagedMediaSourcePrototype>(*target_realm, "ManagedMediaSource"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return MediaSourceExtensions::ManagedMediaSource::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface ManagedMediaSource" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within MediaSourceExtensions::ManagedMediaSource::construct_impl().
...
void ManagedMediaSourceConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<MediaSourcePrototype>(realm, "MediaSource"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ManagedMediaSource"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ManagedMediaSourcePrototype>(realm, "ManagedMediaSource"_fly_string), 0);
