...
GC_DEFINE_ALLOCATOR(TextEncoderConstructor);

TextEncoderConstructor::TextEncoderConstructor(JS::Realm& realm)
    : NativeFunction("TextEncoder"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "TextEncoder");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> TextEncoderConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "TextEncoderConstructor::construct");
...
void TextEncoderConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "TextEncoder"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<TextEncoderPrototype>(realm, "TextEncoder"_fly_string), 0);
