...
GC_DEFINE_ALLOCATOR(SpeechSynthesisConstructor);

SpeechSynthesisConstructor::SpeechSynthesisConstructor(JS::Realm& realm)
    : NativeFunction("SpeechSynthesis"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SpeechSynthesisConstructor::~SpeechSynthesisConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SpeechSynthesisConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SpeechSynthesis");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SpeechSynthesisConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SpeechSynthesisConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SpeechSynthesis");
}


void SpeechSynthesisConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SpeechSynthesis"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SpeechSynthesisPrototype>(realm, "SpeechSynthesis"_fly_string), 0);
