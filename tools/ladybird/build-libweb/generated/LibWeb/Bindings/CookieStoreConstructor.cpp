...
GC_DEFINE_ALLOCATOR(CookieStoreConstructor);

CookieStoreConstructor::CookieStoreConstructor(JS::Realm& realm)
    : NativeFunction("CookieStore"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CookieStoreConstructor::~CookieStoreConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CookieStoreConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CookieStore");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CookieStoreConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CookieStoreConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CookieStore");
}


void CookieStoreConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CookieStore"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CookieStorePrototype>(realm, "CookieStore"_fly_string), 0);
