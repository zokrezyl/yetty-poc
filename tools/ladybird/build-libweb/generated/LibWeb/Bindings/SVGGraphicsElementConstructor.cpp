...
GC_DEFINE_ALLOCATOR(SVGGraphicsElementConstructor);

SVGGraphicsElementConstructor::SVGGraphicsElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGGraphicsElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGGraphicsElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGGraphicsElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGGraphicsElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGGraphicsElement");
}


void SVGGraphicsElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGGraphicsElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGGraphicsElementPrototype>(realm, "SVGGraphicsElement"_fly_string), 0);
