...
GC_DEFINE_ALLOCATOR(EventTargetConstructor);

EventTargetConstructor::EventTargetConstructor(JS::Realm& realm)
    : NativeFunction("EventTarget"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "EventTarget");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> EventTargetConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "EventTargetConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface EventTarget:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<EventTargetPrototype>(*target_realm, "EventTarget"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::EventTarget::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface EventTarget" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOM::EventTarget::construct_impl().
...
void EventTargetConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "EventTarget"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<EventTargetPrototype>(realm, "EventTarget"_fly_string), 0);
