...
GC_DEFINE_ALLOCATOR(AnimationConstructor);

AnimationConstructor::AnimationConstructor(JS::Realm& realm)
    : NativeFunction("Animation"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

AnimationConstructor::~AnimationConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> AnimationConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Animation");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> AnimationConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "AnimationConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface Animation:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<AnimationPrototype>(*target_realm, "Animation"_fly_string);
    }
    ...
    GC::Ptr<AnimationEffect> effect;

    if (!arg0.is_nullish()) {
        if (!arg0.is_object() || !is<AnimationEffect>(arg0.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "AnimationEffect");

        effect = &static_cast<AnimationEffect&>(arg0.as_object());
    }

    auto maybe_arg1 = vm.argument_count() >= 2 ? Optional<JS::Value> { vm.argument(1) } : OptionalNone {};

    Optional<GC::Ptr<AnimationTimeline>> timeline;
    if (maybe_arg1.has_value()) {
        auto arg1 = maybe_arg1.release_value();

    if (!arg1.is_nullish()) {
        if (!arg1.is_object() || !is<AnimationTimeline>(arg1.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "AnimationTimeline");

        timeline = &static_cast<AnimationTimeline&>(arg1.as_object());
    }

    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Animation::construct_impl(realm, effect, timeline); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface Animation" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Animation::construct_impl().
...
void AnimationConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Animation"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<AnimationPrototype>(realm, "Animation"_fly_string), 0);
