...
GC_DEFINE_ALLOCATOR(SVGGraphicsElementPrototype);

SVGGraphicsElementPrototype::SVGGraphicsElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void SVGGraphicsElementPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_transform_getter = JS::NativeFunction::create(realm, transform_getter, 0, "transform"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_transform_setter;

    define_direct_accessor("transform"_utf16_fly_string, native_transform_getter, native_transform_setter, default_attributes);

    define_native_function(realm, "getBBox"_utf16_fly_string, get_b_box, 0, default_attributes);

    define_native_function(realm, "getCTM"_utf16_fly_string, get_ctm, 0, default_attributes);

    define_native_function(realm, "getScreenCTM"_utf16_fly_string, get_screen_ctm, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "SVGGraphicsElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void SVGGraphicsElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<SVG::SVGGraphicsElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<SVG::SVGGraphicsElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "SVGGraphicsElement");
    return static_cast<SVG::SVGGraphicsElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(SVGGraphicsElementPrototype::transform_getter)
{
    WebIDL::log_trace(vm, "SVGGraphicsElementPrototype::transform_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->transform(); }));

    return &const_cast<SVGAnimatedTransformList&>(*retval);

}

JS_DEFINE_NATIVE_FUNCTION(SVGGraphicsElementPrototype::get_b_box)
{
    WebIDL::log_trace(vm, "SVGGraphicsElementPrototype::get_b_box");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "SVGBoundingBoxOptions");

    SVGBoundingBoxOptions options {};

    auto clipped_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        clipped_property_value_0 = TRY(arg0.as_object().get("clipped"_utf16_fly_string));

    bool clipped_value_0;

    if (!clipped_property_value_0.is_undefined())

    clipped_value_0 = clipped_property_value_0.to_boolean();

    else
        clipped_value_0 = static_cast<bool>(false);

    options.clipped = clipped_value_0;

    auto fill_property_value_1 = JS::js_undefined();
    if (arg0.is_object())
        fill_property_value_1 = TRY(arg0.as_object().get("fill"_utf16_fly_string));

    bool fill_value_1;

    if (!fill_property_value_1.is_undefined())

    fill_value_1 = fill_property_value_1.to_boolean();

    else
        fill_value_1 = static_cast<bool>(true);

    options.fill = fill_value_1;

    auto markers_property_value_2 = JS::js_undefined();
    if (arg0.is_object())
        markers_property_value_2 = TRY(arg0.as_object().get("markers"_utf16_fly_string));

    bool markers_value_2;

    if (!markers_property_value_2.is_undefined())

    markers_value_2 = markers_property_value_2.to_boolean();

    else
        markers_value_2 = static_cast<bool>(false);

    options.markers = markers_value_2;

    auto stroke_property_value_3 = JS::js_undefined();
    if (arg0.is_object())
        stroke_property_value_3 = TRY(arg0.as_object().get("stroke"_utf16_fly_string));

    bool stroke_value_3;

    if (!stroke_property_value_3.is_undefined())

    stroke_value_3 = stroke_property_value_3.to_boolean();

    else
        stroke_value_3 = static_cast<bool>(false);

    options.stroke = stroke_value_3;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_b_box(options); }));

    return &const_cast<DOMRect&>(*retval);

}

JS_DEFINE_NATIVE_FUNCTION(SVGGraphicsElementPrototype::get_ctm)
{
    WebIDL::log_trace(vm, "SVGGraphicsElementPrototype::get_ctm");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_ctm(); }));
...
JS_DEFINE_NATIVE_FUNCTION(SVGGraphicsElementPrototype::get_screen_ctm)
{
    WebIDL::log_trace(vm, "SVGGraphicsElementPrototype::get_screen_ctm");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_screen_ctm(); }));
