...
GC_DEFINE_ALLOCATOR(ReadableStreamConstructor);

ReadableStreamConstructor::ReadableStreamConstructor(JS::Realm& realm)
    : NativeFunction("ReadableStream"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ReadableStreamConstructor::~ReadableStreamConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ReadableStreamConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ReadableStream");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ReadableStreamConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ReadableStreamConstructor::construct");
    ...
    Optional<GC::Root<JS::Object>> underlying_source;
    if (!arg0.is_undefined()) {
        if (!arg0.is_object())
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObject, arg0);
        underlying_source = GC::make_root(arg0.as_object());
    }
    ...
    strategy.high_water_mark = high_water_mark_value_0;
...
void ReadableStreamConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ReadableStream"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ReadableStreamPrototype>(realm, "ReadableStream"_fly_string), 0);
...
JS_DEFINE_NATIVE_FUNCTION(ReadableStreamConstructor::from)
{
    ...
    auto async_iterable = arg0;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return Streams::ReadableStream::from(vm, async_iterable); }));
