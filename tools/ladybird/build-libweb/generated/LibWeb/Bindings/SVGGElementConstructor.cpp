...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/SVG/SVGGElement.h>
...
GC_DEFINE_ALLOCATOR(SVGGElementConstructor);

SVGGElementConstructor::SVGGElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGGElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGGElementConstructor::~SVGGElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGGElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGGElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGGElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGGElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGGElement");
}


void SVGGElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGGElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGGElementPrototype>(realm, "SVGGElement"_fly_string), 0);
