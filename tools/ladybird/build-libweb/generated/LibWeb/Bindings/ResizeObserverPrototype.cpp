...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/ResizeObserver/ResizeObserver.h>
...
GC_DEFINE_ALLOCATOR(ResizeObserverPrototype);

ResizeObserverPrototype::ResizeObserverPrototype([[maybe_unused]] JS::Realm& realm)
...
void ResizeObserverPrototype::initialize(JS::Realm& realm)
{
    ...
    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "ResizeObserver"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void ResizeObserverPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<ResizeObserver::ResizeObserver*> impl_from(JS::VM& vm)
{
    ...
    if (!is<ResizeObserver::ResizeObserver>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ResizeObserver");
    return static_cast<ResizeObserver::ResizeObserver*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(ResizeObserverPrototype::observe)
{
    WebIDL::log_trace(vm, "ResizeObserverPrototype::observe");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ResizeObserverOptions");

    ResizeObserverOptions options {};

    auto box_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        box_property_value_0 = TRY(arg1.as_object().get("box"_utf16_fly_string));

    ResizeObserverBoxOptions box_value_0 { ResizeObserverBoxOptions::ContentBox };

    if (!box_property_value_0.is_undefined()) {

    auto box_property_value_0_string = TRY(box_property_value_0.to_string(vm));

    if (box_property_value_0_string == "border-box"sv)
        box_value_0 = ResizeObserverBoxOptions::BorderBox;

    else if (box_property_value_0_string == "content-box"sv)
        box_value_0 = ResizeObserverBoxOptions::ContentBox;

    else if (box_property_value_0_string == "device-pixel-content-box"sv)
        box_value_0 = ResizeObserverBoxOptions::DevicePixelContentBox;

    else
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidEnumerationValue, box_property_value_0_string, "ResizeObserverBoxOptions");

    }

    options.box = box_value_0;
...
{
    WebIDL::log_trace(vm, "ResizeObserverPrototype::unobserve");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "ResizeObserverPrototype::disconnect");
    [[maybe_unused]] auto& realm = *vm.current_realm();
