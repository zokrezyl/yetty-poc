...
GC_DEFINE_ALLOCATOR(SVGGeometryElementPrototype);

SVGGeometryElementPrototype::SVGGeometryElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void SVGGeometryElementPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_path_length_getter = JS::NativeFunction::create(realm, path_length_getter, 0, "pathLength"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_path_length_setter;

    define_direct_accessor("pathLength"_utf16_fly_string, native_path_length_getter, native_path_length_setter, default_attributes);

        define_direct_property("isPointInFill"_utf16_fly_string, JS::js_undefined(), default_attributes | JS::Attribute::Unimplemented);
            
        define_direct_property("isPointInStroke"_utf16_fly_string, JS::js_undefined(), default_attributes | JS::Attribute::Unimplemented);
            
    define_native_function(realm, "getTotalLength"_utf16_fly_string, get_total_length, 0, default_attributes);

    define_native_function(realm, "getPointAtLength"_utf16_fly_string, get_point_at_length, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "SVGGeometryElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void SVGGeometryElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<SVG::SVGGeometryElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<SVG::SVGGeometryElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "SVGGeometryElement");
    return static_cast<SVG::SVGGeometryElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(SVGGeometryElementPrototype::path_length_getter)
{
    WebIDL::log_trace(vm, "SVGGeometryElementPrototype::path_length_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->path_length(); }));

    return &const_cast<SVGAnimatedNumber&>(*retval);

}

JS_DEFINE_NATIVE_FUNCTION(SVGGeometryElementPrototype::get_total_length)
{
    WebIDL::log_trace(vm, "SVGGeometryElementPrototype::get_total_length");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_total_length(); }));

    return JS::Value(retval);

}

JS_DEFINE_NATIVE_FUNCTION(SVGGeometryElementPrototype::get_point_at_length)
{
    WebIDL::log_trace(vm, "SVGGeometryElementPrototype::get_point_at_length");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "getPointAtLength");

    auto arg0 = vm.argument(0);

    float distance = TRY(arg0.to_double(vm));

    if (isinf(distance) || isnan(distance)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "distance");
    }
    
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_point_at_length(distance); }));
