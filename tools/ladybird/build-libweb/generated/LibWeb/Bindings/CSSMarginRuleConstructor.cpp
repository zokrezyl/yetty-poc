...
GC_DEFINE_ALLOCATOR(CSSMarginRuleConstructor);

CSSMarginRuleConstructor::CSSMarginRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSMarginRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMarginRuleConstructor::~CSSMarginRuleConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMarginRuleConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMarginRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMarginRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMarginRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSMarginRule");
}


void CSSMarginRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMarginRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMarginRulePrototype>(realm, "CSSMarginRule"_fly_string), 0);
