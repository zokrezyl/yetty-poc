...
GC_DEFINE_ALLOCATOR(CSSRuleConstructor);

CSSRuleConstructor::CSSRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSRule");
}


void CSSRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSRulePrototype>(realm, "CSSRule"_fly_string), 0);
