...
GC_DEFINE_ALLOCATOR(ImageDataPrototype);

ImageDataPrototype::ImageDataPrototype([[maybe_unused]] JS::Realm& realm)
...
void ImageDataPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_color_space_getter = JS::NativeFunction::create(realm, color_space_getter, 0, "colorSpace"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_color_space_setter;

    define_direct_accessor("colorSpace"_utf16_fly_string, native_color_space_getter, native_color_space_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "ImageData"_string), JS::Attribute::Configurable);
...
JS_DEFINE_NATIVE_FUNCTION(ImageDataPrototype::color_space_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->color_space(); }));
