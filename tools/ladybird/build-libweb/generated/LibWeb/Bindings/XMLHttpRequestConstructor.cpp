...
GC_DEFINE_ALLOCATOR(XMLHttpRequestConstructor);

XMLHttpRequestConstructor::XMLHttpRequestConstructor(JS::Realm& realm)
    : NativeFunction("XMLHttpRequest"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "XMLHttpRequest");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> XMLHttpRequestConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "XMLHttpRequestConstructor::construct");
...
void XMLHttpRequestConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "XMLHttpRequest"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<XMLHttpRequestPrototype>(realm, "XMLHttpRequest"_fly_string), 0);
