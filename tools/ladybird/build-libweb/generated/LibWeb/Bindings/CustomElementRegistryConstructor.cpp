...
GC_DEFINE_ALLOCATOR(CustomElementRegistryConstructor);

CustomElementRegistryConstructor::CustomElementRegistryConstructor(JS::Realm& realm)
    : NativeFunction("CustomElementRegistry"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CustomElementRegistryConstructor::~CustomElementRegistryConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CustomElementRegistryConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CustomElementRegistry");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CustomElementRegistryConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CustomElementRegistryConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CustomElementRegistry");
}


void CustomElementRegistryConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CustomElementRegistry"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CustomElementRegistryPrototype>(realm, "CustomElementRegistry"_fly_string), 0);
