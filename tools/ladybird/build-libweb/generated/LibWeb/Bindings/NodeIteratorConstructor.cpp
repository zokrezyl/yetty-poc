...
GC_DEFINE_ALLOCATOR(NodeIteratorConstructor);

NodeIteratorConstructor::NodeIteratorConstructor(JS::Realm& realm)
    : NativeFunction("NodeIterator"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "NodeIterator");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> NodeIteratorConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "NodeIteratorConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "NodeIterator");
}


void NodeIteratorConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "NodeIterator"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<NodeIteratorPrototype>(realm, "NodeIterator"_fly_string), 0);
