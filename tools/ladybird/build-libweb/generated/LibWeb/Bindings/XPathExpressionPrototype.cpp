...
GC_DEFINE_ALLOCATOR(XPathExpressionPrototype);

XPathExpressionPrototype::XPathExpressionPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

XPathExpressionPrototype::~XPathExpressionPrototype()
{
}

void XPathExpressionPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "evaluate"_utf16_fly_string, evaluate, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "XPathExpression"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void XPathExpressionPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<XPath::XPathExpression*> impl_from(JS::VM& vm)
{
    ...
    if (!is<XPath::XPathExpression>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "XPathExpression");
    return static_cast<XPath::XPathExpression*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(XPathExpressionPrototype::evaluate)
{
    WebIDL::log_trace(vm, "XPathExpressionPrototype::evaluate");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "evaluate");
    ...
    auto& context_node = static_cast<Node&>(arg0.as_object());
    ...
    type = TRY(WebIDL::convert_to_int<WebIDL::UnsignedShort>(vm, arg1, WebIDL::EnforceRange::No, WebIDL::Clamp::No));
    ...
    if (!arg2.is_nullish()) {
        if (!arg2.is_object() || !is<XPathResult>(arg2.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "XPathResult");

        result = &static_cast<XPathResult&>(arg2.as_object());
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->evaluate(context_node, type, result); }));
