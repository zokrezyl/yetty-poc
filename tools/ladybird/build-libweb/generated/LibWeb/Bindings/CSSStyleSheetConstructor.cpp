...
GC_DEFINE_ALLOCATOR(CSSStyleSheetConstructor);

CSSStyleSheetConstructor::CSSStyleSheetConstructor(JS::Realm& realm)
    : NativeFunction("CSSStyleSheet"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSStyleSheet");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSStyleSheetConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSStyleSheetConstructor::construct");
    ...
    if (!base_url_property_value_0.is_undefined()) {
        if (!base_url_property_value_0.is_null())
            base_url_value_0 = TRY(WebIDL::to_string(vm, base_url_property_value_0));
    }

    options.base_url = base_url_value_0;

    auto disabled_property_value_1 = JS::js_undefined();
    if (arg0.is_object())
        disabled_property_value_1 = TRY(arg0.as_object().get("disabled"_utf16_fly_string));

    bool disabled_value_1;

    if (!disabled_property_value_1.is_undefined())

    disabled_value_1 = disabled_property_value_1.to_boolean();

    else
        disabled_value_1 = static_cast<bool>(false);

    options.disabled = disabled_value_1;

    auto media_property_value_2 = JS::js_undefined();
    if (arg0.is_object())
        media_property_value_2 = TRY(arg0.as_object().get("media"_utf16_fly_string));

    auto media_property_value_2_to_variant = [&vm, &realm](JS::Value media_property_value_2) -> JS::ThrowCompletionOr<Variant<GC::Root<MediaList>, String>> {
        // These might be unused.
        (void)vm;
        (void)realm;

        if (media_property_value_2.is_object()) {
            [[maybe_unused]] auto& media_property_value_2_object = media_property_value_2.as_object();

            if (is<PlatformObject>(media_property_value_2_object)) {

                if (auto* media_property_value_2_result = as_if<MediaList>(media_property_value_2_object))
                    return GC::make_root(*media_property_value_2_result);

            }

        }

    String media_property_value_2_string;
    if (!false || !media_property_value_2.is_null()) {
        media_property_value_2_string = TRY(WebIDL::to_string(vm, media_property_value_2));
    }

        return { media_property_value_2_string };

    };

    Variant<GC::Root<MediaList>, String> media_value_2 = media_property_value_2.is_undefined() ? TRY(media_property_value_2_to_variant(JS::Value(JS::PrimitiveString::create(vm, String {})))) : TRY(media_property_value_2_to_variant(media_property_value_2));

    options.media = media_value_2;
...
void CSSStyleSheetConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<StyleSheetPrototype>(realm, "StyleSheet"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSStyleSheet"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSStyleSheetPrototype>(realm, "CSSStyleSheet"_fly_string), 0);
