...
GC_DEFINE_ALLOCATOR(DocumentTypeConstructor);

DocumentTypeConstructor::DocumentTypeConstructor(JS::Realm& realm)
    : NativeFunction("DocumentType"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DocumentType");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DocumentTypeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DocumentTypeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "DocumentType");
}


void DocumentTypeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DocumentType"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DocumentTypePrototype>(realm, "DocumentType"_fly_string), 0);
