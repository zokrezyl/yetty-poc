...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/WebAudio/OfflineAudioContext.h>
...
GC_DEFINE_ALLOCATOR(OfflineAudioContextConstructor);

OfflineAudioContextConstructor::OfflineAudioContextConstructor(JS::Realm& realm)
    : NativeFunction("OfflineAudioContext"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

OfflineAudioContextConstructor::~OfflineAudioContextConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> OfflineAudioContextConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "OfflineAudioContext");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> OfflineAudioContextConstructor::construct0([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "OfflineAudioContextConstructor::construct0");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface OfflineAudioContext:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<OfflineAudioContextPrototype>(*target_realm, "OfflineAudioContext"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "OfflineAudioContext");

    auto arg0 = vm.argument(0);

    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "OfflineAudioContextOptions");

    OfflineAudioContextOptions context_options {};

    auto length_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        length_property_value_0 = TRY(arg0.as_object().get("length"_utf16_fly_string));

    if (length_property_value_0.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "length");

    WebIDL::UnsignedLong length_value_0;

    length_value_0 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, length_property_value_0, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    context_options.length = length_value_0;

    auto number_of_channels_property_value_1 = JS::js_undefined();
    if (arg0.is_object())
        number_of_channels_property_value_1 = TRY(arg0.as_object().get("numberOfChannels"_utf16_fly_string));

    WebIDL::UnsignedLong number_of_channels_value_1;

    if (!number_of_channels_property_value_1.is_undefined())

    number_of_channels_value_1 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, number_of_channels_property_value_1, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        number_of_channels_value_1 = static_cast<WebIDL::UnsignedLong>(1);

    context_options.number_of_channels = number_of_channels_value_1;

    auto sample_rate_property_value_2 = JS::js_undefined();
    if (arg0.is_object())
        sample_rate_property_value_2 = TRY(arg0.as_object().get("sampleRate"_utf16_fly_string));

    if (sample_rate_property_value_2.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "sampleRate");

    float sample_rate_value_2 = TRY(sample_rate_property_value_2.to_double(vm));

    if (isinf(sample_rate_value_2) || isnan(sample_rate_value_2)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "sampleRate");
    }
    
    context_options.sample_rate = sample_rate_value_2;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::OfflineAudioContext::construct_impl(realm, context_options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface OfflineAudioContext" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::OfflineAudioContext::construct_impl().
...
JS::ThrowCompletionOr<GC::Ref<JS::Object>> OfflineAudioContextConstructor::construct1([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "OfflineAudioContextConstructor::construct1");
    ...
    if (vm.argument_count() < 3)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "OfflineAudioContext", "3");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::OfflineAudioContext::construct_impl(realm, number_of_channels, length, sample_rate); }));
...
JS::ThrowCompletionOr<GC::Ref<JS::Object>> OfflineAudioContextConstructor::construct(JS::FunctionObject& new_target)
{
    auto& vm = this->vm();
    WebIDL::log_trace(vm, "OfflineAudioContextConstructor::construct");
...
void OfflineAudioContextConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<BaseAudioContextPrototype>(realm, "BaseAudioContext"_fly_string));

    define_direct_property(vm.names.length, JS::Value(1), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "OfflineAudioContext"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<OfflineAudioContextPrototype>(realm, "OfflineAudioContext"_fly_string), 0);
