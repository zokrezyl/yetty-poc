...
GC_DEFINE_ALLOCATOR(BroadcastChannelConstructor);

BroadcastChannelConstructor::BroadcastChannelConstructor(JS::Realm& realm)
    : NativeFunction("BroadcastChannel"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<JS::Value> BroadcastChannelConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "BroadcastChannel");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> BroadcastChannelConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "BroadcastChannelConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface BroadcastChannel:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<BroadcastChannelPrototype>(*target_realm, "BroadcastChannel"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "BroadcastChannel");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::BroadcastChannel::construct_impl(realm, name); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface BroadcastChannel" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::BroadcastChannel::construct_impl().
...
void BroadcastChannelConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "BroadcastChannel"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<BroadcastChannelPrototype>(realm, "BroadcastChannel"_fly_string), 0);
