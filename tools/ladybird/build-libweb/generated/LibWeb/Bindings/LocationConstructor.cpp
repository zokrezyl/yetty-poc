...
GC_DEFINE_ALLOCATOR(LocationConstructor);

LocationConstructor::LocationConstructor(JS::Realm& realm)
    : NativeFunction("Location"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<GC::Ref<JS::Object>> LocationConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "LocationConstructor::construct");
...
void LocationConstructor::initialize(JS::Realm& realm)
{
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Location"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<LocationPrototype>(realm, "Location"_fly_string), 0);
