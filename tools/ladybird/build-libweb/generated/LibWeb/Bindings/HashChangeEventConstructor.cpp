...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/HashChangeEvent.h>
...
GC_DEFINE_ALLOCATOR(HashChangeEventConstructor);

HashChangeEventConstructor::HashChangeEventConstructor(JS::Realm& realm)
    : NativeFunction("HashChangeEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

HashChangeEventConstructor::~HashChangeEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> HashChangeEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HashChangeEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HashChangeEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HashChangeEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface HashChangeEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<HashChangeEventPrototype>(*target_realm, "HashChangeEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "HashChangeEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HashChangeEventInit");

    HashChangeEventInit event_init_dict {};

    auto new_url_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        new_url_property_value_0 = TRY(arg1.as_object().get("newURL"_utf16_fly_string));

    String new_url_value_0;

    if (!new_url_property_value_0.is_undefined()) {
        if (!false || !new_url_property_value_0.is_null())
            new_url_value_0 = TRY(WebIDL::to_usv_string(vm, new_url_property_value_0));
    } else {
        new_url_value_0 = ""_string;
    }

    event_init_dict.new_url = new_url_value_0;

    auto old_url_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        old_url_property_value_1 = TRY(arg1.as_object().get("oldURL"_utf16_fly_string));

    String old_url_value_1;

    if (!old_url_property_value_1.is_undefined()) {
        if (!false || !old_url_property_value_1.is_null())
            old_url_value_1 = TRY(WebIDL::to_usv_string(vm, old_url_property_value_1));
    } else {
        old_url_value_1 = ""_string;
    }

    event_init_dict.old_url = old_url_value_1;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::HashChangeEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface HashChangeEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::HashChangeEvent::construct_impl().
...
void HashChangeEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HashChangeEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HashChangeEventPrototype>(realm, "HashChangeEvent"_fly_string), 0);
