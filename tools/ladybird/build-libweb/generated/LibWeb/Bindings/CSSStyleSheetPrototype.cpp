...
GC_DEFINE_ALLOCATOR(CSSStyleSheetPrototype);

CSSStyleSheetPrototype::CSSStyleSheetPrototype([[maybe_unused]] JS::Realm& realm)
...
void CSSStyleSheetPrototype::initialize(JS::Realm& realm)
{
    ...
    set_prototype(&ensure_web_prototype<StyleSheetPrototype>(realm, "StyleSheet"_fly_string));


    auto native_owner_rule_getter = JS::NativeFunction::create(realm, owner_rule_getter, 0, "ownerRule"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_owner_rule_setter;

    define_direct_accessor("ownerRule"_utf16_fly_string, native_owner_rule_getter, native_owner_rule_setter, default_attributes);

    auto native_css_rules_getter = JS::NativeFunction::create(realm, css_rules_getter, 0, "cssRules"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_css_rules_setter;

    define_direct_accessor("cssRules"_utf16_fly_string, native_css_rules_getter, native_css_rules_setter, default_attributes);
    ...
    define_native_function(realm, "insertRule"_utf16_fly_string, insert_rule, 1, default_attributes);

    define_native_function(realm, "deleteRule"_utf16_fly_string, delete_rule, 1, default_attributes);

    define_native_function(realm, "replace"_utf16_fly_string, replace, 1, default_attributes);

    define_native_function(realm, "replaceSync"_utf16_fly_string, replace_sync, 1, default_attributes);

    define_native_function(realm, "addRule"_utf16_fly_string, add_rule, 0, default_attributes);

    define_native_function(realm, "removeRule"_utf16_fly_string, remove_rule, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "CSSStyleSheet"_string), JS::Attribute::Configurable);
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::owner_rule_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->owner_rule(); }));
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::css_rules_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->css_rules(); }));

    return &const_cast<CSSRuleList&>(*retval);
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::insert_rule)
{
    ...
    if (!false || !arg0.is_null()) {
        rule = TRY(WebIDL::to_string(vm, arg0));
    }
    ...
    index = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, arg1, WebIDL::EnforceRange::No, WebIDL::Clamp::No));
        ...
        index = static_cast<WebIDL::UnsignedLong>(0);

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->insert_rule(rule, index); }));
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::delete_rule)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->delete_rule(index); }));
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::replace)
{
    ...
    String text;
    if (!false || !arg0.is_null()) {
        text = TRY(WebIDL::to_usv_string(vm, arg0));
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->replace(text); }));
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::replace_sync)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->replace_sync(text); }));
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::add_rule)
{
    ...
    if (!arg0.is_undefined()) {
        if (!false || !arg0.is_null())
            selector = TRY(WebIDL::to_string(vm, arg0));
    } else {
        selector = "undefined"_string;
    }
    ...
    if (!arg1.is_undefined()) {
        if (!false || !arg1.is_null())
            style = TRY(WebIDL::to_string(vm, arg1));
    } else {
        style = "undefined"_string;
    }
    ...
    index = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, arg2, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->add_rule(selector, style, index); }));
...
JS_DEFINE_NATIVE_FUNCTION(CSSStyleSheetPrototype::remove_rule)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->remove_rule(index); }));
