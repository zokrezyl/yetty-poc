...
GC_DEFINE_ALLOCATOR(XMLSerializerPrototype);

XMLSerializerPrototype::XMLSerializerPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

XMLSerializerPrototype::~XMLSerializerPrototype()
{
}

void XMLSerializerPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "serializeToString"_utf16_fly_string, serialize_to_string, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "XMLSerializer"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void XMLSerializerPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::XMLSerializer*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::XMLSerializer>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "XMLSerializer");
    return static_cast<HTML::XMLSerializer*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(XMLSerializerPrototype::serialize_to_string)
{
    WebIDL::log_trace(vm, "XMLSerializerPrototype::serialize_to_string");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "serializeToString");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->serialize_to_string(root); }));
