...
GC_DEFINE_ALLOCATOR(SpeechGrammarConstructor);

SpeechGrammarConstructor::SpeechGrammarConstructor(JS::Realm& realm)
    : NativeFunction("SpeechGrammar"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SpeechGrammarConstructor::~SpeechGrammarConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SpeechGrammarConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SpeechGrammar");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SpeechGrammarConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SpeechGrammarConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface SpeechGrammar:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<SpeechGrammarPrototype>(*target_realm, "SpeechGrammar"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return SpeechGrammar::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface SpeechGrammar" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within SpeechGrammar::construct_impl().
...
void SpeechGrammarConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SpeechGrammar"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SpeechGrammarPrototype>(realm, "SpeechGrammar"_fly_string), 0);
