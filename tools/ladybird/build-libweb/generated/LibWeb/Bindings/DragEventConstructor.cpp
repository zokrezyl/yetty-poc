...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/DragEvent.h>
...
GC_DEFINE_ALLOCATOR(DragEventConstructor);

DragEventConstructor::DragEventConstructor(JS::Realm& realm)
    : NativeFunction("DragEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DragEventConstructor::~DragEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DragEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DragEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DragEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DragEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DragEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DragEventPrototype>(*target_realm, "DragEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "DragEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DragEventInit");

    DragEventInit event_init_dict {};

    auto data_transfer_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        data_transfer_property_value_0 = TRY(arg1.as_object().get("dataTransfer"_utf16_fly_string));

    GC::Ptr<DataTransfer> data_transfer_value_0;

    if (!data_transfer_property_value_0.is_nullish()) {
        if (!data_transfer_property_value_0.is_object() || !is<DataTransfer>(data_transfer_property_value_0.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DataTransfer");

        data_transfer_value_0 = &static_cast<DataTransfer&>(data_transfer_property_value_0.as_object());
    }

    event_init_dict.data_transfer = data_transfer_value_0;

    auto button_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        button_property_value_1 = TRY(arg1.as_object().get("button"_utf16_fly_string));

    WebIDL::Short button_value_1;

    if (!button_property_value_1.is_undefined())

    button_value_1 = TRY(WebIDL::convert_to_int<WebIDL::Short>(vm, button_property_value_1, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        button_value_1 = static_cast<WebIDL::Short>(0);

    event_init_dict.button = button_value_1;

    auto buttons_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        buttons_property_value_2 = TRY(arg1.as_object().get("buttons"_utf16_fly_string));

    WebIDL::UnsignedShort buttons_value_2;

    if (!buttons_property_value_2.is_undefined())

    buttons_value_2 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedShort>(vm, buttons_property_value_2, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        buttons_value_2 = static_cast<WebIDL::UnsignedShort>(0);

    event_init_dict.buttons = buttons_value_2;

    auto client_x_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        client_x_property_value_3 = TRY(arg1.as_object().get("clientX"_utf16_fly_string));

    double client_x_value_3;

    if (!client_x_property_value_3.is_undefined())
        client_x_value_3 = TRY(client_x_property_value_3.to_double(vm));

    else
        client_x_value_3 = 0;

    if (isinf(client_x_value_3) || isnan(client_x_value_3)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "clientX");
    }
    
    event_init_dict.client_x = client_x_value_3;

    auto client_y_property_value_4 = JS::js_undefined();
    if (arg1.is_object())
        client_y_property_value_4 = TRY(arg1.as_object().get("clientY"_utf16_fly_string));

    double client_y_value_4;

    if (!client_y_property_value_4.is_undefined())
        client_y_value_4 = TRY(client_y_property_value_4.to_double(vm));

    else
        client_y_value_4 = 0;

    if (isinf(client_y_value_4) || isnan(client_y_value_4)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "clientY");
    }
    
    event_init_dict.client_y = client_y_value_4;

    auto movement_x_property_value_5 = JS::js_undefined();
    if (arg1.is_object())
        movement_x_property_value_5 = TRY(arg1.as_object().get("movementX"_utf16_fly_string));

    double movement_x_value_5;

    if (!movement_x_property_value_5.is_undefined())
        movement_x_value_5 = TRY(movement_x_property_value_5.to_double(vm));

    else
        movement_x_value_5 = 0;

    if (isinf(movement_x_value_5) || isnan(movement_x_value_5)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "movementX");
    }
    
    event_init_dict.movement_x = movement_x_value_5;

    auto movement_y_property_value_6 = JS::js_undefined();
    if (arg1.is_object())
        movement_y_property_value_6 = TRY(arg1.as_object().get("movementY"_utf16_fly_string));

    double movement_y_value_6;

    if (!movement_y_property_value_6.is_undefined())
        movement_y_value_6 = TRY(movement_y_property_value_6.to_double(vm));

    else
        movement_y_value_6 = 0;

    if (isinf(movement_y_value_6) || isnan(movement_y_value_6)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "movementY");
    }
    
    event_init_dict.movement_y = movement_y_value_6;

    auto related_target_property_value_7 = JS::js_undefined();
    if (arg1.is_object())
        related_target_property_value_7 = TRY(arg1.as_object().get("relatedTarget"_utf16_fly_string));

    GC::Ptr<EventTarget> related_target_value_7;

    if (!related_target_property_value_7.is_nullish()) {
        if (!related_target_property_value_7.is_object() || !is<EventTarget>(related_target_property_value_7.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "EventTarget");

        related_target_value_7 = &static_cast<EventTarget&>(related_target_property_value_7.as_object());
    }

    event_init_dict.related_target = related_target_value_7;

    auto screen_x_property_value_8 = JS::js_undefined();
    if (arg1.is_object())
        screen_x_property_value_8 = TRY(arg1.as_object().get("screenX"_utf16_fly_string));

    double screen_x_value_8;

    if (!screen_x_property_value_8.is_undefined())
        screen_x_value_8 = TRY(screen_x_property_value_8.to_double(vm));

    else
        screen_x_value_8 = 0;

    if (isinf(screen_x_value_8) || isnan(screen_x_value_8)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "screenX");
    }
    
    event_init_dict.screen_x = screen_x_value_8;

    auto screen_y_property_value_9 = JS::js_undefined();
    if (arg1.is_object())
        screen_y_property_value_9 = TRY(arg1.as_object().get("screenY"_utf16_fly_string));

    double screen_y_value_9;

    if (!screen_y_property_value_9.is_undefined())
        screen_y_value_9 = TRY(screen_y_property_value_9.to_double(vm));

    else
        screen_y_value_9 = 0;

    if (isinf(screen_y_value_9) || isnan(screen_y_value_9)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "screenY");
    }
    
    event_init_dict.screen_y = screen_y_value_9;

    auto alt_key_property_value_10 = JS::js_undefined();
    if (arg1.is_object())
        alt_key_property_value_10 = TRY(arg1.as_object().get("altKey"_utf16_fly_string));

    bool alt_key_value_10;

    if (!alt_key_property_value_10.is_undefined())

    alt_key_value_10 = alt_key_property_value_10.to_boolean();

    else
        alt_key_value_10 = static_cast<bool>(false);

    event_init_dict.alt_key = alt_key_value_10;

    auto ctrl_key_property_value_11 = JS::js_undefined();
    if (arg1.is_object())
        ctrl_key_property_value_11 = TRY(arg1.as_object().get("ctrlKey"_utf16_fly_string));

    bool ctrl_key_value_11;

    if (!ctrl_key_property_value_11.is_undefined())

    ctrl_key_value_11 = ctrl_key_property_value_11.to_boolean();

    else
        ctrl_key_value_11 = static_cast<bool>(false);

    event_init_dict.ctrl_key = ctrl_key_value_11;

    auto meta_key_property_value_12 = JS::js_undefined();
    if (arg1.is_object())
        meta_key_property_value_12 = TRY(arg1.as_object().get("metaKey"_utf16_fly_string));

    bool meta_key_value_12;

    if (!meta_key_property_value_12.is_undefined())

    meta_key_value_12 = meta_key_property_value_12.to_boolean();

    else
        meta_key_value_12 = static_cast<bool>(false);

    event_init_dict.meta_key = meta_key_value_12;

    auto modifier_alt_graph_property_value_13 = JS::js_undefined();
    if (arg1.is_object())
        modifier_alt_graph_property_value_13 = TRY(arg1.as_object().get("modifierAltGraph"_utf16_fly_string));

    bool modifier_alt_graph_value_13;

    if (!modifier_alt_graph_property_value_13.is_undefined())

    modifier_alt_graph_value_13 = modifier_alt_graph_property_value_13.to_boolean();

    else
        modifier_alt_graph_value_13 = static_cast<bool>(false);

    event_init_dict.modifier_alt_graph = modifier_alt_graph_value_13;

    auto modifier_caps_lock_property_value_14 = JS::js_undefined();
    if (arg1.is_object())
        modifier_caps_lock_property_value_14 = TRY(arg1.as_object().get("modifierCapsLock"_utf16_fly_string));

    bool modifier_caps_lock_value_14;

    if (!modifier_caps_lock_property_value_14.is_undefined())

    modifier_caps_lock_value_14 = modifier_caps_lock_property_value_14.to_boolean();

    else
        modifier_caps_lock_value_14 = static_cast<bool>(false);

    event_init_dict.modifier_caps_lock = modifier_caps_lock_value_14;

    auto modifier_fn_property_value_15 = JS::js_undefined();
    if (arg1.is_object())
        modifier_fn_property_value_15 = TRY(arg1.as_object().get("modifierFn"_utf16_fly_string));

    bool modifier_fn_value_15;

    if (!modifier_fn_property_value_15.is_undefined())

    modifier_fn_value_15 = modifier_fn_property_value_15.to_boolean();

    else
        modifier_fn_value_15 = static_cast<bool>(false);

    event_init_dict.modifier_fn = modifier_fn_value_15;

    auto modifier_fn_lock_property_value_16 = JS::js_undefined();
    if (arg1.is_object())
        modifier_fn_lock_property_value_16 = TRY(arg1.as_object().get("modifierFnLock"_utf16_fly_string));

    bool modifier_fn_lock_value_16;

    if (!modifier_fn_lock_property_value_16.is_undefined())

    modifier_fn_lock_value_16 = modifier_fn_lock_property_value_16.to_boolean();

    else
        modifier_fn_lock_value_16 = static_cast<bool>(false);

    event_init_dict.modifier_fn_lock = modifier_fn_lock_value_16;

    auto modifier_hyper_property_value_17 = JS::js_undefined();
    if (arg1.is_object())
        modifier_hyper_property_value_17 = TRY(arg1.as_object().get("modifierHyper"_utf16_fly_string));

    bool modifier_hyper_value_17;

    if (!modifier_hyper_property_value_17.is_undefined())

    modifier_hyper_value_17 = modifier_hyper_property_value_17.to_boolean();

    else
        modifier_hyper_value_17 = static_cast<bool>(false);

    event_init_dict.modifier_hyper = modifier_hyper_value_17;

    auto modifier_num_lock_property_value_18 = JS::js_undefined();
    if (arg1.is_object())
        modifier_num_lock_property_value_18 = TRY(arg1.as_object().get("modifierNumLock"_utf16_fly_string));

    bool modifier_num_lock_value_18;

    if (!modifier_num_lock_property_value_18.is_undefined())

    modifier_num_lock_value_18 = modifier_num_lock_property_value_18.to_boolean();

    else
        modifier_num_lock_value_18 = static_cast<bool>(false);

    event_init_dict.modifier_num_lock = modifier_num_lock_value_18;

    auto modifier_scroll_lock_property_value_19 = JS::js_undefined();
    if (arg1.is_object())
        modifier_scroll_lock_property_value_19 = TRY(arg1.as_object().get("modifierScrollLock"_utf16_fly_string));

    bool modifier_scroll_lock_value_19;

    if (!modifier_scroll_lock_property_value_19.is_undefined())

    modifier_scroll_lock_value_19 = modifier_scroll_lock_property_value_19.to_boolean();

    else
        modifier_scroll_lock_value_19 = static_cast<bool>(false);

    event_init_dict.modifier_scroll_lock = modifier_scroll_lock_value_19;

    auto modifier_super_property_value_20 = JS::js_undefined();
    if (arg1.is_object())
        modifier_super_property_value_20 = TRY(arg1.as_object().get("modifierSuper"_utf16_fly_string));

    bool modifier_super_value_20;

    if (!modifier_super_property_value_20.is_undefined())

    modifier_super_value_20 = modifier_super_property_value_20.to_boolean();

    else
        modifier_super_value_20 = static_cast<bool>(false);

    event_init_dict.modifier_super = modifier_super_value_20;

    auto modifier_symbol_property_value_21 = JS::js_undefined();
    if (arg1.is_object())
        modifier_symbol_property_value_21 = TRY(arg1.as_object().get("modifierSymbol"_utf16_fly_string));

    bool modifier_symbol_value_21;

    if (!modifier_symbol_property_value_21.is_undefined())

    modifier_symbol_value_21 = modifier_symbol_property_value_21.to_boolean();

    else
        modifier_symbol_value_21 = static_cast<bool>(false);

    event_init_dict.modifier_symbol = modifier_symbol_value_21;

    auto modifier_symbol_lock_property_value_22 = JS::js_undefined();
    if (arg1.is_object())
        modifier_symbol_lock_property_value_22 = TRY(arg1.as_object().get("modifierSymbolLock"_utf16_fly_string));

    bool modifier_symbol_lock_value_22;

    if (!modifier_symbol_lock_property_value_22.is_undefined())

    modifier_symbol_lock_value_22 = modifier_symbol_lock_property_value_22.to_boolean();

    else
        modifier_symbol_lock_value_22 = static_cast<bool>(false);

    event_init_dict.modifier_symbol_lock = modifier_symbol_lock_value_22;

    auto shift_key_property_value_23 = JS::js_undefined();
    if (arg1.is_object())
        shift_key_property_value_23 = TRY(arg1.as_object().get("shiftKey"_utf16_fly_string));

    bool shift_key_value_23;

    if (!shift_key_property_value_23.is_undefined())

    shift_key_value_23 = shift_key_property_value_23.to_boolean();

    else
        shift_key_value_23 = static_cast<bool>(false);

    event_init_dict.shift_key = shift_key_value_23;

    auto detail_property_value_24 = JS::js_undefined();
    if (arg1.is_object())
        detail_property_value_24 = TRY(arg1.as_object().get("detail"_utf16_fly_string));

    WebIDL::Long detail_value_24;

    if (!detail_property_value_24.is_undefined())

    detail_value_24 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, detail_property_value_24, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        detail_value_24 = static_cast<WebIDL::Long>(0);

    event_init_dict.detail = detail_value_24;

    auto view_property_value_25 = JS::js_undefined();
    if (arg1.is_object())
        view_property_value_25 = TRY(arg1.as_object().get("view"_utf16_fly_string));

    GC::Ptr<WindowProxy> view_value_25;

    if (!view_property_value_25.is_nullish()) {
        if (!view_property_value_25.is_object() || !is<WindowProxy>(view_property_value_25.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WindowProxy");

        view_value_25 = &static_cast<WindowProxy&>(view_property_value_25.as_object());
    }

    event_init_dict.view = view_value_25;

    auto bubbles_property_value_26 = JS::js_undefined();
    if (arg1.is_object())
        bubbles_property_value_26 = TRY(arg1.as_object().get("bubbles"_utf16_fly_string));

    bool bubbles_value_26;

    if (!bubbles_property_value_26.is_undefined())

    bubbles_value_26 = bubbles_property_value_26.to_boolean();

    else
        bubbles_value_26 = static_cast<bool>(false);

    event_init_dict.bubbles = bubbles_value_26;

    auto cancelable_property_value_27 = JS::js_undefined();
    if (arg1.is_object())
        cancelable_property_value_27 = TRY(arg1.as_object().get("cancelable"_utf16_fly_string));

    bool cancelable_value_27;

    if (!cancelable_property_value_27.is_undefined())

    cancelable_value_27 = cancelable_property_value_27.to_boolean();

    else
        cancelable_value_27 = static_cast<bool>(false);

    event_init_dict.cancelable = cancelable_value_27;

    auto composed_property_value_28 = JS::js_undefined();
    if (arg1.is_object())
        composed_property_value_28 = TRY(arg1.as_object().get("composed"_utf16_fly_string));

    bool composed_value_28;

    if (!composed_property_value_28.is_undefined())

    composed_value_28 = composed_property_value_28.to_boolean();

    else
        composed_value_28 = static_cast<bool>(false);

    event_init_dict.composed = composed_value_28;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::DragEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DragEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::DragEvent::construct_impl().
...
void DragEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DragEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DragEventPrototype>(realm, "DragEvent"_fly_string), 0);
