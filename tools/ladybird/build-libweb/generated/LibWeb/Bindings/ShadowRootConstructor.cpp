...
GC_DEFINE_ALLOCATOR(ShadowRootConstructor);

ShadowRootConstructor::ShadowRootConstructor(JS::Realm& realm)
    : NativeFunction("ShadowRoot"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ShadowRoot");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ShadowRootConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ShadowRootConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "ShadowRoot");
}


void ShadowRootConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<DocumentFragmentPrototype>(realm, "DocumentFragment"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ShadowRoot"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ShadowRootPrototype>(realm, "ShadowRoot"_fly_string), 0);
