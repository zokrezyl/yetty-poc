...
GC_DEFINE_ALLOCATOR(HTMLSlotElementPrototype);

HTMLSlotElementPrototype::HTMLSlotElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void HTMLSlotElementPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "assignedNodes"_utf16_fly_string, assigned_nodes, 0, default_attributes);

    define_native_function(realm, "assignedElements"_utf16_fly_string, assigned_elements, 0, default_attributes);

    define_native_function(realm, "assign"_utf16_fly_string, assign, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "HTMLSlotElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void HTMLSlotElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::HTMLSlotElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::HTMLSlotElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLSlotElement");
    return static_cast<HTML::HTMLSlotElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(HTMLSlotElementPrototype::name_getter)
{
    WebIDL::log_trace(vm, "HTMLSlotElementPrototype::name_getter");
...
{
    WebIDL::log_trace(vm, "HTMLSlotElementPrototype::name_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSlotElementPrototype::assigned_nodes)
{
    WebIDL::log_trace(vm, "HTMLSlotElementPrototype::assigned_nodes");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "AssignedNodesOptions");

    AssignedNodesOptions options {};

    auto flatten_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        flatten_property_value_0 = TRY(arg0.as_object().get("flatten"_utf16_fly_string));

    bool flatten_value_0;

    if (!flatten_property_value_0.is_undefined())

    flatten_value_0 = flatten_property_value_0.to_boolean();

    else
        flatten_value_0 = static_cast<bool>(false);

    options.flatten = flatten_value_0;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->assigned_nodes(options); }));
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSlotElementPrototype::assigned_elements)
{
    WebIDL::log_trace(vm, "HTMLSlotElementPrototype::assigned_elements");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    auto flatten_property_value_1 = JS::js_undefined();
    if (arg0.is_object())
        flatten_property_value_1 = TRY(arg0.as_object().get("flatten"_utf16_fly_string));

    bool flatten_value_1;

    if (!flatten_property_value_1.is_undefined())

    flatten_value_1 = flatten_property_value_1.to_boolean();

    else
        flatten_value_1 = static_cast<bool>(false);

    options.flatten = flatten_value_1;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->assigned_elements(options); }));
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSlotElementPrototype::assign)
{
    WebIDL::log_trace(vm, "HTMLSlotElementPrototype::assign");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    auto arg0_to_variant = [&vm, &realm](JS::Value arg0) -> JS::ThrowCompletionOr<Variant<GC::Root<Element>, GC::Root<Text>>> {
        ...
        if (arg0.is_object()) {
            ...
            if (is<PlatformObject>(arg0_object)) {
                ...
                if (auto* arg0_result = as_if<Element>(arg0_object))
                    return GC::make_root(*arg0_result);

                if (auto* arg0_result = as_if<Text>(arg0_object))
                    return GC::make_root(*arg0_result);
        ...
        Vector<Variant<GC::Root<Element>, GC::Root<Text>>> nodes;
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->assign(move(nodes)); }));
