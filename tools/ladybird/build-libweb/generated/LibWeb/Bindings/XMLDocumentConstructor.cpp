...
GC_DEFINE_ALLOCATOR(XMLDocumentConstructor);

XMLDocumentConstructor::XMLDocumentConstructor(JS::Realm& realm)
    : NativeFunction("XMLDocument"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

XMLDocumentConstructor::~XMLDocumentConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> XMLDocumentConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "XMLDocument");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> XMLDocumentConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "XMLDocumentConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "XMLDocument");
}


void XMLDocumentConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "XMLDocument"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<XMLDocumentPrototype>(realm, "XMLDocument"_fly_string), 0);
