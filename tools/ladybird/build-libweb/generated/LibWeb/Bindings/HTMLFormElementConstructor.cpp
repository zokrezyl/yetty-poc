...
GC_DEFINE_ALLOCATOR(HTMLFormElementConstructor);

HTMLFormElementConstructor::HTMLFormElementConstructor(JS::Realm& realm)
    : NativeFunction("HTMLFormElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLFormElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLFormElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLFormElementConstructor::construct");
    ...
    // 6. Otherwise (i.e., if definition is for a customized built-in element):
        ...
        static auto valid_local_names = MUST(DOM::valid_local_names_for_given_html_element_interface("HTMLFormElement"sv));
        ...
        if (!valid_local_names.contains_slow(definition->local_name()))
            return vm.throw_completion<JS::TypeError>(MUST(String::formatted("Local name '{}' of customized built-in element is not a valid local name for HTMLFormElement", definition->local_name())));
    ...
    if (definition->construction_stack().is_empty()) {
        ...
        auto element = realm.create<HTML::HTMLFormElement>(window.associated_document(), DOM::QualifiedName { definition->local_name(), {}, Namespace::HTML });
        ...
        // Important steps from "internally create a new object implementing the interface"
        {
            ...
            if (!prototype.is_object()) {
                ...
                VERIFY(target_realm);
                prototype = &Bindings::ensure_web_prototype<HTMLFormElementPrototype>(*target_realm, "HTMLFormElement"_fly_string);
            }
    ...
    if (!prototype.is_object()) {
        ...
        prototype = &Bindings::ensure_web_prototype<HTMLFormElementPrototype>(*function_realm, "HTMLFormElement"_fly_string);
    }
...
void HTMLFormElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLFormElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLFormElementPrototype>(realm, "HTMLFormElement"_fly_string), 0);
