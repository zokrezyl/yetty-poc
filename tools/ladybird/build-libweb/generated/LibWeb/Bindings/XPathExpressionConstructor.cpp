...
GC_DEFINE_ALLOCATOR(XPathExpressionConstructor);

XPathExpressionConstructor::XPathExpressionConstructor(JS::Realm& realm)
    : NativeFunction("XPathExpression"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

XPathExpressionConstructor::~XPathExpressionConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> XPathExpressionConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "XPathExpression");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> XPathExpressionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "XPathExpressionConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "XPathExpression");
}


void XPathExpressionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "XPathExpression"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<XPathExpressionPrototype>(realm, "XPathExpression"_fly_string), 0);
