...
GC_DEFINE_ALLOCATOR(ProcessingInstructionConstructor);

ProcessingInstructionConstructor::ProcessingInstructionConstructor(JS::Realm& realm)
    : NativeFunction("ProcessingInstruction"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ProcessingInstruction");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ProcessingInstructionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ProcessingInstructionConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "ProcessingInstruction");
}


void ProcessingInstructionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ProcessingInstruction"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ProcessingInstructionPrototype>(realm, "ProcessingInstruction"_fly_string), 0);
