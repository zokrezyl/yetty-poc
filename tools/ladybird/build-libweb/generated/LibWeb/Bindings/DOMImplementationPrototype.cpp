...
GC_DEFINE_ALLOCATOR(DOMImplementationPrototype);

DOMImplementationPrototype::DOMImplementationPrototype([[maybe_unused]] JS::Realm& realm)
...
void DOMImplementationPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "createDocumentType"_utf16_fly_string, create_document_type, 3, default_attributes);

    define_native_function(realm, "createDocument"_utf16_fly_string, create_document, 2, default_attributes);

    define_native_function(realm, "createHTMLDocument"_utf16_fly_string, create_html_document, 0, default_attributes);

    define_native_function(realm, "hasFeature"_utf16_fly_string, has_feature, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "DOMImplementation"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<DOM::DOMImplementation*> impl_from(JS::VM& vm)
{
    ...
    if (!is<DOM::DOMImplementation>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DOMImplementation");
    return static_cast<DOM::DOMImplementation*>(this_object);
}
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::create_document_type)
{
    ...
    if (!false || !arg1.is_null()) {
        public_id = TRY(WebIDL::to_string(vm, arg1));
    }
    ...
    if (!false || !arg2.is_null()) {
        system_id = TRY(WebIDL::to_string(vm, arg2));
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->create_document_type(qualified_name, public_id, system_id); }));
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::create_document)
{
    ...
    GC::Ptr<DocumentType> doctype;

    if (!arg2.is_nullish()) {
        if (!arg2.is_object() || !is<DocumentType>(arg2.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DocumentType");

        doctype = &static_cast<DocumentType&>(arg2.as_object());
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->create_document(namespace_, qualified_name, doctype); }));

    return &const_cast<XMLDocument&>(*retval);
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::create_html_document)
{
    ...
    if (!arg0.is_undefined()) {
        if (!false || !arg0.is_null())
            title = TRY(WebIDL::to_utf16_string(vm, arg0));
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->create_html_document(title); }));
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::has_feature)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->has_feature(); }));
