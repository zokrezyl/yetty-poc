...
GC_DEFINE_ALLOCATOR(TextEventConstructor);

TextEventConstructor::TextEventConstructor(JS::Realm& realm)
    : NativeFunction("TextEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

TextEventConstructor::~TextEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> TextEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "TextEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> TextEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "TextEventConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "TextEvent");
}


void TextEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "TextEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<TextEventPrototype>(realm, "TextEvent"_fly_string), 0);
