...
GC_DEFINE_ALLOCATOR(BaseAudioContextConstructor);

BaseAudioContextConstructor::BaseAudioContextConstructor(JS::Realm& realm)
    : NativeFunction("BaseAudioContext"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

BaseAudioContextConstructor::~BaseAudioContextConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> BaseAudioContextConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "BaseAudioContext");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> BaseAudioContextConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "BaseAudioContextConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "BaseAudioContext");
}


void BaseAudioContextConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "BaseAudioContext"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<BaseAudioContextPrototype>(realm, "BaseAudioContext"_fly_string), 0);
