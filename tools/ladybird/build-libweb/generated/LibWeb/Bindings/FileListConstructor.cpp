...
GC_DEFINE_ALLOCATOR(FileListConstructor);

FileListConstructor::FileListConstructor(JS::Realm& realm)
    : NativeFunction("FileList"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

FileListConstructor::~FileListConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> FileListConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "FileList");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> FileListConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "FileListConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "FileList");
}


void FileListConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "FileList"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<FileListPrototype>(realm, "FileList"_fly_string), 0);
