...
GC_DEFINE_ALLOCATOR(CompressionStreamConstructor);

CompressionStreamConstructor::CompressionStreamConstructor(JS::Realm& realm)
    : NativeFunction("CompressionStream"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CompressionStreamConstructor::~CompressionStreamConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CompressionStreamConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CompressionStream");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CompressionStreamConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CompressionStreamConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CompressionStream:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CompressionStreamPrototype>(*target_realm, "CompressionStream"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CompressionStream");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Compression::CompressionStream::construct_impl(realm, format); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CompressionStream" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Compression::CompressionStream::construct_impl().
...
void CompressionStreamConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CompressionStream"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CompressionStreamPrototype>(realm, "CompressionStream"_fly_string), 0);
