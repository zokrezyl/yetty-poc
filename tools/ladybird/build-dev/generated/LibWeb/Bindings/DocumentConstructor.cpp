...
GC_DEFINE_ALLOCATOR(DocumentConstructor);

DocumentConstructor::DocumentConstructor(JS::Realm& realm)
    : NativeFunction("Document"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Document");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DocumentConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DocumentConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface Document:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DocumentPrototype>(*target_realm, "Document"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::Document::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface Document" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOM::Document::construct_impl().
...
void DocumentConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Document"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DocumentPrototype>(realm, "Document"_fly_string), 0);


    define_native_function(realm, "parseHTMLUnsafe"_utf16_fly_string, parse_html_unsafe, 1, JS::Attribute::Writable | JS::Attribute::Enumerable | JS::Attribute::Configurable);

}

JS_DEFINE_NATIVE_FUNCTION(DocumentConstructor::parse_html_unsafe)
{
    WebIDL::log_trace(vm, "DocumentConstructor::parse_html_unsafe");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "parseHTMLUnsafe");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::Document::parse_html_unsafe(vm, html); }));
