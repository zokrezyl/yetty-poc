...
GC_DEFINE_ALLOCATOR(OfflineAudioContextPrototype);

OfflineAudioContextPrototype::OfflineAudioContextPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

OfflineAudioContextPrototype::~OfflineAudioContextPrototype()
{
}

void OfflineAudioContextPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "startRendering"_utf16_fly_string, start_rendering, 0, default_attributes);

    define_native_function(realm, "resume"_utf16_fly_string, resume, 0, default_attributes);

    define_native_function(realm, "suspend"_utf16_fly_string, suspend, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "OfflineAudioContext"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void OfflineAudioContextPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<WebAudio::OfflineAudioContext*> impl_from(JS::VM& vm)
{
    ...
    if (!is<WebAudio::OfflineAudioContext>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "OfflineAudioContext");
    return static_cast<WebAudio::OfflineAudioContext*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(OfflineAudioContextPrototype::length_getter)
{
    WebIDL::log_trace(vm, "OfflineAudioContextPrototype::length_getter");
...
JS_DEFINE_NATIVE_FUNCTION(OfflineAudioContextPrototype::oncomplete_getter)
{
    WebIDL::log_trace(vm, "OfflineAudioContextPrototype::oncomplete_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(OfflineAudioContextPrototype::oncomplete_setter)
{
    WebIDL::log_trace(vm, "OfflineAudioContextPrototype::oncomplete_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(OfflineAudioContextPrototype::start_rendering)
{
    WebIDL::log_trace(vm, "OfflineAudioContextPrototype::start_rendering");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->start_rendering(); }));
...
JS_DEFINE_NATIVE_FUNCTION(OfflineAudioContextPrototype::resume)
{
    WebIDL::log_trace(vm, "OfflineAudioContextPrototype::resume");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(OfflineAudioContextPrototype::suspend)
{
    WebIDL::log_trace(vm, "OfflineAudioContextPrototype::suspend");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "suspend");

    auto arg0 = vm.argument(0);

    double suspend_time = TRY(arg0.to_double(vm));

    if (isinf(suspend_time) || isnan(suspend_time)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "suspendTime");
    }
    
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->suspend(suspend_time); }));
