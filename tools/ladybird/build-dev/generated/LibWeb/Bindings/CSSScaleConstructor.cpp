...
GC_DEFINE_ALLOCATOR(CSSScaleConstructor);

CSSScaleConstructor::CSSScaleConstructor(JS::Realm& realm)
    : NativeFunction("CSSScale"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSScaleConstructor::~CSSScaleConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSScaleConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSScale");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSScaleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSScaleConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSScale:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSScalePrototype>(*target_realm, "CSSScale"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "CSSScale", "2");
    ...
    Optional<Variant<double, GC::Root<CSSNumericValue>>> z;
    if (!arg2.is_undefined())
        z = TRY(arg2_to_variant(arg2));

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSScale::construct_impl(realm, x, y, z); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSScale" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSScale::construct_impl().
...
void CSSScaleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSScale"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSScalePrototype>(realm, "CSSScale"_fly_string), 0);
