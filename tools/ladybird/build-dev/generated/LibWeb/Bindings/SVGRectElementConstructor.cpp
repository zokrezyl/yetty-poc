...
GC_DEFINE_ALLOCATOR(SVGRectElementConstructor);

SVGRectElementConstructor::SVGRectElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGRectElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGRectElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGRectElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGRectElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGRectElement");
}


void SVGRectElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGRectElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGRectElementPrototype>(realm, "SVGRectElement"_fly_string), 0);
