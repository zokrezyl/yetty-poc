...
GC_DEFINE_ALLOCATOR(MutationObserverConstructor);

MutationObserverConstructor::MutationObserverConstructor(JS::Realm& realm)
    : NativeFunction("MutationObserver"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

MutationObserverConstructor::~MutationObserverConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> MutationObserverConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MutationObserver");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MutationObserverConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MutationObserverConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface MutationObserver:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<MutationObserverPrototype>(*target_realm, "MutationObserver"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "MutationObserver");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return DOM::MutationObserver::construct_impl(realm, callback); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface MutationObserver" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within DOM::MutationObserver::construct_impl().
...
void MutationObserverConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MutationObserver"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MutationObserverPrototype>(realm, "MutationObserver"_fly_string), 0);
