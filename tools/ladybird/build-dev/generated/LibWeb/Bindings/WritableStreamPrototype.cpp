...
GC_DEFINE_ALLOCATOR(WritableStreamPrototype);

WritableStreamPrototype::WritableStreamPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

WritableStreamPrototype::~WritableStreamPrototype()
{
}

void WritableStreamPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "getWriter"_utf16_fly_string, get_writer, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "WritableStream"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<Streams::WritableStream*> impl_from(JS::VM& vm)
{
    ...
    if (!is<Streams::WritableStream>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WritableStream");
    return static_cast<Streams::WritableStream*>(this_object);
}
...
JS_DEFINE_NATIVE_FUNCTION(WritableStreamPrototype::get_writer)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_writer(); }));

    return &const_cast<WritableStreamDefaultWriter&>(*retval);
