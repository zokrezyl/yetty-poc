...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/XMLSerializer.h>
...
GC_DEFINE_ALLOCATOR(XMLSerializerConstructor);

XMLSerializerConstructor::XMLSerializerConstructor(JS::Realm& realm)
    : NativeFunction("XMLSerializer"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

XMLSerializerConstructor::~XMLSerializerConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> XMLSerializerConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "XMLSerializer");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> XMLSerializerConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "XMLSerializerConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface XMLSerializer:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<XMLSerializerPrototype>(*target_realm, "XMLSerializer"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::XMLSerializer::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface XMLSerializer" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::XMLSerializer::construct_impl().
...
void XMLSerializerConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "XMLSerializer"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<XMLSerializerPrototype>(realm, "XMLSerializer"_fly_string), 0);
