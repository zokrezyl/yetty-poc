...
GC_DEFINE_ALLOCATOR(SpeechRecognitionConstructor);

SpeechRecognitionConstructor::SpeechRecognitionConstructor(JS::Realm& realm)
    : NativeFunction("SpeechRecognition"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<JS::Value> SpeechRecognitionConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SpeechRecognition");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SpeechRecognitionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SpeechRecognitionConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface SpeechRecognition:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<SpeechRecognitionPrototype>(*target_realm, "SpeechRecognition"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return SpeechRecognition::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface SpeechRecognition" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within SpeechRecognition::construct_impl().
...
void SpeechRecognitionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SpeechRecognition"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SpeechRecognitionPrototype>(realm, "SpeechRecognition"_fly_string), 0);
