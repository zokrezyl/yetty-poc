...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSSupportsRule.h>
...
GC_DEFINE_ALLOCATOR(CSSSupportsRuleConstructor);

CSSSupportsRuleConstructor::CSSSupportsRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSSupportsRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSSupportsRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSSupportsRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSSupportsRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSSupportsRule");
}


void CSSSupportsRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<CSSConditionRulePrototype>(realm, "CSSConditionRule"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSSupportsRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSSupportsRulePrototype>(realm, "CSSSupportsRule"_fly_string), 0);
