...
GC_DEFINE_ALLOCATOR(SVGSVGElementConstructor);

SVGSVGElementConstructor::SVGSVGElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGSVGElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGSVGElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGSVGElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGSVGElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGSVGElement");
}


void SVGSVGElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGSVGElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGSVGElementPrototype>(realm, "SVGSVGElement"_fly_string), 0);
