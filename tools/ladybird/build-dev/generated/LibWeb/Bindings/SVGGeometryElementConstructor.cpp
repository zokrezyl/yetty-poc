...
GC_DEFINE_ALLOCATOR(SVGGeometryElementConstructor);

SVGGeometryElementConstructor::SVGGeometryElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGGeometryElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGGeometryElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGGeometryElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGGeometryElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGGeometryElement");
}


void SVGGeometryElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGGeometryElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGGeometryElementPrototype>(realm, "SVGGeometryElement"_fly_string), 0);
