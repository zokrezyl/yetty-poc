...
GC_DEFINE_ALLOCATOR(ScriptProcessorNodeConstructor);

ScriptProcessorNodeConstructor::ScriptProcessorNodeConstructor(JS::Realm& realm)
    : NativeFunction("ScriptProcessorNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ScriptProcessorNodeConstructor::~ScriptProcessorNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ScriptProcessorNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ScriptProcessorNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ScriptProcessorNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ScriptProcessorNodeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "ScriptProcessorNode");
}


void ScriptProcessorNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ScriptProcessorNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ScriptProcessorNodePrototype>(realm, "ScriptProcessorNode"_fly_string), 0);
