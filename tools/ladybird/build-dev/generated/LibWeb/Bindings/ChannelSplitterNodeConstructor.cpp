...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/WebAudio/ChannelSplitterNode.h>
...
GC_DEFINE_ALLOCATOR(ChannelSplitterNodeConstructor);

ChannelSplitterNodeConstructor::ChannelSplitterNodeConstructor(JS::Realm& realm)
    : NativeFunction("ChannelSplitterNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ChannelSplitterNodeConstructor::~ChannelSplitterNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ChannelSplitterNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ChannelSplitterNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ChannelSplitterNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ChannelSplitterNodeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface ChannelSplitterNode:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<ChannelSplitterNodePrototype>(*target_realm, "ChannelSplitterNode"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "ChannelSplitterNode");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ChannelSplitterOptions");

    ChannelSplitterOptions options {};

    auto number_of_outputs_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        number_of_outputs_property_value_0 = TRY(arg1.as_object().get("numberOfOutputs"_utf16_fly_string));

    WebIDL::UnsignedLong number_of_outputs_value_0;

    if (!number_of_outputs_property_value_0.is_undefined())

    number_of_outputs_value_0 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, number_of_outputs_property_value_0, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        number_of_outputs_value_0 = static_cast<WebIDL::UnsignedLong>(6);

    options.number_of_outputs = number_of_outputs_value_0;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::ChannelSplitterNode::construct_impl(realm, context, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface ChannelSplitterNode" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::ChannelSplitterNode::construct_impl().
...
void ChannelSplitterNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ChannelSplitterNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ChannelSplitterNodePrototype>(realm, "ChannelSplitterNode"_fly_string), 0);
