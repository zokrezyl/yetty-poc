...
GC_DEFINE_ALLOCATOR(SVGClipPathElementConstructor);

SVGClipPathElementConstructor::SVGClipPathElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGClipPathElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGClipPathElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGClipPathElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGClipPathElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGClipPathElement");
}


void SVGClipPathElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGClipPathElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGClipPathElementPrototype>(realm, "SVGClipPathElement"_fly_string), 0);
