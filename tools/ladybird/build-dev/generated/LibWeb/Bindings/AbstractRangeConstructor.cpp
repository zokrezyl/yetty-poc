...
GC_DEFINE_ALLOCATOR(AbstractRangeConstructor);

AbstractRangeConstructor::AbstractRangeConstructor(JS::Realm& realm)
    : NativeFunction("AbstractRange"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "AbstractRange");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> AbstractRangeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "AbstractRangeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "AbstractRange");
}


void AbstractRangeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "AbstractRange"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<AbstractRangePrototype>(realm, "AbstractRange"_fly_string), 0);
