...
GC_DEFINE_ALLOCATOR(AudioScheduledSourceNodeConstructor);

AudioScheduledSourceNodeConstructor::AudioScheduledSourceNodeConstructor(JS::Realm& realm)
    : NativeFunction("AudioScheduledSourceNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

AudioScheduledSourceNodeConstructor::~AudioScheduledSourceNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> AudioScheduledSourceNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "AudioScheduledSourceNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> AudioScheduledSourceNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "AudioScheduledSourceNodeConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "AudioScheduledSourceNode");
}


void AudioScheduledSourceNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "AudioScheduledSourceNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<AudioScheduledSourceNodePrototype>(realm, "AudioScheduledSourceNode"_fly_string), 0);
