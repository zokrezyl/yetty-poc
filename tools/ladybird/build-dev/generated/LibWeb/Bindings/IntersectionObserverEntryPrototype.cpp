...
GC_DEFINE_ALLOCATOR(IntersectionObserverEntryPrototype);

IntersectionObserverEntryPrototype::IntersectionObserverEntryPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

IntersectionObserverEntryPrototype::~IntersectionObserverEntryPrototype()
{
}

void IntersectionObserverEntryPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_time_getter = JS::NativeFunction::create(realm, time_getter, 0, "time"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_time_setter;

    define_direct_accessor("time"_utf16_fly_string, native_time_getter, native_time_setter, default_attributes);

    auto native_root_bounds_getter = JS::NativeFunction::create(realm, root_bounds_getter, 0, "rootBounds"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_root_bounds_setter;

    define_direct_accessor("rootBounds"_utf16_fly_string, native_root_bounds_getter, native_root_bounds_setter, default_attributes);

    auto native_bounding_client_rect_getter = JS::NativeFunction::create(realm, bounding_client_rect_getter, 0, "boundingClientRect"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_bounding_client_rect_setter;

    define_direct_accessor("boundingClientRect"_utf16_fly_string, native_bounding_client_rect_getter, native_bounding_client_rect_setter, default_attributes);

    auto native_intersection_rect_getter = JS::NativeFunction::create(realm, intersection_rect_getter, 0, "intersectionRect"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_intersection_rect_setter;

    define_direct_accessor("intersectionRect"_utf16_fly_string, native_intersection_rect_getter, native_intersection_rect_setter, default_attributes);

    auto native_is_intersecting_getter = JS::NativeFunction::create(realm, is_intersecting_getter, 0, "isIntersecting"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_is_intersecting_setter;

    define_direct_accessor("isIntersecting"_utf16_fly_string, native_is_intersecting_getter, native_is_intersecting_setter, default_attributes);

    auto native_intersection_ratio_getter = JS::NativeFunction::create(realm, intersection_ratio_getter, 0, "intersectionRatio"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_intersection_ratio_setter;

    define_direct_accessor("intersectionRatio"_utf16_fly_string, native_intersection_ratio_getter, native_intersection_ratio_setter, default_attributes);
    ...
    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "IntersectionObserverEntry"_string), JS::Attribute::Configurable);
...
[[maybe_unused]] static JS::ThrowCompletionOr<IntersectionObserver::IntersectionObserverEntry*> impl_from(JS::VM& vm)
{
    ...
    if (!is<IntersectionObserver::IntersectionObserverEntry>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "IntersectionObserverEntry");
    return static_cast<IntersectionObserver::IntersectionObserverEntry*>(this_object);
}
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverEntryPrototype::time_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->time(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverEntryPrototype::root_bounds_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->root_bounds(); }));

    if (retval) {

    return &const_cast<DOMRectReadOnly&>(*retval);
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverEntryPrototype::bounding_client_rect_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->bounding_client_rect(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverEntryPrototype::intersection_rect_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->intersection_rect(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverEntryPrototype::is_intersecting_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->is_intersecting(); }));
...
JS_DEFINE_NATIVE_FUNCTION(IntersectionObserverEntryPrototype::intersection_ratio_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->intersection_ratio(); }));
