...
GC_DEFINE_ALLOCATOR(MathMLElementConstructor);

MathMLElementConstructor::MathMLElementConstructor(JS::Realm& realm)
    : NativeFunction("MathMLElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

MathMLElementConstructor::~MathMLElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> MathMLElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MathMLElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MathMLElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MathMLElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "MathMLElement");
}


void MathMLElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MathMLElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MathMLElementPrototype>(realm, "MathMLElement"_fly_string), 0);
