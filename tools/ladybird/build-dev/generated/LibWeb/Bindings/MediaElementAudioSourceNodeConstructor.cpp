...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/WebAudio/MediaElementAudioSourceNode.h>
...
GC_DEFINE_ALLOCATOR(MediaElementAudioSourceNodeConstructor);

MediaElementAudioSourceNodeConstructor::MediaElementAudioSourceNodeConstructor(JS::Realm& realm)
    : NativeFunction("MediaElementAudioSourceNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

MediaElementAudioSourceNodeConstructor::~MediaElementAudioSourceNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> MediaElementAudioSourceNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MediaElementAudioSourceNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MediaElementAudioSourceNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MediaElementAudioSourceNodeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface MediaElementAudioSourceNode:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<MediaElementAudioSourceNodePrototype>(*target_realm, "MediaElementAudioSourceNode"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "MediaElementAudioSourceNode", "2");

    auto arg0 = vm.argument(0);

    if (!arg0.is_object() || !is<AudioContext>(arg0.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "AudioContext");

    auto& context = static_cast<AudioContext&>(arg0.as_object());

    auto arg1 = vm.argument(1);

    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "MediaElementAudioSourceOptions");

    MediaElementAudioSourceOptions options {};

    auto media_element_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        media_element_property_value_0 = TRY(arg1.as_object().get("mediaElement"_utf16_fly_string));

    if (media_element_property_value_0.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "mediaElement");

    if (!media_element_property_value_0.is_object() || !is<HTMLMediaElement>(media_element_property_value_0.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLMediaElement");

    auto& media_element_value_0 = static_cast<HTMLMediaElement&>(media_element_property_value_0.as_object());

    options.media_element = media_element_value_0;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::MediaElementAudioSourceNode::construct_impl(realm, context, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface MediaElementAudioSourceNode" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::MediaElementAudioSourceNode::construct_impl().
...
void MediaElementAudioSourceNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MediaElementAudioSourceNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MediaElementAudioSourceNodePrototype>(realm, "MediaElementAudioSourceNode"_fly_string), 0);
