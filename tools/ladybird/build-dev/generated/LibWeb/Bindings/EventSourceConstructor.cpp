...
GC_DEFINE_ALLOCATOR(EventSourceConstructor);

EventSourceConstructor::EventSourceConstructor(JS::Realm& realm)
    : NativeFunction("EventSource"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<JS::Value> EventSourceConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "EventSource");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> EventSourceConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "EventSourceConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface EventSource:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<EventSourcePrototype>(*target_realm, "EventSource"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "EventSource");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "EventSourceInit");

    EventSourceInit event_source_init_dict {};

    auto with_credentials_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        with_credentials_property_value_0 = TRY(arg1.as_object().get("withCredentials"_utf16_fly_string));

    bool with_credentials_value_0;

    if (!with_credentials_property_value_0.is_undefined())

    with_credentials_value_0 = with_credentials_property_value_0.to_boolean();

    else
        with_credentials_value_0 = static_cast<bool>(false);

    event_source_init_dict.with_credentials = with_credentials_value_0;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::EventSource::construct_impl(realm, url, event_source_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface EventSource" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::EventSource::construct_impl().
...
void EventSourceConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "EventSource"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<EventSourcePrototype>(realm, "EventSource"_fly_string), 0);
