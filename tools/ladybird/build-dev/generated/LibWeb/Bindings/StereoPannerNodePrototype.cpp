...
GC_DEFINE_ALLOCATOR(StereoPannerNodePrototype);

StereoPannerNodePrototype::StereoPannerNodePrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

StereoPannerNodePrototype::~StereoPannerNodePrototype()
{
}

void StereoPannerNodePrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_pan_getter = JS::NativeFunction::create(realm, pan_getter, 0, "pan"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_pan_setter;

    define_direct_accessor("pan"_utf16_fly_string, native_pan_getter, native_pan_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "StereoPannerNode"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void StereoPannerNodePrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<WebAudio::StereoPannerNode*> impl_from(JS::VM& vm)
{
    ...
    if (!is<WebAudio::StereoPannerNode>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "StereoPannerNode");
    return static_cast<WebAudio::StereoPannerNode*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(StereoPannerNodePrototype::pan_getter)
{
    WebIDL::log_trace(vm, "StereoPannerNodePrototype::pan_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->pan(); }));
