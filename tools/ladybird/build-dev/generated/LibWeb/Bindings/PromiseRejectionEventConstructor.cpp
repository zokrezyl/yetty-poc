...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/PromiseRejectionEvent.h>
...
GC_DEFINE_ALLOCATOR(PromiseRejectionEventConstructor);

PromiseRejectionEventConstructor::PromiseRejectionEventConstructor(JS::Realm& realm)
    : NativeFunction("PromiseRejectionEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "PromiseRejectionEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> PromiseRejectionEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "PromiseRejectionEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface PromiseRejectionEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<PromiseRejectionEventPrototype>(*target_realm, "PromiseRejectionEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "PromiseRejectionEvent", "2");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "PromiseRejectionEventInit");

    PromiseRejectionEventInit event_init_dict {};

    auto promise_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        promise_property_value_0 = TRY(arg1.as_object().get("promise"_utf16_fly_string));

    if (promise_property_value_0.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "promise");

    if (!promise_property_value_0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObject, promise_property_value_0);
    auto promise_value_0 = GC::make_root(promise_property_value_0.as_object());

    event_init_dict.promise = promise_value_0;
    ...
    JS::Value reason_value_1 = JS::js_undefined();
    if (!reason_property_value_1.is_undefined())
        reason_value_1 = reason_property_value_1;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::PromiseRejectionEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface PromiseRejectionEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::PromiseRejectionEvent::construct_impl().
...
void PromiseRejectionEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "PromiseRejectionEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<PromiseRejectionEventPrototype>(realm, "PromiseRejectionEvent"_fly_string), 0);
