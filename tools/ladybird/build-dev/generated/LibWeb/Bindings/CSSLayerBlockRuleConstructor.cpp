...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSLayerBlockRule.h>
...
GC_DEFINE_ALLOCATOR(CSSLayerBlockRuleConstructor);

CSSLayerBlockRuleConstructor::CSSLayerBlockRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSLayerBlockRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSLayerBlockRuleConstructor::~CSSLayerBlockRuleConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSLayerBlockRuleConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSLayerBlockRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSLayerBlockRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSLayerBlockRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSLayerBlockRule");
}


void CSSLayerBlockRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<CSSGroupingRulePrototype>(realm, "CSSGroupingRule"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSLayerBlockRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSLayerBlockRulePrototype>(realm, "CSSLayerBlockRule"_fly_string), 0);
