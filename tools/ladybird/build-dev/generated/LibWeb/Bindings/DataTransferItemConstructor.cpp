...
GC_DEFINE_ALLOCATOR(DataTransferItemConstructor);

DataTransferItemConstructor::DataTransferItemConstructor(JS::Realm& realm)
    : NativeFunction("DataTransferItem"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DataTransferItemConstructor::~DataTransferItemConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DataTransferItemConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DataTransferItem");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DataTransferItemConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DataTransferItemConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "DataTransferItem");
}


void DataTransferItemConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DataTransferItem"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DataTransferItemPrototype>(realm, "DataTransferItem"_fly_string), 0);
