...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSTranslate.h>
...
GC_DEFINE_ALLOCATOR(CSSTranslateConstructor);

CSSTranslateConstructor::CSSTranslateConstructor(JS::Realm& realm)
    : NativeFunction("CSSTranslate"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSTranslateConstructor::~CSSTranslateConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSTranslateConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSTranslate");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSTranslateConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSTranslateConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSTranslate:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSTranslatePrototype>(*target_realm, "CSSTranslate"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "CSSTranslate", "2");

    auto arg0 = vm.argument(0);

    if (!arg0.is_object() || !is<CSSNumericValue>(arg0.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CSSNumericValue");

    auto& x = static_cast<CSSNumericValue&>(arg0.as_object());

    auto arg1 = vm.argument(1);

    if (!arg1.is_object() || !is<CSSNumericValue>(arg1.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CSSNumericValue");

    auto& y = static_cast<CSSNumericValue&>(arg1.as_object());

    auto arg2 = vm.argument(2);

    GC::Ptr<CSSNumericValue> z;
    if (!arg2.is_undefined()) {
        if (!arg2.is_object() || !is<CSSNumericValue>(arg2.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CSSNumericValue");

        z = static_cast<CSSNumericValue&>(arg2.as_object());
    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSTranslate::construct_impl(realm, x, y, z); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSTranslate" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSTranslate::construct_impl().
...
void CSSTranslateConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSTranslate"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSTranslatePrototype>(realm, "CSSTranslate"_fly_string), 0);
