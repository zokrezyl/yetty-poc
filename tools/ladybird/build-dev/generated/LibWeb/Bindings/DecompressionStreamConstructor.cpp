...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/Compression/DecompressionStream.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/Compression/CompressionStream.h>
...
GC_DEFINE_ALLOCATOR(DecompressionStreamConstructor);

DecompressionStreamConstructor::DecompressionStreamConstructor(JS::Realm& realm)
    : NativeFunction("DecompressionStream"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DecompressionStreamConstructor::~DecompressionStreamConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DecompressionStreamConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DecompressionStream");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DecompressionStreamConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DecompressionStreamConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DecompressionStream:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DecompressionStreamPrototype>(*target_realm, "DecompressionStream"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "DecompressionStream");

    auto arg0 = vm.argument(0);

    CompressionFormat format { CompressionFormat::Deflate };

    auto arg0_string = TRY(arg0.to_string(vm));

    if (arg0_string == "deflate"sv)
        format = CompressionFormat::Deflate;

    else if (arg0_string == "deflate-raw"sv)
        format = CompressionFormat::DeflateRaw;

    else if (arg0_string == "gzip"sv)
        format = CompressionFormat::Gzip;

    else
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidEnumerationValue, arg0_string, "CompressionFormat");

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Compression::DecompressionStream::construct_impl(realm, format); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DecompressionStream" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Compression::DecompressionStream::construct_impl().
...
void DecompressionStreamConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DecompressionStream"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DecompressionStreamPrototype>(realm, "DecompressionStream"_fly_string), 0);
