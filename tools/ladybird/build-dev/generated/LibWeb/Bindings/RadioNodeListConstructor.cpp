...
GC_DEFINE_ALLOCATOR(RadioNodeListConstructor);

RadioNodeListConstructor::RadioNodeListConstructor(JS::Realm& realm)
    : NativeFunction("RadioNodeList"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

RadioNodeListConstructor::~RadioNodeListConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> RadioNodeListConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "RadioNodeList");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> RadioNodeListConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "RadioNodeListConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "RadioNodeList");
}


void RadioNodeListConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<NodeListPrototype>(realm, "NodeList"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "RadioNodeList"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<RadioNodeListPrototype>(realm, "RadioNodeList"_fly_string), 0);
