...
void StoragePrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "key"_utf16_fly_string, key, 1, default_attributes);

    define_native_function(realm, "getItem"_utf16_fly_string, get_item, 1, default_attributes);

    define_native_function(realm, "setItem"_utf16_fly_string, set_item, 2, default_attributes);
    ...
    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "Storage"_string), JS::Attribute::Configurable);
...
JS_DEFINE_NATIVE_FUNCTION(StoragePrototype::key)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->key(index); }));
...
JS_DEFINE_NATIVE_FUNCTION(StoragePrototype::get_item)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_item(key); }));
...
JS_DEFINE_NATIVE_FUNCTION(StoragePrototype::set_item)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->set_item(key, value); }));
...
JS_DEFINE_NATIVE_FUNCTION(StoragePrototype::remove_item)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->remove_item(key); }));
