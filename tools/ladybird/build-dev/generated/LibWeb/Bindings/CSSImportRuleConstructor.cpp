...
GC_DEFINE_ALLOCATOR(CSSImportRuleConstructor);

CSSImportRuleConstructor::CSSImportRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSImportRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSImportRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSImportRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSImportRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSImportRule");
}


void CSSImportRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSImportRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSImportRulePrototype>(realm, "CSSImportRule"_fly_string), 0);
