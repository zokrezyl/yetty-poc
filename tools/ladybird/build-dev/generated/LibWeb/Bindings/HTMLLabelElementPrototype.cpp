...
GC_DEFINE_ALLOCATOR(HTMLLabelElementPrototype);

HTMLLabelElementPrototype::HTMLLabelElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void HTMLLabelElementPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_control_getter = JS::NativeFunction::create(realm, control_getter, 0, "control"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_control_setter;

    define_direct_accessor("control"_utf16_fly_string, native_control_getter, native_control_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "HTMLLabelElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void HTMLLabelElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::HTMLLabelElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::HTMLLabelElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLLabelElement");
    return static_cast<HTML::HTMLLabelElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(HTMLLabelElementPrototype::form_getter)
{
    WebIDL::log_trace(vm, "HTMLLabelElementPrototype::form_getter");
...
{
    WebIDL::log_trace(vm, "HTMLLabelElementPrototype::html_for_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "HTMLLabelElementPrototype::html_for_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(HTMLLabelElementPrototype::control_getter)
{
    WebIDL::log_trace(vm, "HTMLLabelElementPrototype::control_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->control(); }));
