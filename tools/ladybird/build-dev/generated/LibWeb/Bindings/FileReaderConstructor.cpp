...
GC_DEFINE_ALLOCATOR(FileReaderConstructor);

FileReaderConstructor::FileReaderConstructor(JS::Realm& realm)
    : NativeFunction("FileReader"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

FileReaderConstructor::~FileReaderConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> FileReaderConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "FileReader");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> FileReaderConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "FileReaderConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface FileReader:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<FileReaderPrototype>(*target_realm, "FileReader"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return FileAPI::FileReader::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface FileReader" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within FileAPI::FileReader::construct_impl().
...
void FileReaderConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "FileReader"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<FileReaderPrototype>(realm, "FileReader"_fly_string), 0);
