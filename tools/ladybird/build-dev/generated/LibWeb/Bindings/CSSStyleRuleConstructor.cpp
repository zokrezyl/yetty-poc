...
GC_DEFINE_ALLOCATOR(CSSStyleRuleConstructor);

CSSStyleRuleConstructor::CSSStyleRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSStyleRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSStyleRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSStyleRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSStyleRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSStyleRule");
}


void CSSStyleRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSStyleRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSStyleRulePrototype>(realm, "CSSStyleRule"_fly_string), 0);
