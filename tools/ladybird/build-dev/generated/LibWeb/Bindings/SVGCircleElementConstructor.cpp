...
GC_DEFINE_ALLOCATOR(SVGCircleElementConstructor);

SVGCircleElementConstructor::SVGCircleElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGCircleElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGCircleElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGCircleElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGCircleElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGCircleElement");
}


void SVGCircleElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGCircleElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGCircleElementPrototype>(realm, "SVGCircleElement"_fly_string), 0);
