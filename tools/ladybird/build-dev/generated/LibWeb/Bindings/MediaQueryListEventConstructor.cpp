...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/MediaQueryListEvent.h>
...
GC_DEFINE_ALLOCATOR(MediaQueryListEventConstructor);

MediaQueryListEventConstructor::MediaQueryListEventConstructor(JS::Realm& realm)
    : NativeFunction("MediaQueryListEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MediaQueryListEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MediaQueryListEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MediaQueryListEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface MediaQueryListEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<MediaQueryListEventPrototype>(*target_realm, "MediaQueryListEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "MediaQueryListEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "MediaQueryListEventInit");

    MediaQueryListEventInit event_init_dict {};

    auto matches_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        matches_property_value_0 = TRY(arg1.as_object().get("matches"_utf16_fly_string));

    bool matches_value_0;

    if (!matches_property_value_0.is_undefined())

    matches_value_0 = matches_property_value_0.to_boolean();

    else
        matches_value_0 = static_cast<bool>(false);

    event_init_dict.matches = matches_value_0;

    auto media_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        media_property_value_1 = TRY(arg1.as_object().get("media"_utf16_fly_string));

    String media_value_1;

    if (!media_property_value_1.is_undefined()) {
        if (!false || !media_property_value_1.is_null())
            media_value_1 = TRY(WebIDL::to_string(vm, media_property_value_1));
    } else {
        media_value_1 = ""_string;
    }

    event_init_dict.media = media_value_1;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::MediaQueryListEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface MediaQueryListEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::MediaQueryListEvent::construct_impl().
...
void MediaQueryListEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MediaQueryListEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MediaQueryListEventPrototype>(realm, "MediaQueryListEvent"_fly_string), 0);
