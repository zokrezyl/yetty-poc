...
GC_DEFINE_ALLOCATOR(PerformanceMarkConstructor);

PerformanceMarkConstructor::PerformanceMarkConstructor(JS::Realm& realm)
    : NativeFunction("PerformanceMark"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

PerformanceMarkConstructor::~PerformanceMarkConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> PerformanceMarkConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "PerformanceMark");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> PerformanceMarkConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "PerformanceMarkConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface PerformanceMark:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<PerformanceMarkPrototype>(*target_realm, "PerformanceMark"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "PerformanceMark");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return PerformanceMark::construct_impl(realm, mark_name, mark_options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface PerformanceMark" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within PerformanceMark::construct_impl().
...
void PerformanceMarkConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<PerformanceEntryPrototype>(realm, "PerformanceEntry"_fly_string));

    define_direct_property(vm.names.length, JS::Value(1), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "PerformanceMark"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<PerformanceMarkPrototype>(realm, "PerformanceMark"_fly_string), 0);
