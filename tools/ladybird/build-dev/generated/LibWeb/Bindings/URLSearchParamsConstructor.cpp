...
GC_DEFINE_ALLOCATOR(URLSearchParamsConstructor);

URLSearchParamsConstructor::URLSearchParamsConstructor(JS::Realm& realm)
    : NativeFunction("URLSearchParams"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "URLSearchParams");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> URLSearchParamsConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "URLSearchParamsConstructor::construct");
    ...
    auto arg0_to_variant = [&vm, &realm](JS::Value arg0) -> JS::ThrowCompletionOr<Variant<Vector<Vector<String>>, OrderedHashMap<String, String>, String>> {
    ...
    if (!false || !next_item2.is_null()) {
        sequence_item2 = TRY(WebIDL::to_usv_string(vm, next_item2));
    }
    ...
    if (!false || !key1.is_null()) {
        typed_key1 = TRY(WebIDL::to_usv_string(vm, key1));
    }
    ...
    if (!false || !value1.is_null()) {
        typed_value1 = TRY(WebIDL::to_usv_string(vm, value1));
    }
    ...
    Variant<Vector<Vector<String>>, OrderedHashMap<String, String>, String> init = arg0.is_undefined() ? TRY(arg0_to_variant(JS::Value(JS::PrimitiveString::create(vm, String {})))) : TRY(arg0_to_variant(arg0));
...
void URLSearchParamsConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "URLSearchParams"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<URLSearchParamsPrototype>(realm, "URLSearchParams"_fly_string), 0);
