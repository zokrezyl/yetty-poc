...
GC_DEFINE_ALLOCATOR(CSSKeywordValueConstructor);

CSSKeywordValueConstructor::CSSKeywordValueConstructor(JS::Realm& realm)
    : NativeFunction("CSSKeywordValue"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSKeywordValueConstructor::~CSSKeywordValueConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSKeywordValueConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSKeywordValue");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSKeywordValueConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSKeywordValueConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSKeywordValue:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSKeywordValuePrototype>(*target_realm, "CSSKeywordValue"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSKeywordValue");

    auto arg0 = vm.argument(0);

    String value;
    if (!false || !arg0.is_null()) {
        value = TRY(WebIDL::to_usv_string(vm, arg0));
    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSKeywordValue::construct_impl(realm, value); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSKeywordValue" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSKeywordValue::construct_impl().
...
void CSSKeywordValueConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSKeywordValue"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSKeywordValuePrototype>(realm, "CSSKeywordValue"_fly_string), 0);
