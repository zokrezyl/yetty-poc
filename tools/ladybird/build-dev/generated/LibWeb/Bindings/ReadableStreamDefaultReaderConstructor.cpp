...
GC_DEFINE_ALLOCATOR(ReadableStreamDefaultReaderConstructor);

ReadableStreamDefaultReaderConstructor::ReadableStreamDefaultReaderConstructor(JS::Realm& realm)
    : NativeFunction("ReadableStreamDefaultReader"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ReadableStreamDefaultReaderConstructor::~ReadableStreamDefaultReaderConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ReadableStreamDefaultReaderConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ReadableStreamDefaultReader");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ReadableStreamDefaultReaderConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ReadableStreamDefaultReaderConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface ReadableStreamDefaultReader:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<ReadableStreamDefaultReaderPrototype>(*target_realm, "ReadableStreamDefaultReader"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "ReadableStreamDefaultReader");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Streams::ReadableStreamDefaultReader::construct_impl(realm, stream); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface ReadableStreamDefaultReader" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Streams::ReadableStreamDefaultReader::construct_impl().
...
void ReadableStreamDefaultReaderConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ReadableStreamDefaultReader"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ReadableStreamDefaultReaderPrototype>(realm, "ReadableStreamDefaultReader"_fly_string), 0);
