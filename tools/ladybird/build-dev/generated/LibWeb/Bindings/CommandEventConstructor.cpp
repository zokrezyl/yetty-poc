...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/CommandEvent.h>
...
GC_DEFINE_ALLOCATOR(CommandEventConstructor);

CommandEventConstructor::CommandEventConstructor(JS::Realm& realm)
    : NativeFunction("CommandEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CommandEventConstructor::~CommandEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CommandEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CommandEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CommandEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CommandEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CommandEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CommandEventPrototype>(*target_realm, "CommandEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CommandEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CommandEventInit");

    CommandEventInit event_init_dict {};

    auto command_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        command_property_value_0 = TRY(arg1.as_object().get("command"_utf16_fly_string));

    String command_value_0;

    if (!command_property_value_0.is_undefined()) {
        if (!false || !command_property_value_0.is_null())
            command_value_0 = TRY(WebIDL::to_string(vm, command_property_value_0));
    } else {
        command_value_0 = ""_string;
    }

    event_init_dict.command = command_value_0;

    auto source_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        source_property_value_1 = TRY(arg1.as_object().get("source"_utf16_fly_string));
    ...
    event_init_dict.source = source_value_1;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::CommandEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CommandEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::CommandEvent::construct_impl().
...
void CommandEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CommandEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CommandEventPrototype>(realm, "CommandEvent"_fly_string), 0);
