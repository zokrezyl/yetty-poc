...
GC_DEFINE_ALLOCATOR(RangeConstructor);

RangeConstructor::RangeConstructor(JS::Realm& realm)
    : NativeFunction("Range"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Range");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> RangeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "RangeConstructor::construct");
...
void RangeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<AbstractRangePrototype>(realm, "AbstractRange"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Range"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<RangePrototype>(realm, "Range"_fly_string), 0);
