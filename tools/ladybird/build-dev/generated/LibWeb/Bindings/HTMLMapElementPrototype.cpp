...
GC_DEFINE_ALLOCATOR(HTMLMapElementPrototype);

HTMLMapElementPrototype::HTMLMapElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void HTMLMapElementPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_areas_getter = JS::NativeFunction::create(realm, areas_getter, 0, "areas"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_areas_setter;

    define_direct_accessor("areas"_utf16_fly_string, native_areas_getter, native_areas_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "HTMLMapElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void HTMLMapElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::HTMLMapElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::HTMLMapElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLMapElement");
    return static_cast<HTML::HTMLMapElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(HTMLMapElementPrototype::name_getter)
{
    WebIDL::log_trace(vm, "HTMLMapElementPrototype::name_getter");
...
{
    WebIDL::log_trace(vm, "HTMLMapElementPrototype::name_setter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(HTMLMapElementPrototype::areas_getter)
{
    WebIDL::log_trace(vm, "HTMLMapElementPrototype::areas_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->areas(); }));
