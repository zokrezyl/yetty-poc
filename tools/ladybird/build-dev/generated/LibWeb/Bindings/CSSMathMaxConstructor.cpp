...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMathMax.h>
...
GC_DEFINE_ALLOCATOR(CSSMathMaxConstructor);

CSSMathMaxConstructor::CSSMathMaxConstructor(JS::Realm& realm)
    : NativeFunction("CSSMathMax"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMathMaxConstructor::~CSSMathMaxConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMathMaxConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMathMax");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMathMaxConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMathMaxConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSMathMax:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSMathMaxPrototype>(*target_realm, "CSSMathMax"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSMathMax::construct_impl(realm, move(args)); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSMathMax" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSMathMax::construct_impl().
...
void CSSMathMaxConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMathMax"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMathMaxPrototype>(realm, "CSSMathMax"_fly_string), 0);
