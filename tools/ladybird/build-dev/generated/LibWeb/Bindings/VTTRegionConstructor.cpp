...
GC_DEFINE_ALLOCATOR(VTTRegionConstructor);

VTTRegionConstructor::VTTRegionConstructor(JS::Realm& realm)
    : NativeFunction("VTTRegion"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

VTTRegionConstructor::~VTTRegionConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> VTTRegionConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "VTTRegion");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> VTTRegionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "VTTRegionConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface VTTRegion:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<VTTRegionPrototype>(*target_realm, "VTTRegion"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return VTTRegion::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface VTTRegion" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within VTTRegion::construct_impl().
...
void VTTRegionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "VTTRegion"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<VTTRegionPrototype>(realm, "VTTRegion"_fly_string), 0);
