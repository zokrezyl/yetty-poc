...
GC_DEFINE_ALLOCATOR(DynamicsCompressorNodeConstructor);

DynamicsCompressorNodeConstructor::DynamicsCompressorNodeConstructor(JS::Realm& realm)
    : NativeFunction("DynamicsCompressorNode"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DynamicsCompressorNodeConstructor::~DynamicsCompressorNodeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DynamicsCompressorNodeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DynamicsCompressorNode");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DynamicsCompressorNodeConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DynamicsCompressorNodeConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DynamicsCompressorNode:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DynamicsCompressorNodePrototype>(*target_realm, "DynamicsCompressorNode"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "DynamicsCompressorNode");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DynamicsCompressorOptions");

    DynamicsCompressorOptions options {};

    auto attack_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        attack_property_value_0 = TRY(arg1.as_object().get("attack"_utf16_fly_string));

    float attack_value_0;

    if (!attack_property_value_0.is_undefined())
        attack_value_0 = TRY(attack_property_value_0.to_double(vm));

    else
        attack_value_0 = 0.003;

    if (isinf(attack_value_0) || isnan(attack_value_0)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "attack");
    }
    
    options.attack = attack_value_0;

    auto knee_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        knee_property_value_1 = TRY(arg1.as_object().get("knee"_utf16_fly_string));

    float knee_value_1;

    if (!knee_property_value_1.is_undefined())
        knee_value_1 = TRY(knee_property_value_1.to_double(vm));

    else
        knee_value_1 = 30;

    if (isinf(knee_value_1) || isnan(knee_value_1)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "knee");
    }
    
    options.knee = knee_value_1;

    auto ratio_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        ratio_property_value_2 = TRY(arg1.as_object().get("ratio"_utf16_fly_string));

    float ratio_value_2;

    if (!ratio_property_value_2.is_undefined())
        ratio_value_2 = TRY(ratio_property_value_2.to_double(vm));

    else
        ratio_value_2 = 12;

    if (isinf(ratio_value_2) || isnan(ratio_value_2)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "ratio");
    }
    
    options.ratio = ratio_value_2;

    auto release_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        release_property_value_3 = TRY(arg1.as_object().get("release"_utf16_fly_string));

    float release_value_3;

    if (!release_property_value_3.is_undefined())
        release_value_3 = TRY(release_property_value_3.to_double(vm));

    else
        release_value_3 = 0.25;

    if (isinf(release_value_3) || isnan(release_value_3)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "release");
    }
    
    options.release = release_value_3;
    ...
    float threshold_value_4;

    if (!threshold_property_value_4.is_undefined())
        threshold_value_4 = TRY(threshold_property_value_4.to_double(vm));

    else
        threshold_value_4 = -24;

    if (isinf(threshold_value_4) || isnan(threshold_value_4)) {
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return WebAudio::DynamicsCompressorNode::construct_impl(realm, context, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DynamicsCompressorNode" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within WebAudio::DynamicsCompressorNode::construct_impl().
...
void DynamicsCompressorNodeConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DynamicsCompressorNode"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DynamicsCompressorNodePrototype>(realm, "DynamicsCompressorNode"_fly_string), 0);
