...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/Encoding/TextEncoderStream.h>
...
GC_DEFINE_ALLOCATOR(TextEncoderStreamConstructor);

TextEncoderStreamConstructor::TextEncoderStreamConstructor(JS::Realm& realm)
    : NativeFunction("TextEncoderStream"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

TextEncoderStreamConstructor::~TextEncoderStreamConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> TextEncoderStreamConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "TextEncoderStream");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> TextEncoderStreamConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "TextEncoderStreamConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface TextEncoderStream:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<TextEncoderStreamPrototype>(*target_realm, "TextEncoderStream"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Encoding::TextEncoderStream::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface TextEncoderStream" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Encoding::TextEncoderStream::construct_impl().
...
void TextEncoderStreamConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "TextEncoderStream"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<TextEncoderStreamPrototype>(realm, "TextEncoderStream"_fly_string), 0);
