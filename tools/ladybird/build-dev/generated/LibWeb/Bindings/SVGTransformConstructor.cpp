...
GC_DEFINE_ALLOCATOR(SVGTransformConstructor);

SVGTransformConstructor::SVGTransformConstructor(JS::Realm& realm)
    : NativeFunction("SVGTransform"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGTransformConstructor::~SVGTransformConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGTransformConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGTransform");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGTransformConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGTransformConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGTransform");
}


void SVGTransformConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGTransform"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGTransformPrototype>(realm, "SVGTransform"_fly_string), 0);
