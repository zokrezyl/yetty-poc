...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/DOM/MutationObserver.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/DOM/MutationRecord.h>
...
GC_DEFINE_ALLOCATOR(MutationObserverPrototype);

MutationObserverPrototype::MutationObserverPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

MutationObserverPrototype::~MutationObserverPrototype()
{
}

void MutationObserverPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "observe"_utf16_fly_string, observe, 1, default_attributes);
    ...
    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "MutationObserver"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void MutationObserverPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<DOM::MutationObserver*> impl_from(JS::VM& vm)
{
    ...
    if (!is<DOM::MutationObserver>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "MutationObserver");
    return static_cast<DOM::MutationObserver*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(MutationObserverPrototype::observe)
{
    WebIDL::log_trace(vm, "MutationObserverPrototype::observe");
    ...
    auto& target = static_cast<Node&>(arg0.as_object());

    auto arg1 = vm.argument(1);

    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "MutationObserverInit");

    MutationObserverInit options {};

    auto attribute_filter_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        attribute_filter_property_value_0 = TRY(arg1.as_object().get("attributeFilter"_utf16_fly_string));

    if (!attribute_filter_property_value_0.is_undefined()) {

    Optional<Vector<String>> attribute_filter_value_0;

    if (!attribute_filter_property_value_0.is_undefined()) {

    if (!attribute_filter_property_value_0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObject, attribute_filter_property_value_0);

    auto attribute_filter_property_value_0_iterator_method0 = TRY(attribute_filter_property_value_0.get_method(vm, vm.well_known_symbol_iterator()));
    if (!attribute_filter_property_value_0_iterator_method0)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotIterable, attribute_filter_property_value_0);

    auto attribute_filter_property_value_0_iterator1 = TRY(JS::get_iterator_from_method(vm, attribute_filter_property_value_0, *attribute_filter_property_value_0_iterator_method0));

    Vector<String> attribute_filter_value_0_non_optional;

    for (;;) {
        auto next1 = TRY(JS::iterator_step(vm, attribute_filter_property_value_0_iterator1));
    ...
    attribute_filter_value_0_non_optional.append(sequence_item1);
    }

        attribute_filter_value_0 = move(attribute_filter_value_0_non_optional);
    }

    options.attribute_filter = attribute_filter_value_0;

    }

    auto attribute_old_value_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        attribute_old_value_property_value_1 = TRY(arg1.as_object().get("attributeOldValue"_utf16_fly_string));

    if (!attribute_old_value_property_value_1.is_undefined()) {

    Optional<bool> attribute_old_value_value_1;

    if (!attribute_old_value_property_value_1.is_undefined())

    attribute_old_value_value_1 = attribute_old_value_property_value_1.to_boolean();

    options.attribute_old_value = attribute_old_value_value_1;

    }

    auto attributes_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        attributes_property_value_2 = TRY(arg1.as_object().get("attributes"_utf16_fly_string));

    if (!attributes_property_value_2.is_undefined()) {

    Optional<bool> attributes_value_2;

    if (!attributes_property_value_2.is_undefined())

    attributes_value_2 = attributes_property_value_2.to_boolean();

    options.attributes = attributes_value_2;

    }

    auto character_data_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        character_data_property_value_3 = TRY(arg1.as_object().get("characterData"_utf16_fly_string));

    if (!character_data_property_value_3.is_undefined()) {

    Optional<bool> character_data_value_3;

    if (!character_data_property_value_3.is_undefined())

    character_data_value_3 = character_data_property_value_3.to_boolean();

    options.character_data = character_data_value_3;

    }

    auto character_data_old_value_property_value_4 = JS::js_undefined();
    if (arg1.is_object())
        character_data_old_value_property_value_4 = TRY(arg1.as_object().get("characterDataOldValue"_utf16_fly_string));

    if (!character_data_old_value_property_value_4.is_undefined()) {

    Optional<bool> character_data_old_value_value_4;

    if (!character_data_old_value_property_value_4.is_undefined())

    character_data_old_value_value_4 = character_data_old_value_property_value_4.to_boolean();

    options.character_data_old_value = character_data_old_value_value_4;

    }

    auto child_list_property_value_5 = JS::js_undefined();
    if (arg1.is_object())
        child_list_property_value_5 = TRY(arg1.as_object().get("childList"_utf16_fly_string));

    bool child_list_value_5;

    if (!child_list_property_value_5.is_undefined())

    child_list_value_5 = child_list_property_value_5.to_boolean();

    else
        child_list_value_5 = static_cast<bool>(false);

    options.child_list = child_list_value_5;

    auto subtree_property_value_6 = JS::js_undefined();
    if (arg1.is_object())
        subtree_property_value_6 = TRY(arg1.as_object().get("subtree"_utf16_fly_string));

    bool subtree_value_6;

    if (!subtree_property_value_6.is_undefined())

    subtree_value_6 = subtree_property_value_6.to_boolean();

    else
        subtree_value_6 = static_cast<bool>(false);

    options.subtree = subtree_value_6;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->observe(target, options); }));

    return JS::js_undefined();

}

JS_DEFINE_NATIVE_FUNCTION(MutationObserverPrototype::disconnect)
{
    WebIDL::log_trace(vm, "MutationObserverPrototype::disconnect");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(MutationObserverPrototype::take_records)
{
    WebIDL::log_trace(vm, "MutationObserverPrototype::take_records");
    [[maybe_unused]] auto& realm = *vm.current_realm();
