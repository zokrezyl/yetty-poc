...
GC_DEFINE_ALLOCATOR(OffscreenCanvasConstructor);

OffscreenCanvasConstructor::OffscreenCanvasConstructor(JS::Realm& realm)
    : NativeFunction("OffscreenCanvas"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

OffscreenCanvasConstructor::~OffscreenCanvasConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> OffscreenCanvasConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "OffscreenCanvas");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> OffscreenCanvasConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "OffscreenCanvasConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface OffscreenCanvas:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<OffscreenCanvasPrototype>(*target_realm, "OffscreenCanvas"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "OffscreenCanvas", "2");

    auto arg0 = vm.argument(0);

    WebIDL::UnsignedLongLong width;

    width = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLongLong>(vm, arg0, WebIDL::EnforceRange::Yes, WebIDL::Clamp::No));

    auto arg1 = vm.argument(1);

    WebIDL::UnsignedLongLong height;

    height = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLongLong>(vm, arg1, WebIDL::EnforceRange::Yes, WebIDL::Clamp::No));

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::OffscreenCanvas::construct_impl(realm, width, height); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface OffscreenCanvas" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::OffscreenCanvas::construct_impl().
...
void OffscreenCanvasConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "OffscreenCanvas"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<OffscreenCanvasPrototype>(realm, "OffscreenCanvas"_fly_string), 0);
