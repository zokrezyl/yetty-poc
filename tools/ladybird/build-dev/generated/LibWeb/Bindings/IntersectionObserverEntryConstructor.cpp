...
GC_DEFINE_ALLOCATOR(IntersectionObserverEntryConstructor);

IntersectionObserverEntryConstructor::IntersectionObserverEntryConstructor(JS::Realm& realm)
    : NativeFunction("IntersectionObserverEntry"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

IntersectionObserverEntryConstructor::~IntersectionObserverEntryConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> IntersectionObserverEntryConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "IntersectionObserverEntry");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> IntersectionObserverEntryConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "IntersectionObserverEntryConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "IntersectionObserverEntry");
}


void IntersectionObserverEntryConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "IntersectionObserverEntry"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<IntersectionObserverEntryPrototype>(realm, "IntersectionObserverEntry"_fly_string), 0);
