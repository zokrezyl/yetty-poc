...
GC_DEFINE_ALLOCATOR(ElementInternalsConstructor);

ElementInternalsConstructor::ElementInternalsConstructor(JS::Realm& realm)
    : NativeFunction("ElementInternals"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ElementInternalsConstructor::~ElementInternalsConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ElementInternalsConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ElementInternals");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ElementInternalsConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ElementInternalsConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "ElementInternals");
}


void ElementInternalsConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ElementInternals"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ElementInternalsPrototype>(realm, "ElementInternals"_fly_string), 0);
