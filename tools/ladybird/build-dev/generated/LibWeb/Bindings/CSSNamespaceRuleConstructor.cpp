...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSNamespaceRule.h>
...
GC_DEFINE_ALLOCATOR(CSSNamespaceRuleConstructor);

CSSNamespaceRuleConstructor::CSSNamespaceRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSNamespaceRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSNamespaceRuleConstructor::~CSSNamespaceRuleConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSNamespaceRuleConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSNamespaceRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSNamespaceRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSNamespaceRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSNamespaceRule");
}


void CSSNamespaceRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSNamespaceRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSNamespaceRulePrototype>(realm, "CSSNamespaceRule"_fly_string), 0);
