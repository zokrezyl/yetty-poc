...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/HTMLSelectElement.h>
...
GC_DEFINE_ALLOCATOR(HTMLSelectElementConstructor);

HTMLSelectElementConstructor::HTMLSelectElementConstructor(JS::Realm& realm)
    : NativeFunction("HTMLSelectElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLSelectElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLSelectElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLSelectElementConstructor::construct");
    ...
    // 6. Otherwise (i.e., if definition is for a customized built-in element):
        ...
        static auto valid_local_names = MUST(DOM::valid_local_names_for_given_html_element_interface("HTMLSelectElement"sv));
        ...
        if (!valid_local_names.contains_slow(definition->local_name()))
            return vm.throw_completion<JS::TypeError>(MUST(String::formatted("Local name '{}' of customized built-in element is not a valid local name for HTMLSelectElement", definition->local_name())));
    ...
    if (definition->construction_stack().is_empty()) {
        ...
        // Important steps from "internally create a new object implementing the interface"
        {
            ...
            if (!prototype.is_object()) {
                ...
                VERIFY(target_realm);
                prototype = &Bindings::ensure_web_prototype<HTMLSelectElementPrototype>(*target_realm, "HTMLSelectElement"_fly_string);
            }
...
void HTMLSelectElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLSelectElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLSelectElementPrototype>(realm, "HTMLSelectElement"_fly_string), 0);
