...
GC_DEFINE_ALLOCATOR(MediaErrorConstructor);

MediaErrorConstructor::MediaErrorConstructor(JS::Realm& realm)
    : NativeFunction("MediaError"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

MediaErrorConstructor::~MediaErrorConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> MediaErrorConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MediaError");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MediaErrorConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MediaErrorConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "MediaError");
}


void MediaErrorConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MediaError"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MediaErrorPrototype>(realm, "MediaError"_fly_string), 0);
