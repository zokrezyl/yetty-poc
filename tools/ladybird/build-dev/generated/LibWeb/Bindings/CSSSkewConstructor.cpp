...
GC_DEFINE_ALLOCATOR(CSSSkewConstructor);

CSSSkewConstructor::CSSSkewConstructor(JS::Realm& realm)
    : NativeFunction("CSSSkew"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSSkewConstructor::~CSSSkewConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSSkewConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSSkew");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSSkewConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSSkewConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSSkew:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSSkewPrototype>(*target_realm, "CSSSkew"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "CSSSkew", "2");
    ...
    auto& ax = static_cast<CSSNumericValue&>(arg0.as_object());
    ...
    auto& ay = static_cast<CSSNumericValue&>(arg1.as_object());

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSSkew::construct_impl(realm, ax, ay); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSSkew" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSSkew::construct_impl().
...
void CSSSkewConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSSkew"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSSkewPrototype>(realm, "CSSSkew"_fly_string), 0);
