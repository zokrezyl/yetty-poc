...
GC_DEFINE_ALLOCATOR(CompositionEventPrototype);

CompositionEventPrototype::CompositionEventPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

CompositionEventPrototype::~CompositionEventPrototype()
{
}

void CompositionEventPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "initCompositionEvent"_utf16_fly_string, init_composition_event, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "CompositionEvent"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void CompositionEventPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<UIEvents::CompositionEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<UIEvents::CompositionEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CompositionEvent");
    return static_cast<UIEvents::CompositionEvent*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(CompositionEventPrototype::data_getter)
{
    WebIDL::log_trace(vm, "CompositionEventPrototype::data_getter");
...
JS_DEFINE_NATIVE_FUNCTION(CompositionEventPrototype::init_composition_event)
{
    WebIDL::log_trace(vm, "CompositionEventPrototype::init_composition_event");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "initCompositionEvent");
    ...
    if (!arg4.is_undefined()) {
        if (!false || !arg4.is_null())
            data_arg = TRY(WebIDL::to_string(vm, arg4));
    } else {
        data_arg = ""_string;
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->init_composition_event(type_arg, bubbles_arg, cancelable_arg, view_arg, data_arg); }));
