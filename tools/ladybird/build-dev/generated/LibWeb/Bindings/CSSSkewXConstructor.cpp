...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSSkewX.h>
...
GC_DEFINE_ALLOCATOR(CSSSkewXConstructor);

CSSSkewXConstructor::CSSSkewXConstructor(JS::Realm& realm)
    : NativeFunction("CSSSkewX"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSSkewXConstructor::~CSSSkewXConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSSkewXConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSSkewX");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSSkewXConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSSkewXConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSSkewX:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSSkewXPrototype>(*target_realm, "CSSSkewX"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSSkewX");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSSkewX::construct_impl(realm, ax); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSSkewX" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSSkewX::construct_impl().
...
void CSSSkewXConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSSkewX"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSSkewXPrototype>(realm, "CSSSkewX"_fly_string), 0);
