...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/Animations/ScrollTimeline.h>
...
GC_DEFINE_ALLOCATOR(ScrollTimelineConstructor);

ScrollTimelineConstructor::ScrollTimelineConstructor(JS::Realm& realm)
    : NativeFunction("ScrollTimeline"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

ScrollTimelineConstructor::~ScrollTimelineConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> ScrollTimelineConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ScrollTimeline");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ScrollTimelineConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ScrollTimelineConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface ScrollTimeline:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<ScrollTimelinePrototype>(*target_realm, "ScrollTimeline"_fly_string);
    }
    ...
    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ScrollTimelineOptions");

    ScrollTimelineOptions options {};

    auto axis_property_value_0 = JS::js_undefined();
    if (arg0.is_object())
        axis_property_value_0 = TRY(arg0.as_object().get("axis"_utf16_fly_string));

    ScrollAxis axis_value_0 { ScrollAxis::Block };

    if (!axis_property_value_0.is_undefined()) {

    auto axis_property_value_0_string = TRY(axis_property_value_0.to_string(vm));

    if (axis_property_value_0_string == "block"sv)
        axis_value_0 = ScrollAxis::Block;

    else if (axis_property_value_0_string == "inline"sv)
        axis_value_0 = ScrollAxis::Inline;

    else if (axis_property_value_0_string == "x"sv)
        axis_value_0 = ScrollAxis::X;

    else if (axis_property_value_0_string == "y"sv)
        axis_value_0 = ScrollAxis::Y;

    else
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidEnumerationValue, axis_property_value_0_string, "ScrollAxis");

    }

    options.axis = axis_value_0;
    ...
    GC::Ptr<Element> source_value_1;

    if (!source_property_value_1.is_nullish()) {
        if (!source_property_value_1.is_object() || !is<Element>(source_property_value_1.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "Element");

        source_value_1 = &static_cast<Element&>(source_property_value_1.as_object());
    }

    options.source = source_value_1;

    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return ScrollTimeline::construct_impl(realm, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface ScrollTimeline" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within ScrollTimeline::construct_impl().
...
void ScrollTimelineConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<AnimationTimelinePrototype>(realm, "AnimationTimeline"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ScrollTimeline"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ScrollTimelinePrototype>(realm, "ScrollTimeline"_fly_string), 0);
