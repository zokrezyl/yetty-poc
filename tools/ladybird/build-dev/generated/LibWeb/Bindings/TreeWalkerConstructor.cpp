...
GC_DEFINE_ALLOCATOR(TreeWalkerConstructor);

TreeWalkerConstructor::TreeWalkerConstructor(JS::Realm& realm)
    : NativeFunction("TreeWalker"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "TreeWalker");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> TreeWalkerConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "TreeWalkerConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "TreeWalker");
}


void TreeWalkerConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "TreeWalker"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<TreeWalkerPrototype>(realm, "TreeWalker"_fly_string), 0);
