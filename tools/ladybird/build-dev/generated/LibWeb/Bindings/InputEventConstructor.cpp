...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/UIEvents/InputEvent.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/UIEvents/UIEvent.h>

#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/DOM/StaticRange.h>
...
GC_DEFINE_ALLOCATOR(InputEventConstructor);

InputEventConstructor::InputEventConstructor(JS::Realm& realm)
    : NativeFunction("InputEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

InputEventConstructor::~InputEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> InputEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "InputEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> InputEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "InputEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface InputEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<InputEventPrototype>(*target_realm, "InputEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "InputEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "InputEventInit");
    ...
    Optional<Utf16String> data_value_0;

    if (!data_property_value_0.is_undefined()) {
        if (!data_property_value_0.is_null())
            data_value_0 = TRY(WebIDL::to_utf16_string(vm, data_property_value_0));
    }

    event_init_dict.data = data_value_0;

    auto input_type_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        input_type_property_value_1 = TRY(arg1.as_object().get("inputType"_utf16_fly_string));

    String input_type_value_1;

    if (!input_type_property_value_1.is_undefined()) {
        if (!false || !input_type_property_value_1.is_null())
            input_type_value_1 = TRY(WebIDL::to_string(vm, input_type_property_value_1));
    } else {
        input_type_value_1 = ""_string;
    }

    event_init_dict.input_type = input_type_value_1;
    ...
    auto detail_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        detail_property_value_3 = TRY(arg1.as_object().get("detail"_utf16_fly_string));

    WebIDL::Long detail_value_3;

    if (!detail_property_value_3.is_undefined())

    detail_value_3 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, detail_property_value_3, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        detail_value_3 = static_cast<WebIDL::Long>(0);

    event_init_dict.detail = detail_value_3;

    auto view_property_value_4 = JS::js_undefined();
    if (arg1.is_object())
        view_property_value_4 = TRY(arg1.as_object().get("view"_utf16_fly_string));

    GC::Ptr<WindowProxy> view_value_4;

    if (!view_property_value_4.is_nullish()) {
        if (!view_property_value_4.is_object() || !is<WindowProxy>(view_property_value_4.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WindowProxy");

        view_value_4 = &static_cast<WindowProxy&>(view_property_value_4.as_object());
    }

    event_init_dict.view = view_value_4;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return UIEvents::InputEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface InputEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within UIEvents::InputEvent::construct_impl().
...
void InputEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "InputEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<InputEventPrototype>(realm, "InputEvent"_fly_string), 0);
