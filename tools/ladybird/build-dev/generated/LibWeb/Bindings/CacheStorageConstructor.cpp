...
GC_DEFINE_ALLOCATOR(CacheStorageConstructor);

CacheStorageConstructor::CacheStorageConstructor(JS::Realm& realm)
    : NativeFunction("CacheStorage"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CacheStorageConstructor::~CacheStorageConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CacheStorageConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CacheStorage");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CacheStorageConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CacheStorageConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CacheStorage");
}


void CacheStorageConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CacheStorage"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CacheStoragePrototype>(realm, "CacheStorage"_fly_string), 0);
