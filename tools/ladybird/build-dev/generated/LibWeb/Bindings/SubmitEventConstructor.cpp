...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/SubmitEvent.h>
...
GC_DEFINE_ALLOCATOR(SubmitEventConstructor);

SubmitEventConstructor::SubmitEventConstructor(JS::Realm& realm)
    : NativeFunction("SubmitEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SubmitEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SubmitEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SubmitEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface SubmitEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<SubmitEventPrototype>(*target_realm, "SubmitEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "SubmitEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "SubmitEventInit");

    SubmitEventInit event_init_dict {};

    auto submitter_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        submitter_property_value_0 = TRY(arg1.as_object().get("submitter"_utf16_fly_string));

    GC::Ptr<HTMLElement> submitter_value_0;

    if (!submitter_property_value_0.is_nullish()) {
        if (!submitter_property_value_0.is_object() || !is<HTMLElement>(submitter_property_value_0.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "HTMLElement");

        submitter_value_0 = &static_cast<HTMLElement&>(submitter_property_value_0.as_object());
    }

    event_init_dict.submitter = submitter_value_0;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::SubmitEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface SubmitEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::SubmitEvent::construct_impl().
...
void SubmitEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SubmitEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SubmitEventPrototype>(realm, "SubmitEvent"_fly_string), 0);
