...
GC_DEFINE_ALLOCATOR(ElementConstructor);

ElementConstructor::ElementConstructor(JS::Realm& realm)
    : NativeFunction("Element"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Element");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "Element");
}


void ElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Element"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ElementPrototype>(realm, "Element"_fly_string), 0);
