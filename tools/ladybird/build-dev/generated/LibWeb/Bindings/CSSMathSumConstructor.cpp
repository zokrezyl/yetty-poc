...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMathSum.h>
...
GC_DEFINE_ALLOCATOR(CSSMathSumConstructor);

CSSMathSumConstructor::CSSMathSumConstructor(JS::Realm& realm)
    : NativeFunction("CSSMathSum"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMathSumConstructor::~CSSMathSumConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMathSumConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMathSum");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMathSumConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMathSumConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSMathSum:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSMathSumPrototype>(*target_realm, "CSSMathSum"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSMathSum::construct_impl(realm, move(args)); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSMathSum" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSMathSum::construct_impl().
...
void CSSMathSumConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMathSum"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMathSumPrototype>(realm, "CSSMathSum"_fly_string), 0);
