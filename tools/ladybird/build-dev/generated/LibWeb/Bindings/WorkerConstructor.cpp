...
GC_DEFINE_ALLOCATOR(WorkerConstructor);

WorkerConstructor::WorkerConstructor(JS::Realm& realm)
    : NativeFunction("Worker"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Worker");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> WorkerConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "WorkerConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface Worker:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<WorkerPrototype>(*target_realm, "Worker"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "Worker");
    ...
    options.credentials = credentials_value_0;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::Worker::construct_impl(realm, script_url, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface Worker" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::Worker::construct_impl().
...
void WorkerConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Worker"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<WorkerPrototype>(realm, "Worker"_fly_string), 0);
