...
GC_DEFINE_ALLOCATOR(InstanceConstructor);

InstanceConstructor::InstanceConstructor(JS::Realm& realm)
    : NativeFunction("Instance"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<JS::Value> InstanceConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "WebAssembly.Instance");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> InstanceConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "InstanceConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface Instance:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<InstancePrototype>(*target_realm, "WebAssembly.Instance"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "Instance");
    ...
    auto& module = static_cast<Module&>(arg0.as_object());
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Instance::construct_impl(realm, module, import_object); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface Instance" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Instance::construct_impl().
...
void InstanceConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Instance"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<InstancePrototype>(realm, "WebAssembly.Instance"_fly_string), 0);
