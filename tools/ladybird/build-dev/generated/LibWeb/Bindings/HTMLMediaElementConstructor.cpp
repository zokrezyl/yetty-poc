...
GC_DEFINE_ALLOCATOR(HTMLMediaElementConstructor);

HTMLMediaElementConstructor::HTMLMediaElementConstructor(JS::Realm& realm)
    : NativeFunction("HTMLMediaElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLMediaElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLMediaElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLMediaElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "HTMLMediaElement");
}


void HTMLMediaElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLMediaElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLMediaElementPrototype>(realm, "HTMLMediaElement"_fly_string), 0);
