...
GC_DEFINE_ALLOCATOR(SVGForeignObjectElementConstructor);

SVGForeignObjectElementConstructor::SVGForeignObjectElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGForeignObjectElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGForeignObjectElementConstructor::~SVGForeignObjectElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGForeignObjectElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGForeignObjectElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGForeignObjectElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGForeignObjectElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGForeignObjectElement");
}


void SVGForeignObjectElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGForeignObjectElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGForeignObjectElementPrototype>(realm, "SVGForeignObjectElement"_fly_string), 0);
