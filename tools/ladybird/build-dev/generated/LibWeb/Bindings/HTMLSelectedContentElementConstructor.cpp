...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/HTMLSelectedContentElement.h>
...
GC_DEFINE_ALLOCATOR(HTMLSelectedContentElementConstructor);

HTMLSelectedContentElementConstructor::HTMLSelectedContentElementConstructor(JS::Realm& realm)
    : NativeFunction("HTMLSelectedContentElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

HTMLSelectedContentElementConstructor::~HTMLSelectedContentElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> HTMLSelectedContentElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLSelectedContentElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLSelectedContentElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLSelectedContentElementConstructor::construct");
    ...
    // 6. Otherwise (i.e., if definition is for a customized built-in element):
        ...
        static auto valid_local_names = MUST(DOM::valid_local_names_for_given_html_element_interface("HTMLSelectedContentElement"sv));
        ...
        if (!valid_local_names.contains_slow(definition->local_name()))
            return vm.throw_completion<JS::TypeError>(MUST(String::formatted("Local name '{}' of customized built-in element is not a valid local name for HTMLSelectedContentElement", definition->local_name())));
    ...
    if (definition->construction_stack().is_empty()) {
        ...
        auto element = realm.create<HTML::HTMLSelectedContentElement>(window.associated_document(), DOM::QualifiedName { definition->local_name(), {}, Namespace::HTML });
        ...
        // Important steps from "internally create a new object implementing the interface"
        {
            ...
            if (!prototype.is_object()) {
                ...
                VERIFY(target_realm);
                prototype = &Bindings::ensure_web_prototype<HTMLSelectedContentElementPrototype>(*target_realm, "HTMLSelectedContentElement"_fly_string);
            }
    ...
    if (!prototype.is_object()) {
        ...
        prototype = &Bindings::ensure_web_prototype<HTMLSelectedContentElementPrototype>(*function_realm, "HTMLSelectedContentElement"_fly_string);
    }
...
void HTMLSelectedContentElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLSelectedContentElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLSelectedContentElementPrototype>(realm, "HTMLSelectedContentElement"_fly_string), 0);
