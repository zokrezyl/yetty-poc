...
GC_DEFINE_ALLOCATOR(ResizeObserverConstructor);

ResizeObserverConstructor::ResizeObserverConstructor(JS::Realm& realm)
    : NativeFunction("ResizeObserver"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "ResizeObserver");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> ResizeObserverConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "ResizeObserverConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface ResizeObserver:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<ResizeObserverPrototype>(*target_realm, "ResizeObserver"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "ResizeObserver");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return ResizeObserver::ResizeObserver::construct_impl(realm, callback); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface ResizeObserver" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within ResizeObserver::ResizeObserver::construct_impl().
...
void ResizeObserverConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "ResizeObserver"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<ResizeObserverPrototype>(realm, "ResizeObserver"_fly_string), 0);
