...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/IntersectionObserver/IntersectionObserver.h>
...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/IntersectionObserver/IntersectionObserverEntry.h>
...
GC_DEFINE_ALLOCATOR(IntersectionObserverConstructor);

IntersectionObserverConstructor::IntersectionObserverConstructor(JS::Realm& realm)
    : NativeFunction("IntersectionObserver"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "IntersectionObserver");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> IntersectionObserverConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "IntersectionObserverConstructor::construct");
    ...
    if (arg1.is_object())
        delay_property_value_0 = TRY(arg1.as_object().get("delay"_utf16_fly_string));

    WebIDL::Long delay_value_0;

    if (!delay_property_value_0.is_undefined())

    delay_value_0 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, delay_property_value_0, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        delay_value_0 = static_cast<WebIDL::Long>(0);

    options.delay = delay_value_0;

    auto root_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        root_property_value_1 = TRY(arg1.as_object().get("root"_utf16_fly_string));

    auto root_property_value_1_to_variant = [&vm, &realm](JS::Value root_property_value_1) -> JS::ThrowCompletionOr<Variant<GC::Root<Element>, GC::Root<Document>>> {
        // These might be unused.
        (void)vm;
        (void)realm;

        if (root_property_value_1.is_object()) {
            [[maybe_unused]] auto& root_property_value_1_object = root_property_value_1.as_object();

            if (is<PlatformObject>(root_property_value_1_object)) {

                if (auto* root_property_value_1_result = as_if<Element>(root_property_value_1_object))
                    return GC::make_root(*root_property_value_1_result);

                if (auto* root_property_value_1_result = as_if<Document>(root_property_value_1_object))
                    return GC::make_root(*root_property_value_1_result);
    ...
    Optional<Variant<GC::Root<Element>, GC::Root<Document>>> root_value_1;
    if (!root_property_value_1.is_nullish())
        root_value_1 = TRY(root_property_value_1_to_variant(root_property_value_1));

    options.root = root_value_1;

    auto root_margin_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        root_margin_property_value_2 = TRY(arg1.as_object().get("rootMargin"_utf16_fly_string));

    String root_margin_value_2;

    if (!root_margin_property_value_2.is_undefined()) {
        if (!false || !root_margin_property_value_2.is_null())
            root_margin_value_2 = TRY(WebIDL::to_string(vm, root_margin_property_value_2));
    } else {
        root_margin_value_2 = "0px"_string;
    }

    options.root_margin = root_margin_value_2;

    auto scroll_margin_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        scroll_margin_property_value_3 = TRY(arg1.as_object().get("scrollMargin"_utf16_fly_string));

    String scroll_margin_value_3;

    if (!scroll_margin_property_value_3.is_undefined()) {
        if (!false || !scroll_margin_property_value_3.is_null())
            scroll_margin_value_3 = TRY(WebIDL::to_string(vm, scroll_margin_property_value_3));
    } else {
        scroll_margin_value_3 = "0px"_string;
    }

    options.scroll_margin = scroll_margin_value_3;

    auto threshold_property_value_4 = JS::js_undefined();
    if (arg1.is_object())
        threshold_property_value_4 = TRY(arg1.as_object().get("threshold"_utf16_fly_string));

    auto threshold_property_value_4_to_variant = [&vm, &realm](JS::Value threshold_property_value_4) -> JS::ThrowCompletionOr<Variant<double, Vector<double>>> {
        // These might be unused.
        (void)vm;
        (void)realm;

        if (threshold_property_value_4.is_object()) {
            [[maybe_unused]] auto& threshold_property_value_4_object = threshold_property_value_4.as_object();

        auto method = TRY(threshold_property_value_4.get_method(vm, vm.well_known_symbol_iterator()));

        if (method) {

    auto threshold_property_value_4_iterator1 = TRY(JS::get_iterator_from_method(vm, threshold_property_value_4, *method));

    Vector<double> threshold_value_4;

    for (;;) {
        auto next1 = TRY(JS::iterator_step(vm, threshold_property_value_4_iterator1));
    ...
    if (isinf(sequence_item1) || isnan(sequence_item1)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "threshold_property_value_4");
    }
    
    threshold_value_4.append(sequence_item1);
    }


            return threshold_value_4;
        }

        }

        if (threshold_property_value_4.is_number()) {

    double threshold_property_value_4_number = TRY(threshold_property_value_4.to_double(vm));

    if (isinf(threshold_property_value_4_number) || isnan(threshold_property_value_4_number)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "threshold");
    }
    
            return { threshold_property_value_4_number };
        }
    ...
    Variant<double, Vector<double>> threshold_value_4 = threshold_property_value_4.is_undefined() ? 0 : TRY(threshold_property_value_4_to_variant(threshold_property_value_4));

    options.threshold = threshold_value_4;

    auto track_visibility_property_value_5 = JS::js_undefined();
    if (arg1.is_object())
        track_visibility_property_value_5 = TRY(arg1.as_object().get("trackVisibility"_utf16_fly_string));

    bool track_visibility_value_5;

    if (!track_visibility_property_value_5.is_undefined())

    track_visibility_value_5 = track_visibility_property_value_5.to_boolean();

    else
        track_visibility_value_5 = static_cast<bool>(false);

    options.track_visibility = track_visibility_value_5;
...
void IntersectionObserverConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "IntersectionObserver"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<IntersectionObserverPrototype>(realm, "IntersectionObserver"_fly_string), 0);
