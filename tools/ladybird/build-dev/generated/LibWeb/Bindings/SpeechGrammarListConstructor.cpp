...
GC_DEFINE_ALLOCATOR(SpeechGrammarListConstructor);

SpeechGrammarListConstructor::SpeechGrammarListConstructor(JS::Realm& realm)
    : NativeFunction("SpeechGrammarList"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SpeechGrammarListConstructor::~SpeechGrammarListConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SpeechGrammarListConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SpeechGrammarList");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SpeechGrammarListConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SpeechGrammarListConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface SpeechGrammarList:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<SpeechGrammarListPrototype>(*target_realm, "SpeechGrammarList"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return SpeechGrammarList::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface SpeechGrammarList" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within SpeechGrammarList::construct_impl().
...
void SpeechGrammarListConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SpeechGrammarList"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SpeechGrammarListPrototype>(realm, "SpeechGrammarList"_fly_string), 0);
