...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/XPath/XPathEvaluator.h>
...
GC_DEFINE_ALLOCATOR(XPathEvaluatorConstructor);

XPathEvaluatorConstructor::XPathEvaluatorConstructor(JS::Realm& realm)
    : NativeFunction("XPathEvaluator"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

XPathEvaluatorConstructor::~XPathEvaluatorConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> XPathEvaluatorConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "XPathEvaluator");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> XPathEvaluatorConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "XPathEvaluatorConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface XPathEvaluator:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<XPathEvaluatorPrototype>(*target_realm, "XPathEvaluator"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return XPath::XPathEvaluator::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface XPathEvaluator" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within XPath::XPathEvaluator::construct_impl().
...
void XPathEvaluatorConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "XPathEvaluator"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<XPathEvaluatorPrototype>(realm, "XPathEvaluator"_fly_string), 0);
