...
GC_DEFINE_ALLOCATOR(WindowPrototype);

WindowPrototype::WindowPrototype([[maybe_unused]] JS::Realm& realm)
...
void WindowPrototype::initialize(JS::Realm& realm)
{
    ...
    define_direct_property(vm().well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm(), "Window"_string), JS::Attribute::Configurable);
    set_prototype(&ensure_web_prototype<WindowPrototype>(realm, "WindowProperties"_fly_string));

}

#include <LibWeb/WebIDL/AbstractOperations.h>

GC_DEFINE_ALLOCATOR(WindowProperties);

WindowProperties::WindowProperties(JS::Realm& realm)
...
void WindowProperties::initialize(JS::Realm& realm)
{
    Base::initialize(realm);
    auto& vm = realm.vm();
    ...
    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "WindowProperties"_string), JS::Attribute::Configurable);
...
JS::ThrowCompletionOr<Optional<JS::PropertyDescriptor>> WindowProperties::internal_get_own_property(JS::PropertyKey const& property_name) const
{
...
JS::ThrowCompletionOr<bool> WindowProperties::internal_define_own_property(JS::PropertyKey const&, JS::PropertyDescriptor&, Optional<JS::PropertyDescriptor>*)
{
    // 1. Return false.
    return false;
}
...
JS::ThrowCompletionOr<bool> WindowProperties::internal_delete(JS::PropertyKey const&)
{
    // 1. Return false.
    return false;
}
...
JS::ThrowCompletionOr<bool> WindowProperties::internal_set_prototype_of(JS::Object* prototype)
{
...
JS::ThrowCompletionOr<bool> WindowProperties::internal_prevent_extensions()
{
    // 1. Return false.
...
void WindowProperties::visit_edges(Visitor& visitor)
{
