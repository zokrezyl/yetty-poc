...
GC_DEFINE_ALLOCATOR(NamedNodeMapConstructor);

NamedNodeMapConstructor::NamedNodeMapConstructor(JS::Realm& realm)
    : NativeFunction("NamedNodeMap"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "NamedNodeMap");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> NamedNodeMapConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "NamedNodeMapConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "NamedNodeMap");
}


void NamedNodeMapConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "NamedNodeMap"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<NamedNodeMapPrototype>(realm, "NamedNodeMap"_fly_string), 0);
