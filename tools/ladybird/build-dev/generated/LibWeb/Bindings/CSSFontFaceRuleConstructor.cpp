...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSFontFaceRule.h>
...
GC_DEFINE_ALLOCATOR(CSSFontFaceRuleConstructor);

CSSFontFaceRuleConstructor::CSSFontFaceRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSFontFaceRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSFontFaceRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSFontFaceRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSFontFaceRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSFontFaceRule");
}


void CSSFontFaceRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSFontFaceRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSFontFaceRulePrototype>(realm, "CSSFontFaceRule"_fly_string), 0);
