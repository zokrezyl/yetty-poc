...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/MediaSourceExtensions/BufferedChangeEvent.h>
...
GC_DEFINE_ALLOCATOR(BufferedChangeEventConstructor);

BufferedChangeEventConstructor::BufferedChangeEventConstructor(JS::Realm& realm)
    : NativeFunction("BufferedChangeEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

BufferedChangeEventConstructor::~BufferedChangeEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> BufferedChangeEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "BufferedChangeEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> BufferedChangeEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "BufferedChangeEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface BufferedChangeEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<BufferedChangeEventPrototype>(*target_realm, "BufferedChangeEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "BufferedChangeEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "BufferedChangeEventInit");

    BufferedChangeEventInit event_init_dict {};

    auto added_ranges_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        added_ranges_property_value_0 = TRY(arg1.as_object().get("addedRanges"_utf16_fly_string));

    if (!added_ranges_property_value_0.is_undefined()) {

    GC::Ptr<TimeRanges> added_ranges_value_0;
    if (!added_ranges_property_value_0.is_undefined()) {
        if (!added_ranges_property_value_0.is_object() || !is<TimeRanges>(added_ranges_property_value_0.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "TimeRanges");

        added_ranges_value_0 = static_cast<TimeRanges&>(added_ranges_property_value_0.as_object());
    }

    event_init_dict.added_ranges = added_ranges_value_0;

    }

    auto removed_ranges_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        removed_ranges_property_value_1 = TRY(arg1.as_object().get("removedRanges"_utf16_fly_string));

    if (!removed_ranges_property_value_1.is_undefined()) {

    GC::Ptr<TimeRanges> removed_ranges_value_1;
    if (!removed_ranges_property_value_1.is_undefined()) {
        if (!removed_ranges_property_value_1.is_object() || !is<TimeRanges>(removed_ranges_property_value_1.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "TimeRanges");

        removed_ranges_value_1 = static_cast<TimeRanges&>(removed_ranges_property_value_1.as_object());
    }

    event_init_dict.removed_ranges = removed_ranges_value_1;
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return MediaSourceExtensions::BufferedChangeEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface BufferedChangeEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within MediaSourceExtensions::BufferedChangeEvent::construct_impl().
...
void BufferedChangeEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "BufferedChangeEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<BufferedChangeEventPrototype>(realm, "BufferedChangeEvent"_fly_string), 0);
