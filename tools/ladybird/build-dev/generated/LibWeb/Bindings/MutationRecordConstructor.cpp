...
GC_DEFINE_ALLOCATOR(MutationRecordConstructor);

MutationRecordConstructor::MutationRecordConstructor(JS::Realm& realm)
    : NativeFunction("MutationRecord"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

MutationRecordConstructor::~MutationRecordConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> MutationRecordConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "MutationRecord");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> MutationRecordConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "MutationRecordConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "MutationRecord");
}


void MutationRecordConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "MutationRecord"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<MutationRecordPrototype>(realm, "MutationRecord"_fly_string), 0);
