...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/UIEvents/WheelEvent.h>
...
GC_DEFINE_ALLOCATOR(WheelEventConstructor);

WheelEventConstructor::WheelEventConstructor(JS::Realm& realm)
    : NativeFunction("WheelEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

WheelEventConstructor::~WheelEventConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> WheelEventConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "WheelEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> WheelEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "WheelEventConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface WheelEvent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<WheelEventPrototype>(*target_realm, "WheelEvent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "WheelEvent");
    ...
    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WheelEventInit");

    WheelEventInit event_init_dict {};

    auto delta_mode_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        delta_mode_property_value_0 = TRY(arg1.as_object().get("deltaMode"_utf16_fly_string));

    WebIDL::UnsignedLong delta_mode_value_0;

    if (!delta_mode_property_value_0.is_undefined())

    delta_mode_value_0 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedLong>(vm, delta_mode_property_value_0, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        delta_mode_value_0 = static_cast<WebIDL::UnsignedLong>(0);

    event_init_dict.delta_mode = delta_mode_value_0;

    auto delta_x_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        delta_x_property_value_1 = TRY(arg1.as_object().get("deltaX"_utf16_fly_string));

    double delta_x_value_1;

    if (!delta_x_property_value_1.is_undefined())
        delta_x_value_1 = TRY(delta_x_property_value_1.to_double(vm));

    else
        delta_x_value_1 = 0;

    if (isinf(delta_x_value_1) || isnan(delta_x_value_1)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "deltaX");
    }
    
    event_init_dict.delta_x = delta_x_value_1;

    auto delta_y_property_value_2 = JS::js_undefined();
    if (arg1.is_object())
        delta_y_property_value_2 = TRY(arg1.as_object().get("deltaY"_utf16_fly_string));

    double delta_y_value_2;

    if (!delta_y_property_value_2.is_undefined())
        delta_y_value_2 = TRY(delta_y_property_value_2.to_double(vm));

    else
        delta_y_value_2 = 0;

    if (isinf(delta_y_value_2) || isnan(delta_y_value_2)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "deltaY");
    }
    
    event_init_dict.delta_y = delta_y_value_2;

    auto delta_z_property_value_3 = JS::js_undefined();
    if (arg1.is_object())
        delta_z_property_value_3 = TRY(arg1.as_object().get("deltaZ"_utf16_fly_string));

    double delta_z_value_3;

    if (!delta_z_property_value_3.is_undefined())
        delta_z_value_3 = TRY(delta_z_property_value_3.to_double(vm));

    else
        delta_z_value_3 = 0;

    if (isinf(delta_z_value_3) || isnan(delta_z_value_3)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "deltaZ");
    }
    
    event_init_dict.delta_z = delta_z_value_3;

    auto button_property_value_4 = JS::js_undefined();
    if (arg1.is_object())
        button_property_value_4 = TRY(arg1.as_object().get("button"_utf16_fly_string));

    WebIDL::Short button_value_4;

    if (!button_property_value_4.is_undefined())

    button_value_4 = TRY(WebIDL::convert_to_int<WebIDL::Short>(vm, button_property_value_4, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        button_value_4 = static_cast<WebIDL::Short>(0);

    event_init_dict.button = button_value_4;

    auto buttons_property_value_5 = JS::js_undefined();
    if (arg1.is_object())
        buttons_property_value_5 = TRY(arg1.as_object().get("buttons"_utf16_fly_string));

    WebIDL::UnsignedShort buttons_value_5;

    if (!buttons_property_value_5.is_undefined())

    buttons_value_5 = TRY(WebIDL::convert_to_int<WebIDL::UnsignedShort>(vm, buttons_property_value_5, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        buttons_value_5 = static_cast<WebIDL::UnsignedShort>(0);

    event_init_dict.buttons = buttons_value_5;

    auto client_x_property_value_6 = JS::js_undefined();
    if (arg1.is_object())
        client_x_property_value_6 = TRY(arg1.as_object().get("clientX"_utf16_fly_string));

    double client_x_value_6;

    if (!client_x_property_value_6.is_undefined())
        client_x_value_6 = TRY(client_x_property_value_6.to_double(vm));

    else
        client_x_value_6 = 0;

    if (isinf(client_x_value_6) || isnan(client_x_value_6)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "clientX");
    }
    
    event_init_dict.client_x = client_x_value_6;

    auto client_y_property_value_7 = JS::js_undefined();
    if (arg1.is_object())
        client_y_property_value_7 = TRY(arg1.as_object().get("clientY"_utf16_fly_string));

    double client_y_value_7;

    if (!client_y_property_value_7.is_undefined())
        client_y_value_7 = TRY(client_y_property_value_7.to_double(vm));

    else
        client_y_value_7 = 0;

    if (isinf(client_y_value_7) || isnan(client_y_value_7)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "clientY");
    }
    
    event_init_dict.client_y = client_y_value_7;

    auto movement_x_property_value_8 = JS::js_undefined();
    if (arg1.is_object())
        movement_x_property_value_8 = TRY(arg1.as_object().get("movementX"_utf16_fly_string));

    double movement_x_value_8;

    if (!movement_x_property_value_8.is_undefined())
        movement_x_value_8 = TRY(movement_x_property_value_8.to_double(vm));

    else
        movement_x_value_8 = 0;

    if (isinf(movement_x_value_8) || isnan(movement_x_value_8)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "movementX");
    }
    
    event_init_dict.movement_x = movement_x_value_8;

    auto movement_y_property_value_9 = JS::js_undefined();
    if (arg1.is_object())
        movement_y_property_value_9 = TRY(arg1.as_object().get("movementY"_utf16_fly_string));

    double movement_y_value_9;

    if (!movement_y_property_value_9.is_undefined())
        movement_y_value_9 = TRY(movement_y_property_value_9.to_double(vm));

    else
        movement_y_value_9 = 0;

    if (isinf(movement_y_value_9) || isnan(movement_y_value_9)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "movementY");
    }
    
    event_init_dict.movement_y = movement_y_value_9;

    auto related_target_property_value_10 = JS::js_undefined();
    if (arg1.is_object())
        related_target_property_value_10 = TRY(arg1.as_object().get("relatedTarget"_utf16_fly_string));

    GC::Ptr<EventTarget> related_target_value_10;

    if (!related_target_property_value_10.is_nullish()) {
        if (!related_target_property_value_10.is_object() || !is<EventTarget>(related_target_property_value_10.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "EventTarget");

        related_target_value_10 = &static_cast<EventTarget&>(related_target_property_value_10.as_object());
    }

    event_init_dict.related_target = related_target_value_10;

    auto screen_x_property_value_11 = JS::js_undefined();
    if (arg1.is_object())
        screen_x_property_value_11 = TRY(arg1.as_object().get("screenX"_utf16_fly_string));

    double screen_x_value_11;

    if (!screen_x_property_value_11.is_undefined())
        screen_x_value_11 = TRY(screen_x_property_value_11.to_double(vm));

    else
        screen_x_value_11 = 0;

    if (isinf(screen_x_value_11) || isnan(screen_x_value_11)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "screenX");
    }
    
    event_init_dict.screen_x = screen_x_value_11;

    auto screen_y_property_value_12 = JS::js_undefined();
    if (arg1.is_object())
        screen_y_property_value_12 = TRY(arg1.as_object().get("screenY"_utf16_fly_string));

    double screen_y_value_12;

    if (!screen_y_property_value_12.is_undefined())
        screen_y_value_12 = TRY(screen_y_property_value_12.to_double(vm));

    else
        screen_y_value_12 = 0;

    if (isinf(screen_y_value_12) || isnan(screen_y_value_12)) {
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::InvalidRestrictedFloatingPointParameter, "screenY");
    }
    
    event_init_dict.screen_y = screen_y_value_12;

    auto alt_key_property_value_13 = JS::js_undefined();
    if (arg1.is_object())
        alt_key_property_value_13 = TRY(arg1.as_object().get("altKey"_utf16_fly_string));

    bool alt_key_value_13;

    if (!alt_key_property_value_13.is_undefined())

    alt_key_value_13 = alt_key_property_value_13.to_boolean();

    else
        alt_key_value_13 = static_cast<bool>(false);

    event_init_dict.alt_key = alt_key_value_13;

    auto ctrl_key_property_value_14 = JS::js_undefined();
    if (arg1.is_object())
        ctrl_key_property_value_14 = TRY(arg1.as_object().get("ctrlKey"_utf16_fly_string));

    bool ctrl_key_value_14;

    if (!ctrl_key_property_value_14.is_undefined())

    ctrl_key_value_14 = ctrl_key_property_value_14.to_boolean();

    else
        ctrl_key_value_14 = static_cast<bool>(false);

    event_init_dict.ctrl_key = ctrl_key_value_14;

    auto meta_key_property_value_15 = JS::js_undefined();
    if (arg1.is_object())
        meta_key_property_value_15 = TRY(arg1.as_object().get("metaKey"_utf16_fly_string));

    bool meta_key_value_15;

    if (!meta_key_property_value_15.is_undefined())

    meta_key_value_15 = meta_key_property_value_15.to_boolean();

    else
        meta_key_value_15 = static_cast<bool>(false);

    event_init_dict.meta_key = meta_key_value_15;

    auto modifier_alt_graph_property_value_16 = JS::js_undefined();
    if (arg1.is_object())
        modifier_alt_graph_property_value_16 = TRY(arg1.as_object().get("modifierAltGraph"_utf16_fly_string));

    bool modifier_alt_graph_value_16;

    if (!modifier_alt_graph_property_value_16.is_undefined())

    modifier_alt_graph_value_16 = modifier_alt_graph_property_value_16.to_boolean();

    else
        modifier_alt_graph_value_16 = static_cast<bool>(false);

    event_init_dict.modifier_alt_graph = modifier_alt_graph_value_16;

    auto modifier_caps_lock_property_value_17 = JS::js_undefined();
    if (arg1.is_object())
        modifier_caps_lock_property_value_17 = TRY(arg1.as_object().get("modifierCapsLock"_utf16_fly_string));

    bool modifier_caps_lock_value_17;

    if (!modifier_caps_lock_property_value_17.is_undefined())

    modifier_caps_lock_value_17 = modifier_caps_lock_property_value_17.to_boolean();

    else
        modifier_caps_lock_value_17 = static_cast<bool>(false);

    event_init_dict.modifier_caps_lock = modifier_caps_lock_value_17;

    auto modifier_fn_property_value_18 = JS::js_undefined();
    if (arg1.is_object())
        modifier_fn_property_value_18 = TRY(arg1.as_object().get("modifierFn"_utf16_fly_string));

    bool modifier_fn_value_18;

    if (!modifier_fn_property_value_18.is_undefined())

    modifier_fn_value_18 = modifier_fn_property_value_18.to_boolean();

    else
        modifier_fn_value_18 = static_cast<bool>(false);

    event_init_dict.modifier_fn = modifier_fn_value_18;

    auto modifier_fn_lock_property_value_19 = JS::js_undefined();
    if (arg1.is_object())
        modifier_fn_lock_property_value_19 = TRY(arg1.as_object().get("modifierFnLock"_utf16_fly_string));

    bool modifier_fn_lock_value_19;

    if (!modifier_fn_lock_property_value_19.is_undefined())

    modifier_fn_lock_value_19 = modifier_fn_lock_property_value_19.to_boolean();

    else
        modifier_fn_lock_value_19 = static_cast<bool>(false);

    event_init_dict.modifier_fn_lock = modifier_fn_lock_value_19;

    auto modifier_hyper_property_value_20 = JS::js_undefined();
    if (arg1.is_object())
        modifier_hyper_property_value_20 = TRY(arg1.as_object().get("modifierHyper"_utf16_fly_string));

    bool modifier_hyper_value_20;

    if (!modifier_hyper_property_value_20.is_undefined())

    modifier_hyper_value_20 = modifier_hyper_property_value_20.to_boolean();

    else
        modifier_hyper_value_20 = static_cast<bool>(false);

    event_init_dict.modifier_hyper = modifier_hyper_value_20;

    auto modifier_num_lock_property_value_21 = JS::js_undefined();
    if (arg1.is_object())
        modifier_num_lock_property_value_21 = TRY(arg1.as_object().get("modifierNumLock"_utf16_fly_string));

    bool modifier_num_lock_value_21;

    if (!modifier_num_lock_property_value_21.is_undefined())

    modifier_num_lock_value_21 = modifier_num_lock_property_value_21.to_boolean();

    else
        modifier_num_lock_value_21 = static_cast<bool>(false);

    event_init_dict.modifier_num_lock = modifier_num_lock_value_21;

    auto modifier_scroll_lock_property_value_22 = JS::js_undefined();
    if (arg1.is_object())
        modifier_scroll_lock_property_value_22 = TRY(arg1.as_object().get("modifierScrollLock"_utf16_fly_string));

    bool modifier_scroll_lock_value_22;

    if (!modifier_scroll_lock_property_value_22.is_undefined())

    modifier_scroll_lock_value_22 = modifier_scroll_lock_property_value_22.to_boolean();

    else
        modifier_scroll_lock_value_22 = static_cast<bool>(false);

    event_init_dict.modifier_scroll_lock = modifier_scroll_lock_value_22;

    auto modifier_super_property_value_23 = JS::js_undefined();
    if (arg1.is_object())
        modifier_super_property_value_23 = TRY(arg1.as_object().get("modifierSuper"_utf16_fly_string));

    bool modifier_super_value_23;

    if (!modifier_super_property_value_23.is_undefined())

    modifier_super_value_23 = modifier_super_property_value_23.to_boolean();

    else
        modifier_super_value_23 = static_cast<bool>(false);

    event_init_dict.modifier_super = modifier_super_value_23;

    auto modifier_symbol_property_value_24 = JS::js_undefined();
    if (arg1.is_object())
        modifier_symbol_property_value_24 = TRY(arg1.as_object().get("modifierSymbol"_utf16_fly_string));

    bool modifier_symbol_value_24;

    if (!modifier_symbol_property_value_24.is_undefined())

    modifier_symbol_value_24 = modifier_symbol_property_value_24.to_boolean();

    else
        modifier_symbol_value_24 = static_cast<bool>(false);

    event_init_dict.modifier_symbol = modifier_symbol_value_24;

    auto modifier_symbol_lock_property_value_25 = JS::js_undefined();
    if (arg1.is_object())
        modifier_symbol_lock_property_value_25 = TRY(arg1.as_object().get("modifierSymbolLock"_utf16_fly_string));

    bool modifier_symbol_lock_value_25;

    if (!modifier_symbol_lock_property_value_25.is_undefined())

    modifier_symbol_lock_value_25 = modifier_symbol_lock_property_value_25.to_boolean();

    else
        modifier_symbol_lock_value_25 = static_cast<bool>(false);

    event_init_dict.modifier_symbol_lock = modifier_symbol_lock_value_25;

    auto shift_key_property_value_26 = JS::js_undefined();
    if (arg1.is_object())
        shift_key_property_value_26 = TRY(arg1.as_object().get("shiftKey"_utf16_fly_string));

    bool shift_key_value_26;

    if (!shift_key_property_value_26.is_undefined())

    shift_key_value_26 = shift_key_property_value_26.to_boolean();

    else
        shift_key_value_26 = static_cast<bool>(false);

    event_init_dict.shift_key = shift_key_value_26;

    auto detail_property_value_27 = JS::js_undefined();
    if (arg1.is_object())
        detail_property_value_27 = TRY(arg1.as_object().get("detail"_utf16_fly_string));

    WebIDL::Long detail_value_27;

    if (!detail_property_value_27.is_undefined())

    detail_value_27 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, detail_property_value_27, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        detail_value_27 = static_cast<WebIDL::Long>(0);

    event_init_dict.detail = detail_value_27;

    auto view_property_value_28 = JS::js_undefined();
    if (arg1.is_object())
        view_property_value_28 = TRY(arg1.as_object().get("view"_utf16_fly_string));

    GC::Ptr<WindowProxy> view_value_28;

    if (!view_property_value_28.is_nullish()) {
        if (!view_property_value_28.is_object() || !is<WindowProxy>(view_property_value_28.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WindowProxy");

        view_value_28 = &static_cast<WindowProxy&>(view_property_value_28.as_object());
    }

    event_init_dict.view = view_value_28;

    auto bubbles_property_value_29 = JS::js_undefined();
    if (arg1.is_object())
        bubbles_property_value_29 = TRY(arg1.as_object().get("bubbles"_utf16_fly_string));

    bool bubbles_value_29;

    if (!bubbles_property_value_29.is_undefined())

    bubbles_value_29 = bubbles_property_value_29.to_boolean();

    else
        bubbles_value_29 = static_cast<bool>(false);

    event_init_dict.bubbles = bubbles_value_29;

    auto cancelable_property_value_30 = JS::js_undefined();
    if (arg1.is_object())
        cancelable_property_value_30 = TRY(arg1.as_object().get("cancelable"_utf16_fly_string));

    bool cancelable_value_30;

    if (!cancelable_property_value_30.is_undefined())

    cancelable_value_30 = cancelable_property_value_30.to_boolean();

    else
        cancelable_value_30 = static_cast<bool>(false);

    event_init_dict.cancelable = cancelable_value_30;

    auto composed_property_value_31 = JS::js_undefined();
    if (arg1.is_object())
        composed_property_value_31 = TRY(arg1.as_object().get("composed"_utf16_fly_string));

    bool composed_value_31;

    if (!composed_property_value_31.is_undefined())

    composed_value_31 = composed_property_value_31.to_boolean();

    else
        composed_value_31 = static_cast<bool>(false);

    event_init_dict.composed = composed_value_31;

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return UIEvents::WheelEvent::construct_impl(realm, type, event_init_dict); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface WheelEvent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within UIEvents::WheelEvent::construct_impl().
...
void WheelEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "WheelEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<WheelEventPrototype>(realm, "WheelEvent"_fly_string), 0);


    auto constant_DOM_DELTA_PIXEL_value = JS::Value(static_cast<WebIDL::UnsignedLong>(0x00));

    define_direct_property("DOM_DELTA_PIXEL"_utf16_fly_string, constant_DOM_DELTA_PIXEL_value, JS::Attribute::Enumerable);

    auto constant_DOM_DELTA_LINE_value = JS::Value(static_cast<WebIDL::UnsignedLong>(0x01));

    define_direct_property("DOM_DELTA_LINE"_utf16_fly_string, constant_DOM_DELTA_LINE_value, JS::Attribute::Enumerable);

    auto constant_DOM_DELTA_PAGE_value = JS::Value(static_cast<WebIDL::UnsignedLong>(0x02));

    define_direct_property("DOM_DELTA_PAGE"_utf16_fly_string, constant_DOM_DELTA_PAGE_value, JS::Attribute::Enumerable);
