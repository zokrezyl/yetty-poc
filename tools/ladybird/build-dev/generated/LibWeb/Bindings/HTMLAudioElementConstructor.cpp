...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/HTML/HTMLAudioElement.h>
...
GC_DEFINE_ALLOCATOR(HTMLAudioElementConstructor);

HTMLAudioElementConstructor::HTMLAudioElementConstructor(JS::Realm& realm)
    : NativeFunction("HTMLAudioElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLAudioElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLAudioElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLAudioElementConstructor::construct");
    ...
    // 6. Otherwise (i.e., if definition is for a customized built-in element):
        ...
        static auto valid_local_names = MUST(DOM::valid_local_names_for_given_html_element_interface("HTMLAudioElement"sv));
        ...
        if (!valid_local_names.contains_slow(definition->local_name()))
            return vm.throw_completion<JS::TypeError>(MUST(String::formatted("Local name '{}' of customized built-in element is not a valid local name for HTMLAudioElement", definition->local_name())));
    ...
    if (definition->construction_stack().is_empty()) {
        ...
        auto element = realm.create<HTML::HTMLAudioElement>(window.associated_document(), DOM::QualifiedName { definition->local_name(), {}, Namespace::HTML });
        ...
        // Important steps from "internally create a new object implementing the interface"
        {
            ...
            if (!prototype.is_object()) {
                ...
                VERIFY(target_realm);
                prototype = &Bindings::ensure_web_prototype<HTMLAudioElementPrototype>(*target_realm, "HTMLAudioElement"_fly_string);
            }
    ...
    if (!prototype.is_object()) {
        ...
        prototype = &Bindings::ensure_web_prototype<HTMLAudioElementPrototype>(*function_realm, "HTMLAudioElement"_fly_string);
    }
...
void HTMLAudioElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<HTMLMediaElementPrototype>(realm, "HTMLMediaElement"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLAudioElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLAudioElementPrototype>(realm, "HTMLAudioElement"_fly_string), 0);
