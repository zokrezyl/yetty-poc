...
GC_DEFINE_ALLOCATOR(UIEventConstructor);

UIEventConstructor::UIEventConstructor(JS::Realm& realm)
    : NativeFunction("UIEvent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "UIEvent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> UIEventConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "UIEventConstructor::construct");
    ...
    WebIDL::Long detail_value_0;

    if (!detail_property_value_0.is_undefined())

    detail_value_0 = TRY(WebIDL::convert_to_int<WebIDL::Long>(vm, detail_property_value_0, WebIDL::EnforceRange::No, WebIDL::Clamp::No));

    else
        detail_value_0 = static_cast<WebIDL::Long>(0);

    event_init_dict.detail = detail_value_0;

    auto view_property_value_1 = JS::js_undefined();
    if (arg1.is_object())
        view_property_value_1 = TRY(arg1.as_object().get("view"_utf16_fly_string));

    GC::Ptr<WindowProxy> view_value_1;

    if (!view_property_value_1.is_nullish()) {
        if (!view_property_value_1.is_object() || !is<WindowProxy>(view_property_value_1.as_object()))
            return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "WindowProxy");

        view_value_1 = &static_cast<WindowProxy&>(view_property_value_1.as_object());
    }

    event_init_dict.view = view_value_1;
...
void UIEventConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "UIEvent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<UIEventPrototype>(realm, "UIEvent"_fly_string), 0);
