...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/SVG/SVGRectElement.h>
...
GC_DEFINE_ALLOCATOR(SVGRectElementPrototype);

SVGRectElementPrototype::SVGRectElementPrototype([[maybe_unused]] JS::Realm& realm)
...
void SVGRectElementPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_rx_getter = JS::NativeFunction::create(realm, rx_getter, 0, "rx"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_rx_setter;

    define_direct_accessor("rx"_utf16_fly_string, native_rx_getter, native_rx_setter, default_attributes);

    auto native_ry_getter = JS::NativeFunction::create(realm, ry_getter, 0, "ry"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_ry_setter;

    define_direct_accessor("ry"_utf16_fly_string, native_ry_getter, native_ry_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "SVGRectElement"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void SVGRectElementPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<SVG::SVGRectElement*> impl_from(JS::VM& vm)
{
    ...
    if (!is<SVG::SVGRectElement>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "SVGRectElement");
    return static_cast<SVG::SVGRectElement*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(SVGRectElementPrototype::x_getter)
{
    WebIDL::log_trace(vm, "SVGRectElementPrototype::x_getter");
...
{
    WebIDL::log_trace(vm, "SVGRectElementPrototype::y_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "SVGRectElementPrototype::width_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "SVGRectElementPrototype::height_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "SVGRectElementPrototype::rx_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->rx(); }));
...
{
    WebIDL::log_trace(vm, "SVGRectElementPrototype::ry_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->ry(); }));
