...
GC_DEFINE_ALLOCATOR(InputEventPrototype);

InputEventPrototype::InputEventPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(realm, nullptr)

{
}

InputEventPrototype::~InputEventPrototype()
{
}

void InputEventPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_input_type_getter = JS::NativeFunction::create(realm, input_type_getter, 0, "inputType"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_input_type_setter;

    define_direct_accessor("inputType"_utf16_fly_string, native_input_type_getter, native_input_type_setter, default_attributes);

    define_native_function(realm, "getTargetRanges"_utf16_fly_string, get_target_ranges, 0, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "InputEvent"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void InputEventPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<UIEvents::InputEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<UIEvents::InputEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "InputEvent");
    return static_cast<UIEvents::InputEvent*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(InputEventPrototype::data_getter)
{
    WebIDL::log_trace(vm, "InputEventPrototype::data_getter");
...
JS_DEFINE_NATIVE_FUNCTION(InputEventPrototype::is_composing_getter)
{
    WebIDL::log_trace(vm, "InputEventPrototype::is_composing_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
JS_DEFINE_NATIVE_FUNCTION(InputEventPrototype::input_type_getter)
{
    WebIDL::log_trace(vm, "InputEventPrototype::input_type_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->input_type(); }));

    return JS::PrimitiveString::create(vm, retval);

}

JS_DEFINE_NATIVE_FUNCTION(InputEventPrototype::get_target_ranges)
{
    WebIDL::log_trace(vm, "InputEventPrototype::get_target_ranges");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_target_ranges(); }));
...
JS::Value wrapped_element0;
    ...
    wrapped_element0 = &const_cast<StaticRange&>(*element0);
