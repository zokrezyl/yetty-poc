...
GC_DEFINE_ALLOCATOR(DOMPointConstructor);

DOMPointConstructor::DOMPointConstructor(JS::Realm& realm)
    : NativeFunction("DOMPoint"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DOMPointConstructor::~DOMPointConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DOMPointConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DOMPoint");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DOMPointConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DOMPointConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DOMPoint:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DOMPointPrototype>(*target_realm, "DOMPoint"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return Geometry::DOMPoint::construct_impl(realm, x, y, z, w); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DOMPoint" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within Geometry::DOMPoint::construct_impl().
...
void DOMPointConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<DOMPointReadOnlyPrototype>(realm, "DOMPointReadOnly"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DOMPoint"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DOMPointPrototype>(realm, "DOMPoint"_fly_string), 0);
...
JS_DEFINE_NATIVE_FUNCTION(DOMPointConstructor::from_point)
{
    WebIDL::log_trace(vm, "DOMPointConstructor::from_point");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return Geometry::DOMPoint::from_point(vm, other); }));
