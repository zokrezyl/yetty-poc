...
GC_DEFINE_ALLOCATOR(CustomElementRegistryPrototype);

CustomElementRegistryPrototype::CustomElementRegistryPrototype([[maybe_unused]] JS::Realm& realm)
    : Object(ConstructWithPrototypeTag::Tag, realm.intrinsics().object_prototype())

{
}

CustomElementRegistryPrototype::~CustomElementRegistryPrototype()
{
}

void CustomElementRegistryPrototype::initialize(JS::Realm& realm)
{
    ...
    define_native_function(realm, "define"_utf16_fly_string, define, 2, default_attributes);

    define_native_function(realm, "get"_utf16_fly_string, get, 1, default_attributes);

    define_native_function(realm, "getName"_utf16_fly_string, get_name, 1, default_attributes);

    define_native_function(realm, "whenDefined"_utf16_fly_string, when_defined, 1, default_attributes);

    define_native_function(realm, "upgrade"_utf16_fly_string, upgrade, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "CustomElementRegistry"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void CustomElementRegistryPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<CustomElementRegistry*> impl_from(JS::VM& vm)
{
    ...
    if (!is<CustomElementRegistry>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CustomElementRegistry");
    return static_cast<CustomElementRegistry*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(CustomElementRegistryPrototype::define)
{
    WebIDL::log_trace(vm, "CustomElementRegistryPrototype::define");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "define", "2");
    ...
    auto constructor = vm.heap().allocate<WebIDL::CallbackType>(arg1.as_object(), HTML::incumbent_realm(), WebIDL::OperationReturnsPromise::No);

    auto arg2 = vm.argument(2);

    if (!arg2.is_nullish() && !arg2.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "ElementDefinitionOptions");

    ElementDefinitionOptions options {};

    auto extends_property_value_0 = JS::js_undefined();
    if (arg2.is_object())
        extends_property_value_0 = TRY(arg2.as_object().get("extends"_utf16_fly_string));

    if (!extends_property_value_0.is_undefined()) {

    Optional<String> extends_value_0;

    if (!extends_property_value_0.is_undefined()) {
        if (!false || !extends_property_value_0.is_null())
            extends_value_0 = TRY(WebIDL::to_string(vm, extends_property_value_0));
    }

    options.extends = extends_value_0;
    ...
    auto retval_or_exception = throw_dom_exception_if_needed(vm, [&] { return impl->define(name, constructor, options); });
...
JS_DEFINE_NATIVE_FUNCTION(CustomElementRegistryPrototype::get)
{
    WebIDL::log_trace(vm, "CustomElementRegistryPrototype::get");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    return retval.visit(
        ...
        [&vm, &realm]([[maybe_unused]] GC::Root<WebIDL::CallbackType> const& visited_union_value0) -> JS::Value {
            // These may be unused.
            (void)vm;
            (void)realm;

  return visited_union_value0->callback;
...
JS_DEFINE_NATIVE_FUNCTION(CustomElementRegistryPrototype::get_name)
{
    WebIDL::log_trace(vm, "CustomElementRegistryPrototype::get_name");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "getName");
    ...
    auto constructor = vm.heap().allocate<WebIDL::CallbackType>(arg0.as_object(), HTML::incumbent_realm(), WebIDL::OperationReturnsPromise::No);

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->get_name(constructor); }));
...
JS_DEFINE_NATIVE_FUNCTION(CustomElementRegistryPrototype::when_defined)
{
    WebIDL::log_trace(vm, "CustomElementRegistryPrototype::when_defined");
    [[maybe_unused]] auto& realm = *vm.current_realm();
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "whenDefined");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->when_defined(name); }));
...
JS_DEFINE_NATIVE_FUNCTION(CustomElementRegistryPrototype::upgrade)
{
    WebIDL::log_trace(vm, "CustomElementRegistryPrototype::upgrade");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "upgrade");
    ...
    auto retval_or_exception = throw_dom_exception_if_needed(vm, [&] { return impl->upgrade(root); });
