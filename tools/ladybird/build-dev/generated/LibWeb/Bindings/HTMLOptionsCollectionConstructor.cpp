...
GC_DEFINE_ALLOCATOR(HTMLOptionsCollectionConstructor);

HTMLOptionsCollectionConstructor::HTMLOptionsCollectionConstructor(JS::Realm& realm)
    : NativeFunction("HTMLOptionsCollection"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "HTMLOptionsCollection");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> HTMLOptionsCollectionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "HTMLOptionsCollectionConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "HTMLOptionsCollection");
}


void HTMLOptionsCollectionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "HTMLOptionsCollection"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<HTMLOptionsCollectionPrototype>(realm, "HTMLOptionsCollection"_fly_string), 0);
