...
GC_DEFINE_ALLOCATOR(TextPrototype);

TextPrototype::TextPrototype([[maybe_unused]] JS::Realm& realm)
...
void TextPrototype::initialize(JS::Realm& realm)
{
    ...
    set_prototype(&ensure_web_prototype<CharacterDataPrototype>(realm, "CharacterData"_fly_string));


    auto native_whole_text_getter = JS::NativeFunction::create(realm, whole_text_getter, 0, "wholeText"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_whole_text_setter;

    define_direct_accessor("wholeText"_utf16_fly_string, native_whole_text_getter, native_whole_text_setter, default_attributes);
    ...
    define_native_function(realm, "splitText"_utf16_fly_string, split_text, 1, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "Text"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void TextPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<DOM::Text*> impl_from(JS::VM& vm)
{
    ...
    if (!is<DOM::Text>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "Text");
    return static_cast<DOM::Text*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(TextPrototype::whole_text_getter)
{
    WebIDL::log_trace(vm, "TextPrototype::whole_text_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->whole_text(); }));

    return JS::PrimitiveString::create(vm, retval);

}

JS_DEFINE_NATIVE_FUNCTION(TextPrototype::assigned_slot_getter)
{
    WebIDL::log_trace(vm, "TextPrototype::assigned_slot_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();
...
{
    WebIDL::log_trace(vm, "TextPrototype::split_text");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    auto* impl = TRY(impl_from(vm));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "splitText");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->split_text(offset); }));
