...
GC_DEFINE_ALLOCATOR(CSSUnitValueConstructor);

CSSUnitValueConstructor::CSSUnitValueConstructor(JS::Realm& realm)
    : NativeFunction("CSSUnitValue"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSUnitValueConstructor::~CSSUnitValueConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSUnitValueConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSUnitValue");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSUnitValueConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSUnitValueConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSUnitValue:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSUnitValuePrototype>(*target_realm, "CSSUnitValue"_fly_string);
    }
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountMany, "CSSUnitValue", "2");
    ...
    if (!false || !arg1.is_null()) {
        unit = TRY(WebIDL::to_usv_string(vm, arg1));
    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSUnitValue::construct_impl(realm, value, unit); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSUnitValue" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSUnitValue::construct_impl().
...
void CSSUnitValueConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSUnitValue"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSUnitValuePrototype>(realm, "CSSUnitValue"_fly_string), 0);
