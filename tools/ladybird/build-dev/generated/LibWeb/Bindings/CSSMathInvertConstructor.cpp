...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMathInvert.h>
...
GC_DEFINE_ALLOCATOR(CSSMathInvertConstructor);

CSSMathInvertConstructor::CSSMathInvertConstructor(JS::Realm& realm)
    : NativeFunction("CSSMathInvert"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMathInvertConstructor::~CSSMathInvertConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMathInvertConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMathInvert");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMathInvertConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMathInvertConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSMathInvert:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSMathInvertPrototype>(*target_realm, "CSSMathInvert"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSMathInvert");
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSMathInvert::construct_impl(realm, arg); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSMathInvert" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSMathInvert::construct_impl().
...
void CSSMathInvertConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMathInvert"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMathInvertPrototype>(realm, "CSSMathInvert"_fly_string), 0);
