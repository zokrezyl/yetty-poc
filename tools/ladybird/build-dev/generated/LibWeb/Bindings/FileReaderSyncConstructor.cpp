...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/FileAPI/FileReaderSync.h>
...
GC_DEFINE_ALLOCATOR(FileReaderSyncConstructor);

FileReaderSyncConstructor::FileReaderSyncConstructor(JS::Realm& realm)
    : NativeFunction("FileReaderSync"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

FileReaderSyncConstructor::~FileReaderSyncConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> FileReaderSyncConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "FileReaderSync");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> FileReaderSyncConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "FileReaderSyncConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface FileReaderSync:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<FileReaderSyncPrototype>(*target_realm, "FileReaderSync"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return FileAPI::FileReaderSync::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface FileReaderSync" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within FileAPI::FileReaderSync::construct_impl().
...
void FileReaderSyncConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "FileReaderSync"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<FileReaderSyncPrototype>(realm, "FileReaderSync"_fly_string), 0);
