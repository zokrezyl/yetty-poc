...
GC_DEFINE_ALLOCATOR(DataTransferConstructor);

DataTransferConstructor::DataTransferConstructor(JS::Realm& realm)
    : NativeFunction("DataTransfer"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

DataTransferConstructor::~DataTransferConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> DataTransferConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "DataTransfer");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> DataTransferConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "DataTransferConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface DataTransfer:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<DataTransferPrototype>(*target_realm, "DataTransfer"_fly_string);
    }
    ...
    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return HTML::DataTransfer::construct_impl(realm); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface DataTransfer" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within HTML::DataTransfer::construct_impl().
...
void DataTransferConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "DataTransfer"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<DataTransferPrototype>(realm, "DataTransfer"_fly_string), 0);
