...
GC_DEFINE_ALLOCATOR(SVGAElementConstructor);

SVGAElementConstructor::SVGAElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGAElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

SVGAElementConstructor::~SVGAElementConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> SVGAElementConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGAElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGAElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGAElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGAElement");
}


void SVGAElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGAElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGAElementPrototype>(realm, "SVGAElement"_fly_string), 0);
