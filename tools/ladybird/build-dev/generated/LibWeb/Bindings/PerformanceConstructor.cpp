...
GC_DEFINE_ALLOCATOR(PerformanceConstructor);

PerformanceConstructor::PerformanceConstructor(JS::Realm& realm)
    : NativeFunction("Performance"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "Performance");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> PerformanceConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "PerformanceConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "Performance");
}


void PerformanceConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Performance"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<PerformancePrototype>(realm, "Performance"_fly_string), 0);
