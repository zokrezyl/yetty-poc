
#pragma once

#include <LibJS/Runtime/Object.h>

namespace Web::Bindings {

class SVGPatternElementPrototype : public JS::Object {
    JS_OBJECT(SVGPatternElementPrototype, JS::Object);
    GC_DECLARE_ALLOCATOR(SVGPatternElementPrototype);
public:
    static void define_unforgeable_attributes(JS::Realm&, JS::Object&);

    explicit SVGPatternElementPrototype(JS::Realm&);
    virtual void initialize(JS::Realm&) override;
    virtual ~SVGPatternElementPrototype() override;
private:

    JS_DECLARE_NATIVE_FUNCTION(x_getter);

    JS_DECLARE_NATIVE_FUNCTION(y_getter);

    JS_DECLARE_NATIVE_FUNCTION(width_getter);

    JS_DECLARE_NATIVE_FUNCTION(height_getter);

    JS_DECLARE_NATIVE_FUNCTION(view_box_getter);

    JS_DECLARE_NATIVE_FUNCTION(href_getter);


};


} // namespace Web::Bindings
    