...
GC_DEFINE_ALLOCATOR(WindowConstructor);

WindowConstructor::WindowConstructor(JS::Realm& realm)
    : NativeFunction("Window"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
JS::ThrowCompletionOr<GC::Ref<JS::Object>> WindowConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "WindowConstructor::construct");
...
void WindowConstructor::initialize(JS::Realm& realm)
{
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "Window"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<WindowPrototype>(realm, "Window"_fly_string), 0);
