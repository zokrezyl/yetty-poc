...
GC_DEFINE_ALLOCATOR(CSSGroupingRuleConstructor);

CSSGroupingRuleConstructor::CSSGroupingRuleConstructor(JS::Realm& realm)
    : NativeFunction("CSSGroupingRule"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSGroupingRule");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSGroupingRuleConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSGroupingRuleConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CSSGroupingRule");
}


void CSSGroupingRuleConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSGroupingRule"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSGroupingRulePrototype>(realm, "CSSGroupingRule"_fly_string), 0);
