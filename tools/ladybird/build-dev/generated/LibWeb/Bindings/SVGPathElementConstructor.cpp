...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/SVG/SVGPathElement.h>
...
GC_DEFINE_ALLOCATOR(SVGPathElementConstructor);

SVGPathElementConstructor::SVGPathElementConstructor(JS::Realm& realm)
    : NativeFunction("SVGPathElement"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}
...
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "SVGPathElement");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> SVGPathElementConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "SVGPathElementConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "SVGPathElement");
}


void SVGPathElementConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "SVGPathElement"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<SVGPathElementPrototype>(realm, "SVGPathElement"_fly_string), 0);
