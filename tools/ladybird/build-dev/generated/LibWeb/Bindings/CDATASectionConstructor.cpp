...
#if __has_include(<LibWeb/Bindings/TextPrototype.h>)
...
GC_DEFINE_ALLOCATOR(CDATASectionConstructor);

CDATASectionConstructor::CDATASectionConstructor(JS::Realm& realm)
    : NativeFunction("CDATASection"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CDATASectionConstructor::~CDATASectionConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CDATASectionConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CDATASection");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CDATASectionConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CDATASectionConstructor::construct");

    return vm().throw_completion<JS::TypeError>(JS::ErrorType::NotAConstructor, "CDATASection");
}


void CDATASectionConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<TextPrototype>(realm, "Text"_fly_string));

    define_direct_property(vm.names.length, JS::Value(0), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CDATASection"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CDATASectionPrototype>(realm, "CDATASection"_fly_string), 0);
