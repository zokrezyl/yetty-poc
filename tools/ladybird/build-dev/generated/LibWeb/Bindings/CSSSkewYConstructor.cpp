...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSSkewY.h>
...
GC_DEFINE_ALLOCATOR(CSSSkewYConstructor);

CSSSkewYConstructor::CSSSkewYConstructor(JS::Realm& realm)
    : NativeFunction("CSSSkewY"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSSkewYConstructor::~CSSSkewYConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSSkewYConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSSkewY");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSSkewYConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSSkewYConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSSkewY:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSSkewYPrototype>(*target_realm, "CSSSkewY"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSSkewY");
    ...
    auto& ay = static_cast<CSSNumericValue&>(arg0.as_object());

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSSkewY::construct_impl(realm, ay); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSSkewY" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSSkewY::construct_impl().
...
void CSSSkewYConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSSkewY"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSSkewYPrototype>(realm, "CSSSkewY"_fly_string), 0);
