...
GC_DEFINE_ALLOCATOR(CSSUnparsedValueConstructor);

CSSUnparsedValueConstructor::CSSUnparsedValueConstructor(JS::Realm& realm)
    : NativeFunction("CSSUnparsedValue"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSUnparsedValueConstructor::~CSSUnparsedValueConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSUnparsedValueConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSUnparsedValue");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSUnparsedValueConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSUnparsedValueConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSUnparsedValue:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSUnparsedValuePrototype>(*target_realm, "CSSUnparsedValue"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSUnparsedValue");
    ...
    Vector<Variant<String, GC::Root<CSSVariableReferenceValue>>> members;
    ...
    auto next_item1_to_variant = [&vm, &realm](JS::Value next_item1) -> JS::ThrowCompletionOr<Variant<String, GC::Root<CSSVariableReferenceValue>>> {
        ...
        if (next_item1.is_object()) {
            ...
            if (is<PlatformObject>(next_item1_object)) {
                ...
                if (auto* next_item1_result = as_if<CSSVariableReferenceValue>(next_item1_object))
    ...
    Variant<String, GC::Root<CSSVariableReferenceValue>> sequence_item1 = TRY(next_item1_to_variant(next_item1));

    members.append(sequence_item1);
    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSUnparsedValue::construct_impl(realm, members); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSUnparsedValue" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSUnparsedValue::construct_impl().
...
void CSSUnparsedValueConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<CSSStyleValuePrototype>(realm, "CSSStyleValue"_fly_string));

    define_direct_property(vm.names.length, JS::Value(1), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSUnparsedValue"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSUnparsedValuePrototype>(realm, "CSSUnparsedValue"_fly_string), 0);
