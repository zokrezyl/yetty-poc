...
#include </home/misi/work/my/yetty/tmp/ladybird/Libraries/LibWeb/CSS/CSSMatrixComponent.h>
...
GC_DEFINE_ALLOCATOR(CSSMatrixComponentConstructor);

CSSMatrixComponentConstructor::CSSMatrixComponentConstructor(JS::Realm& realm)
    : NativeFunction("CSSMatrixComponent"_utf16_fly_string, realm.intrinsics().function_prototype())
{
}

CSSMatrixComponentConstructor::~CSSMatrixComponentConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> CSSMatrixComponentConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(JS::ErrorType::ConstructorWithoutNew, "CSSMatrixComponent");
}


JS::ThrowCompletionOr<GC::Ref<JS::Object>> CSSMatrixComponentConstructor::construct([[maybe_unused]] FunctionObject& new_target)
{
    WebIDL::log_trace(vm(), "CSSMatrixComponentConstructor::construct");

    auto& vm = this->vm();
    auto& realm = *vm.current_realm();

    // To internally create a new object implementing the interface CSSMatrixComponent:
    ...
    if (!prototype.is_object()) {
        ...
        VERIFY(target_realm);
        prototype = &Bindings::ensure_web_prototype<CSSMatrixComponentPrototype>(*target_realm, "CSSMatrixComponent"_fly_string);
    }
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::BadArgCountOne, "CSSMatrixComponent");

    auto arg0 = vm.argument(0);

    if (!arg0.is_object() || !is<DOMMatrixReadOnly>(arg0.as_object()))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "DOMMatrixReadOnly");

    auto& matrix = static_cast<DOMMatrixReadOnly&>(arg0.as_object());

    auto arg1 = vm.argument(1);

    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "CSSMatrixComponentOptions");

    CSSMatrixComponentOptions options {};

    auto is2d_property_value_0 = JS::js_undefined();
    if (arg1.is_object())
        is2d_property_value_0 = TRY(arg1.as_object().get("is2D"_utf16_fly_string));
    ...
    options.is2d = is2d_value_0;

    }

    auto impl = TRY(throw_dom_exception_if_needed(vm, [&] { return CSS::CSSMatrixComponent::construct_impl(realm, matrix, options); }));

    // 7. Set instance.[[Prototype]] to prototype.
    VERIFY(prototype.is_object());
    impl->set_prototype(&prototype.as_object());

    // FIXME: Steps 8...11. of the "internally create a new object implementing the interface CSSMatrixComponent" algorithm
    // (https://webidl.spec.whatwg.org/#js-platform-objects) are currently not handled, or are handled within CSS::CSSMatrixComponent::construct_impl().
...
void CSSMatrixComponentConstructor::initialize(JS::Realm& realm)
{
    auto& vm = this->vm();
    ...
    set_prototype(&ensure_web_constructor<CSSTransformComponentPrototype>(realm, "CSSTransformComponent"_fly_string));

    define_direct_property(vm.names.length, JS::Value(1), JS::Attribute::Configurable);
    define_direct_property(vm.names.name, JS::PrimitiveString::create(vm, "CSSMatrixComponent"_string), JS::Attribute::Configurable);
    define_direct_property(vm.names.prototype, &ensure_web_prototype<CSSMatrixComponentPrototype>(realm, "CSSMatrixComponent"_fly_string), 0);
