...
GC_DEFINE_ALLOCATOR(SubmitEventPrototype);

SubmitEventPrototype::SubmitEventPrototype([[maybe_unused]] JS::Realm& realm)
...
void SubmitEventPrototype::initialize(JS::Realm& realm)
{
    ...
    auto native_submitter_getter = JS::NativeFunction::create(realm, submitter_getter, 0, "submitter"_utf16_fly_string, &realm, "get"sv);

    GC::Ptr<JS::NativeFunction> native_submitter_setter;

    define_direct_accessor("submitter"_utf16_fly_string, native_submitter_getter, native_submitter_setter, default_attributes);

    define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "SubmitEvent"_string), JS::Attribute::Configurable);

    Base::initialize(realm);

}

void SubmitEventPrototype::define_unforgeable_attributes(JS::Realm& realm, [[maybe_unused]] JS::Object& object)
{
...
[[maybe_unused]] static JS::ThrowCompletionOr<HTML::SubmitEvent*> impl_from(JS::VM& vm)
{
    ...
    if (!is<HTML::SubmitEvent>(this_object))
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAnObjectOfType, "SubmitEvent");
    return static_cast<HTML::SubmitEvent*>(this_object);
}

JS_DEFINE_NATIVE_FUNCTION(SubmitEventPrototype::submitter_getter)
{
    WebIDL::log_trace(vm, "SubmitEventPrototype::submitter_getter");
    [[maybe_unused]] auto& realm = *vm.current_realm();

    [[maybe_unused]] auto* impl = TRY(impl_from(vm));

    auto retval = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->submitter(); }));
