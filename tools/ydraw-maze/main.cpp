// ydraw-maze: Standalone terminal maze animation
//
// Renders an animated maze solver via YDrawBuffer, sends ydraw primitives
// to yetty via OSC 666666. Runs in alternate screen with a simple event loop.
...
static const char* g_cardName = "ydraw-maze-app";
...
int main(int argc, char** argv) {
    char logPath[256];
    std::snprintf(logPath, sizeof(logPath), "/tmp/ydraw-maze-%d.log", getpid());
    auto fileLogger = spdlog::basic_logger_mt("ydraw-maze", logPath, true);
    ...
    auto cfg = maze::parseArgs(rendererArgs);
    maze::MazeRenderer renderer(cfg);
