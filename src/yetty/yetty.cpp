...
#if defined(_WIN32) && !defined(__EMSCRIPTEN__)
    // Enumerate DXGI adapters for diagnostics
    {
        ...
        if (SUCCEEDED(CreateDXGIFactory1(__uuidof(IDXGIFactory1), (void**)&dxgiFactory))) {
            ...
            for (UINT i = 0; dxgiFactory->EnumAdapters1(i, &dxgiAdapter) != DXGI_ERROR_NOT_FOUND; i++) {
                ...
                yinfo("DXGI Adapter[{}]: {} VendorId=0x{:x} DeviceId=0x{:x} VRAM={}MB flags=0x{:x}{}",
                ...
                // Test D3D11 feature levels on this adapter
                if (!(desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE)) {
                    D3D_FEATURE_LEVEL testLevels[] = {
                        D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0,
                        D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_10_0
                    };
                    const char* levelNames[] = {"11_1", "11_0", "10_1", "10_0"};
                    for (int fl = 0; fl < 4; fl++) {
                        ID3D11Device* testDev = nullptr;
                        D3D_FEATURE_LEVEL achieved;
                        HRESULT hr = D3D11CreateDevice(dxgiAdapter, D3D_DRIVER_TYPE_UNKNOWN,
                            nullptr, 0, &testLevels[fl], 1, D3D11_SDK_VERSION,
                            &testDev, &achieved, nullptr);
                        if (SUCCEEDED(hr)) {
                            yinfo("  D3D11 FL {}: OK (achieved=0x{:x})", levelNames[fl], (int)achieved);
                            testDev->Release();
                        } else {
                            yinfo("  D3D11 FL {}: FAILED (hr=0x{:x})", levelNames[fl], (unsigned)hr);
                        }
                    }
                }
                dxgiAdapter->Release();
    ...
    if (_adapter) {
        ...
        if (wgpuAdapterGetInfo(_adapter, &info) == WGPUStatus_Success) {
            ...
            if (info.adapterType == WGPUAdapterType_CPU) {
                ...
                wgpuInstanceRequestAdapter(_instance, &adapterOpts, adapterCallbackInfo);
                // Check what D3D11 gave us
                if (_adapter) {
                    WGPUAdapterInfo d3d11Info = {};
                    if (wgpuAdapterGetInfo(_adapter, &d3d11Info) == WGPUStatus_Success) {
                        auto sv2 = [](WGPUStringView s) -> std::string {
                            return (s.data && s.length > 0) ? std::string(s.data, s.length) : "?";
                        };
                        yinfo("initWebGPU: D3D11 adapter: {} type={} vendor=0x{:x} arch={}",
                              sv2(d3d11Info.device), static_cast<int>(d3d11Info.adapterType),
                              d3d11Info.vendorID, sv2(d3d11Info.architecture));
                        wgpuAdapterInfoFreeMembers(d3d11Info);
                    }
                }
