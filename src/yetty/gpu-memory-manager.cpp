...
public:
    GpuMemoryManagerImpl(GPUContext* gpuContext, GpuAllocator::Ptr allocator,
    ...
    GpuTextureManager::Ptr textureManager() const override { return _textureMgr; }
...
private:
    ...
    GpuTextureManager::Ptr _textureMgr;
...
Result<GpuMemoryManager::Ptr> GpuMemoryManager::create(GPUContext* gpuContext,
                                              GpuAllocator::Ptr allocator,
                                              WGPUBuffer uniformBuffer,
                                              uint32_t uniformSize,
                                              Config config) noexcept {
    auto mgr = std::make_shared<GpuMemoryManagerImpl>(gpuContext, std::move(allocator), uniformBuffer, uniformSize, config);
    if (auto res = mgr->init(); !res) {
        return Err<Ptr>("GpuMemoryManager init failed", res);
    }
    return Ok(std::static_pointer_cast<GpuMemoryManager>(mgr));
}
...
Result<void> GpuMemoryManagerImpl::init() noexcept {
    ...
    auto texRes = GpuTextureManager::create(_gpuContext, _allocator, _config.texture);
    ...
    if (!shmRes) {
        ywarn("GpuMemoryManager: failed to create shared memory, streaming disabled: {}",
              shmRes.error().message());
        // Continue without shm - streaming won't work but local cards will
    } else {
        _shmData = *shmRes;
        ydebug("GpuMemoryManager: created shared memory '{}' ({} bytes)", shmName, INITIAL_SHM_SIZE);
    }
...
GpuMemoryManagerImpl::~GpuMemoryManagerImpl() {
...
Result<void> GpuMemoryManagerImpl::createMetadataGpuBuffer() {
...
Result<void> GpuMemoryManagerImpl::createDummyResources() {
...
Result<void> GpuMemoryManagerImpl::createSharedBindGroup() {
    if (_sharedBindGroup) {
    ...
    // Binding 1: metadata buffer (owned by GpuMemoryManager)
    ...
    ydebug("GpuMemoryManager: bind group created (atlas={})", atlasReady ? "real" : "dummy");
    return Ok();
}

Result<void> GpuMemoryManagerImpl::updateBindGroup() {
    bool atlasReady = _textureMgr && _textureMgr->atlasInitialized();
...
Result<void> GpuMemoryManagerImpl::flush(WGPUQueue queue) {
    // Flush metadata
    if (_metadataDirty.hasDirty()) {
        ...
        for (const auto& [offset, size] : ranges) {
            ydebug("GpuMemoryManager::flush: metadata GPU upload offset={} size={}", offset, size);
            wgpuQueueWriteBuffer(queue, _metadataGpuBuffer, offset,
                                 metadataBytes + offset, size);
        }
        _metadataDirty.clear();
    } else {
        ydebug("GpuMemoryManager::flush: no dirty metadata");
    }

    // Upload dirty atlas regions to GPU
    if (_textureMgr) {
        ydebug("GpuMemoryManager::flush: uploadAtlas");
        if (auto res = _textureMgr->uploadAtlas(queue); !res) {
            return Err<void>("GpuMemoryManager: atlas upload failed", res);
        }
    }

    // Update bind group if needed
    bool atlasReady = _textureMgr && _textureMgr->atlasInitialized();
    ydebug("GpuMemoryManager::flush: updateBindGroup atlasReady={} hasRealAtlas={}", atlasReady, _bindGroupHasRealAtlas);
    if (auto res = updateBindGroup(); !res) {
        return Err<void>("GpuMemoryManager: bind group update failed", res);
    }

    // Flush buffer manager
    if (_bufferMgr) {
        if (auto res = _bufferMgr->flush(queue); !res) {
            return Err<void>("GpuMemoryManager: buffer flush failed", res);
        }
    }
