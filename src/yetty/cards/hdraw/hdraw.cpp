...
//=============================================================================
// HDrawImpl - Implementation of HDraw
//=============================================================================

class HDrawImpl : public HDraw {
public:
    HDrawImpl(CardManager::Ptr mgr, const GPUContext& gpu,
              int32_t x, int32_t y,
...
private:
        ...
        while (iss >> token) {
            if (token == "--bg-color") {
                ...
                if (iss >> colorStr) {
                    ...
                    yinfo("HDraw::parseArgs: bgColor={:#010x}", _bgColor);
                }
    ...
    Result<void> parsePayload(const std::string& payload) {
        ...
        if (payload.empty()) {
            yinfo("HDraw::parsePayload: empty payload, nothing to parse");
            return Ok();
        }
    ...
    Result<void> uploadMetadata() {
        ...
        yinfo("HDraw::uploadMetadata: metaOffset={} slotIndex={} prims={} primOffset={} "
...
{
    yinfo("HDraw::create: ENTERED pos=({},{}) size={}x{} args='{}' payload_len={}",
    ...
    yinfo("HDraw::create: calling init()...");
    ...
    const std::string& payload) noexcept
{
    ...
    // Dynamic cast from CardPtr to HDraw::Ptr
    auto hdraw = std::dynamic_pointer_cast<HDraw>(*result);
    if (!hdraw) {
        return Err<Ptr>("Created card is not a HDraw");
    }
    return Ok(hdraw);
}
