...
public:
    HtmlContainerImpl(YDrawBase* drawBase, MsMsdfFont::Ptr font,
        ...
        litehtml::font_metrics* fm) override
    {
        ...
        // Resolve font via fontconfig and register with YDrawBase
        if (faceName && *faceName && _drawBase) {
            std::string ttfPath = resolveFontPath(
                faceName, weight, fi->italic);
            if (!ttfPath.empty()) {
                int id = _drawBase->addFont(ttfPath);
                if (id >= 0) {
                    fi->fontId = id;
        ...
        // Metrics (approximate — atlas metrics are per-glyph)
        float fontSize = fi->size;
        if (_font) {
            float fontBaseSize = _font->atlas()->getFontSize();
            float lineHeight = _font->atlas()->getLineHeight();
            float ratio = lineHeight / fontBaseSize;

            fi->ascent = fontSize * 0.8f;
            fi->descent = fontSize * 0.2f;
            fi->height = fontSize * ratio;
        ...
        const std::vector<litehtml::background_paint>& bg) override
    {
        ...
        for (const auto& paint : bg) {
            ...
            if (w > 0 && h > 0) {
                ...
                _drawBase->addBox(cx, cy, w * 0.5f, h * 0.5f,
        ...
        bool /*root*/) override
    {
        ...
        if (borders.top.width > 0 && borders.top.color.alpha > 0) {
            ...
            _drawBase->addSegment(x, y + bw * 0.5f,
        ...
        if (borders.bottom.width > 0 && borders.bottom.color.alpha > 0) {
            ...
            _drawBase->addSegment(x, y + h - bw * 0.5f,
        ...
        if (borders.left.width > 0 && borders.left.color.alpha > 0) {
            ...
            _drawBase->addSegment(x + bw * 0.5f, y,
        ...
        if (borders.right.width > 0 && borders.right.color.alpha > 0) {
            ...
            _drawBase->addSegment(x + w - bw * 0.5f, y,
...
private:
    struct FontInfo {
        ...
        float height;
        float xHeight;
        int fontId;  // YDrawBase font ID (0 = default)
    };
    ...
    float measureTextWidth(const char* text, const FontInfo& fi) const {
        if (!text || !*text || !_drawBase) return 0.0f;

        auto atlas = _drawBase->atlas();
...
Result<HtmlContainer::Ptr> HtmlContainer::createImpl(
    YDrawBase* drawBase, MsMsdfFont::Ptr font,
    float defaultFontSize, HttpFetcher* fetcher)
{
    return Ok(Ptr(new HtmlContainerImpl(drawBase, std::move(font),
