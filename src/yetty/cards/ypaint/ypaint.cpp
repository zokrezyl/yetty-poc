...
public:
  YPaintImpl(const YettyContext &ctx, int32_t x, int32_t y, uint32_t widthCells,
  ...
  Result<void> dispose() override {
    ...
    if (_metaHandle.isValid() && _cardMgr) {
      if (auto res = _cardMgr->deallocateMetadata(_metaHandle); !res) {
        yerror("YPaintImpl::dispose: deallocateMetadata failed: {}",
               error_msg(res));
      }
  ...
  Result<bool> onEvent(const base::Event &event) override {
    ...
    if (event.type == base::Event::Type::CardKeyDown &&
        event.cardKey.targetId == id() && event.cardKey.key == GLFW_KEY_C &&
        (event.cardKey.mods & GLFW_MOD_CONTROL)) {
      std::string text = _builder->getSelectedText();
      if (!text.empty()) {
        auto loop = *base::EventLoop::instance();
        loop->dispatch(
            base::Event::copyEvent(std::make_shared<std::string>(text)));
  ...
  Result<void> update(const std::string &args,
    ...
    ydebug("YPaintImpl::update: {} prims, {} text spans", _buffer->primCount(),
           _buffer->textSpanCount());
    return Ok();
  }
  ...
  Result<void> init() {
    ...
    auto builderRes = Painter::create(_fontManager, _gpuAllocator, _cardMgr,
                                      metadataSlotIndex());
    if (!builderRes) {
      return Err<void>("YPaintImpl::init: failed to create builder",
                       builderRes);
    }
    _builder = *builderRes;

    _builder->addFlags(Painter::FLAG_UNIFORM_SCALE);
    ...
    ydebug("YPaintImpl::init: {} prims, {} glyphs", _builder->primitiveCount(),
           _builder->glyphCount());
...
private:
  ...
  Result<void> registerForEvents() {
    ...
    if (!loopResult)
      return Err<void>("no EventLoop", loopResult);
  ...
  void startAnimation() {
    if (!_animation || !_animation->hasProperties())
      return;
    if (_buffer->empty())
      return;
    ...
    if (!_builder->hasExplicitBounds()) {
      ...
      _builder->setSceneBounds(minX - padX, minY - padY, maxX + padX,
                               maxY + padY);
    }
    _animation->play();
  }
  ...
  void parseArgs(const std::string &args) {
    ...
    while (iss >> token) {
      if (token == "--bg-color") {
        ...
        std::string s;
        if (iss >> s)
          _viewZoom = std::clamp(std::stof(s), 0.1f, 1000.0f);
      } else if (token == "--pan-y") {
        std::string s;
        if (iss >> s)
          _viewPanY = std::stof(s);
      } else if (token == "--yaml") {
        _yamlMode = true;
      } else if (token == "--stretch") {
        ydebug(">>> YPaint: --stretch flag, clearing FLAG_UNIFORM_SCALE (was {})",
               _builder->flags());
        _builder->setFlags(_builder->flags() & ~Painter::FLAG_UNIFORM_SCALE);
        ydebug(">>> YPaint: flags now {}", _builder->flags());
        ...
        if (iss >> s) {
          float p = std::stof(s);
          ydebug(">>> YPaint: --padding {} -> setPadding({}, {})", s, p, p);
  ...
  Result<void> parseBinary(const std::string &payload) {
    ...
    if (payload.empty())
      return Ok();
    if (payload.size() >= 8) {
      ydebug("parseBinary: first 8 bytes: {:02X} {:02X} {:02X} {:02X} {:02X} "
             "{:02X} {:02X} {:02X}",
             (uint8_t)payload[0], (uint8_t)payload[1], (uint8_t)payload[2],
             (uint8_t)payload[3], (uint8_t)payload[4], (uint8_t)payload[5],
             (uint8_t)payload[6], (uint8_t)payload[7]);
    }
    ...
    if (size >= 4) {
      ...
      if (magic == LZ4F_MAGICNUMBER) {
        ...
        ydebug("parseBinary: decompressed {} -> {} bytes", payload.size(),
               size);
      }
    }
  ...
  Result<void> decompressLZ4(const uint8_t *src, size_t srcSize,
                             std::vector<uint8_t> &dst) {
    LZ4F_dctx *dctx = nullptr;
    LZ4F_errorCode_t err = LZ4F_createDecompressionContext(&dctx, LZ4F_VERSION);
    if (LZ4F_isError(err)) {
      return Err<void>(std::string("LZ4F_createDecompressionContext: ") +
    ...
    if (LZ4F_isError(err)) {
      ...
      return Err<void>(std::string("LZ4F_getFrameInfo: ") +
                       LZ4F_getErrorName(err));
    }
    ...
    while (srcPos < srcSize) {
      ...
      if (LZ4F_isError(ret)) {
        LZ4F_freeDecompressionContext(dctx);
        return Err<void>(std::string("LZ4F_decompress: ") +
  ...
  // Old parseYAML/parseYAMLPrimitive/parseColor removed — now in
  // yaml2ypaint.cpp
  //=========================================================================
  // State
  //=========================================================================

  Painter::Ptr _builder;
...
Result<CardPtr> YPaint::create(const YettyContext &ctx, int32_t x, int32_t y,
...
Result<YPaint::Ptr> YPaint::createImpl(ContextType &ctx,
                                       const YettyContext &yettyCtx, int32_t x,
                                       int32_t y, uint32_t widthCells,
  ...
  if (!result)
    return Err<Ptr>("Failed to create YPaint", result);
  auto ypaint = std::dynamic_pointer_cast<YPaint>(*result);
  if (!ypaint)
    return Err<Ptr>("Created card is not a YPaint");
  return Ok(ypaint);
}
