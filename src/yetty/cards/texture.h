...
//=============================================================================
// Texture - Card for rendering textures via shader glyph + texture atlas
//
...
// Texture Flow:
// =============================================================================
// 1. OSC command received with base94-encoded texture payload (decoded by OSC parser)
// 2. TextureCard loads raw texture file data (via stb_image) and stores RGBA in cardImageData buffer
// 3. Compute shader copies visible card textures from buffer to atlas texture
// 4. Fragment shader samples from atlas using cell-relative UV coordinates
//
// The atlas is managed per-frame - only visible cards have their textures in it.
// This allows unlimited textures in scrollback without exhausting GPU memory.
//
// =============================================================================
// Metadata layout (64 bytes):
//   offset 0:  textureDataOffset (u32) - byte offset in cardImageData buffer
//   offset 4:  textureWidth (u32)      - texture width in pixels
//   offset 8:  textureHeight (u32)     - texture height in pixels
...
class Texture : public Card,
                public base::ObjectFactory<Texture> {
public:
    using Ptr = std::shared_ptr<Texture>;

    // Shader glyph codepoint for textures
    ...
    const char* typeName() const override { return "texture"; }

    //=========================================================================
    // Texture-specific API
    //=========================================================================

    // Set texture data (copies and uploads to GPU buffer)
    // Data should be RGBA8 pixels (4 bytes per pixel)
    virtual Result<void> setTextureData(const uint8_t* data, uint32_t width, uint32_t height) = 0;
...
protected:
    Texture(CardBufferManager::Ptr mgr, const GPUContext& gpu,
            int32_t x, int32_t y,
            uint32_t widthCells, uint32_t heightCells)
