...
//=============================================================================
// AABB utilities (shared with hdraw)
//=============================================================================

static void computeAABB_ydraw(SDFPrimitive& prim) {
...
//=============================================================================
// YDrawImpl - Implementation of YDraw with spatial hashing
//=============================================================================
...
public:
    ...
    void suspend() override {
        ...
        yinfo("YDraw::suspend: deallocated storage, saved {} primitives to staging", _primStaging.size());
    }
    ...
    uint32_t addPrimitive(const SDFPrimitive& prim) override {
        ...
        if (_primitives[idx].aabbMinX == 0 && _primitives[idx].aabbMaxX == 0) {
            computeAABB_ydraw(_primitives[idx]);
        }
        _cardMgr->bufferManager()->markBufferDirty(_primStorage);
        _dirty = true;
        return idx;
...
private:
    ...
    Result<void> parseBinary(const std::string& payload) {
        ...
        if (_initParsing) {
            ...
            _dirty = true;
            yinfo("YDraw::parseBinary: loaded {} primitives into staging", primCount);
            return Ok();
        }

        // Allocate buffer and copy directly from payload
        if (auto res = ensurePrimCapacity(primCount); !res) {
            return Err<void>("YDraw::parseBinary: failed to allocate prim storage");
        }
        ...
        _dirty = true;
        yinfo("YDraw::parseBinary: loaded {} primitives", _primCount);
    ...
    Result<void> uploadMetadata() {
        ...
        yinfo("YDraw::uploadMetadata: metaOffset={} prims={} grid={}x{} cellSize={} "
