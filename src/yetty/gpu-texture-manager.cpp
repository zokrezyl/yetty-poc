...
public:
    GpuTextureManagerImpl(GPUContext* gpuContext, GpuAllocator::Ptr allocator, Config config) noexcept;
    ~GpuTextureManagerImpl() override;
...
Result<GpuTextureManager::Ptr> GpuTextureManager::create(GPUContext* gpuContext,
                                                              GpuAllocator::Ptr allocator,
                                                              Config config) noexcept {
    auto mgr = std::make_shared<GpuTextureManagerImpl>(gpuContext, std::move(allocator), config);
    if (auto res = mgr->init(); !res) {
        return Err<Ptr>("Failed to initialize GpuTextureManager", res);
    }
...
GpuTextureManagerImpl::GpuTextureManagerImpl(GPUContext* gpuContext, GpuAllocator::Ptr allocator,
                                                 Config config) noexcept
...
GpuTextureManagerImpl::~GpuTextureManagerImpl() {
...
Result<void> GpuTextureManagerImpl::write(TextureHandle handle, const uint8_t* pixels) {
    ...
    ydebug("GpuTextureManager: write handle={} {}x{}", handle.id,
...
Result<void> GpuTextureManagerImpl::uploadAtlas(WGPUQueue queue) {
    ...
    for (auto& [id, data] : _textureHandles) {
        ...
        ydebug("GpuTextureManager: uploaded {}x{} pixels to atlas at ({}, {})",
...
Result<void> GpuTextureManagerImpl::createAtlasTexture() {
