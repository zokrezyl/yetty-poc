// Shader glyph: Fire (codepoint 1048580 / U+100004)
// Procedural noise-based fire effect

fn shaderGlyph_1048580_hash(p: vec2<f32>) -> f32 {
    return fract(sin(dot(p, vec2<f32>(127.1, 311.7))) * 43758.5453);
}

fn shaderGlyph_1048580_noise(p: vec2<f32>) -> f32 {
    let i = floor(p);
    let f = fract(p);
    let u = f * f * (3.0 - 2.0 * f);

    let a = shaderGlyph_1048580_hash(i);
    let b = shaderGlyph_1048580_hash(i + vec2<f32>(1.0, 0.0));
    let c = shaderGlyph_1048580_hash(i + vec2<f32>(0.0, 1.0));
    let d = shaderGlyph_1048580_hash(i + vec2<f32>(1.0, 1.0));

    return mix(mix(a, b, u.x), mix(c, d, u.x), u.y);
}

fn shaderGlyph_1048580_fbm(p: vec2<f32>) -> f32 {
    var v = 0.0;
    var a = 0.5;
    var q = p;
    for (var i = 0; i < 4; i++) {
        v += a * shaderGlyph_1048580_noise(q);
        q = q * 2.0;
        a *= 0.5;
    }
    return v;
...
fn shaderGlyph_1048580(localUV: vec2<f32>, time: f32, fg: u32, bg: u32, pixelPos: vec2<f32>, mousePos: vec2<f32>) -> vec3<f32> {
    ...
    // Animate
    let n = shaderGlyph_1048580_fbm(vec2<f32>(p.x * 4.0, p.y * 3.0 - time * 2.0));
