// Shader glyph: JDraw widget (codepoint 1048581 / U+100005)
// GPU-accelerated SDF primitives with GPU COMPUTE SHADER TILE CULLING
// + MSDF text rendering
//
...
// Architecture:
// =============================================================================
// Identical to KDraw fragment shader, but tile lists are built by a GPU
// compute shader instead of CPU. Used for benchmarking GPU vs CPU tile culling.
//
// =============================================================================
...
// =============================================================================
// JDrawGlyph layout (32 bytes = 8 floats):
...
const JDRAW_FLAG_SHOW_BOUNDS: u32 = 1u;
const JDRAW_FLAG_SHOW_TILES: u32 = 2u;
const JDRAW_FLAG_SHOW_EVAL_COUNT: u32 = 4u;

// Hardcoded max primitives per tile (matches C++ MAX_PRIMS_PER_TILE)
const JDRAW_MAX_PRIMS_PER_TILE: u32 = 16u;
const JDRAW_TILE_SIZE: u32 = 8u;

// =============================================================================
// Main shader function - TILE-BASED RENDERING
// =============================================================================

fn shaderGlyph_1048581(localUV: vec2<f32>, time: f32, fg: u32, bg: u32, pixelPos: vec2<f32>, mousePos: vec2<f32>, lastChar: u32, lastCharTime: f32) -> vec3<f32> {
    ...
    if (primitiveCount > 0u && tileCountX > 0u && tileCountY > 0u) {
        ...
        let tileStride = 1u + JDRAW_MAX_PRIMS_PER_TILE;
        ...
        let maxPrimsToCheck = min(tilePrimCount, JDRAW_MAX_PRIMS_PER_TILE);
    ...
    if ((flags & JDRAW_FLAG_SHOW_TILES) != 0u) {
    ...
    if ((flags & JDRAW_FLAG_SHOW_BOUNDS) != 0u) {
        let edgeDist = min(
    ...
    if ((flags & JDRAW_FLAG_SHOW_EVAL_COUNT) != 0u) {
