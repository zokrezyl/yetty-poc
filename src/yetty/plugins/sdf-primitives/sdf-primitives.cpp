...
Result<void> SDFPrimitivesLayer::render(WebGPUContext& ctx) {
    ...
    if (!_gpu_initialized) {
        ...
        if (!result) {
            _failed = true;
            return Err<void>("SDFPrimitivesLayer: Failed to create pipeline", result);
        }
    ...
    if (!encoder) {
        return Err<void>("SDFPrimitivesLayer: Failed to create command encoder");
    }
    ...
    if (!pass) {
        ...
        return Err<void>("SDFPrimitivesLayer: Failed to begin render pass");
    }
    ...
    if (!cmdBuffer) {
        wgpuCommandEncoderRelease(encoder);
        return Err<void>("SDFPrimitivesLayer: Failed to finish command encoder");
    }
