...
fn fs_main(uv: vec2<f32>) -> vec4<f32> {
    let p = (uv - 0.5) * 2.0;
    let blink = step(0.9, fract(iTime * 0.5));
    ...
    let leftEye = sdCircle(vec2<f32>(p.x + 0.3, (p.y - 0.2) / (eyeH / 0.12)), 0.12);
    let rightEye = sdCircle(vec2<f32>(p.x - 0.3, (p.y - 0.2) / (eyeH / 0.12)), 0.12);
