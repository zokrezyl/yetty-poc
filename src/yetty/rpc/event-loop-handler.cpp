...
static msgpack::object_handle okResult() {
...
static msgpack::object_handle boolResult(bool value) {
    msgpack::sbuffer sbuf;
...
Result<void> registerEventLoopHandlers(RpcServer& server) {
    ...
    server.registerHandler(Channel::EventLoop, "key_down", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
        auto event = base::Event::keyDown(
            params.via.map.ptr[0].val.as<int>(),   // key
            params.via.map.ptr[1].val.as<int>(),    // mods
            params.via.map.ptr[2].val.as<int>()     // scancode
        );
    ...
    server.registerHandler(Channel::EventLoop, "key_up", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "char", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "mouse_down", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "mouse_up", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "mouse_move", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "mouse_drag", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "scroll", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "set_focus", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "resize", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "context_menu_action", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "card_mouse_down", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "card_mouse_up", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "card_mouse_move", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "card_scroll", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
    ...
    server.registerHandler(Channel::EventLoop, "close", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
        ...
        auto res = eventLoop->dispatch(event);
        if (!res) {
            throw std::runtime_error(error_msg(res));
        }
        return okResult();
    });

    // split: {object_id: uint64, orientation: int}
    server.registerHandler(Channel::EventLoop, "split", [eventLoop](const RpcMessage& msg) {
        auto& params = msg.params.get();
...
// Helper: create a string response as msgpack object_handle
static msgpack::object_handle stringResult(const std::string& value) {
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, value);
    return msgpack::unpack(sbuf.data(), sbuf.size());
}
...
Result<void> registerWorkspaceHandlers(RpcServer& server, std::shared_ptr<Workspace> workspace) {
    ...
    server.registerHandler(Channel::EventLoop, "ui_tree",
        [workspace](const RpcMessage& /*msg*/) {
