...
// DamageRect is in separate header to avoid libuv dependency for grid-renderer
// on web
...
public:
  ...
  int getCursorRow() const {
    return _gpuScreen ? _gpuScreen->getCursorRow() : 0;
  }
  int getCursorCol() const {
    return _gpuScreen ? _gpuScreen->getCursorCol() : 0;
  }
  ...
  int getScrollOffset() const {
    return _gpuScreen ? _gpuScreen->getScrollOffset() : 0;
  }
  bool isScrolledBack() const {
    return _gpuScreen ? _gpuScreen->isScrolledBack() : false;
  }
  size_t getScrollbackSize() const {
    return _gpuScreen ? _gpuScreen->getScrollbackSize() : 0;
  }
  ...
  const std::vector<WidgetPtr> &getChildWidgets() const {
    static std::vector<WidgetPtr> empty;
    return _gpuScreen ? _gpuScreen->getChildWidgets() : empty;
  }
...
private:
  ...
  // Widget position update on scroll (called when lines are pushed/popped from
  // scrollback)
  ...
  // Note: cursor position is now tracked by GPUScreen (getCursorRow/Col
  // delegate to it)
  ...
  CardBufferManager *_cardBufferManager = nullptr;

  // Child widgets are now managed by GPUScreen
