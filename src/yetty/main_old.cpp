...
// - Render thread: Runs libuv event loop, processes events, renders
//
// Event flow:
// - GLFW callbacks (main thread) push events to EventQueue
// - EventQueue wakes render thread via libuv async
// - Render thread dispatches events to listeners
//-----------------------------------------------------------------------------
#include "platform/shared/glfw-window-singleton.h"
...
int main(int argc, char* argv[]) {
    ...
    // VERSION MARKER
    ydebug("=== YETTY BUILD VERSION 004 ===");
    spdlog::cfg::load_env_levels();

    // Debug: catch SIGINT to see if process receives it
    signal(SIGINT, sigint_handler);

    // 1. Create window (must be main thread for GLFW)
    auto windowResult = yetty::GlfwWindowSingleton::instance();
    if (!windowResult) {
        yerror("Failed to create window: {}", yetty::error_msg(windowResult));
        std::cerr << "ERROR: Failed to create window: " << yetty::error_msg(windowResult) << std::endl;
        return 1;
    }

    // 2. Create InitManager (sets up callbacks, will spawn render thread)
    auto initResult = yetty::InitManager::instance();
    if (!initResult) {
    ...
    // Exit code from render thread
    std::atomic<int> exitCode{0};

    // Run: spawns render thread, blocks main thread on event loop
    initManager->run([&]() {
        // === RENDER THREAD ===
        ...
        if (!result) {
            ...
            if (msg == "Help requested") {
                return;  // InitManager::run() will return
            }
        ...
        // InitManager::run() will return after this
    });
...
int main(int argc, char* argv[]) {
    ...
    g_yetty = *result;
    auto runResult = g_yetty->run();
    // Note: run() doesn't return on web (emscripten takes over)
    return 0;
}
#endif
