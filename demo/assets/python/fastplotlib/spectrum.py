...
def init(width_cells, height_cells):
    ...
    heights = np.random.rand(_N_BARS)
    _bars = []
    for i in range(_N_BARS):
        data = np.array([[i, 0], [i, heights[i]]])
        bar = _fig[0, 0].add_line(data, thickness=8, cmap="cool", cmap_transform=[0, 1])
        _bars.append(bar)
    ...
    def animate():
        ...
        for i, bar in enumerate(_bars):
            h = (
                0.3 + 0.3 * np.sin(_t + i * 0.2) +
                0.2 * np.sin(_t * 2 + i * 0.1) +
                0.1 * np.sin(_t * 5 + i * 0.05) +
                0.1 * np.random.rand()
            )
