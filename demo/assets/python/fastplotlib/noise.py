...
def init(width_cells, height_cells):
    global _tex, _fig, _np, _current_data, _image
    ...
    size = 200
    _current_data = np.random.rand(size, size).astype(np.float32)
    _image = _fig[0, 0].add_image(_current_data, cmap="magma")

    def animate():
        global _current_data
        noise = np.random.rand(size, size).astype(np.float32)
        _current_data = 0.95 * _current_data + 0.05 * noise
        _image.data = _current_data
...
def shutdown():
    global _tex, _fig, _image, _current_data
